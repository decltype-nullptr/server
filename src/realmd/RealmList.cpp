...
// will only support WoW 1.12.1/1.12.2/1.12.3 , WoW:TBC 2.4.3 and official release for WoW:WotLK and
// later, client builds 10505, 8606, 6141, 6005, 5875 if you need more from old build then add it in
// cases in realmd sources code list sorted from high to low build and first build used as low bound
// for accepted by default range (any > it will accepted by realmd at least)

static RealmBuildInfo ExpectedRealmdClientBuilds[] = {
    {12340, 3, 3, 5, 'a'}, // highest supported build, also auto accept all above for simplify
...
void RealmList::UpdateRealms(bool init)
{
    ...
    if (result)
    {
        do
        {
            ...
            if (realmflags & ~(REALM_FLAG_OFFLINE | REALM_FLAG_NEW_PLAYERS |
                               REALM_FLAG_RECOMMENDED | REALM_FLAG_SPECIFYBUILD))
            {
                sLog.outError("Realm allowed have only OFFLINE Mask 0x2), or NEWPLAYERS (mask "
                              "0x20), or RECOMENDED (mask 0x40), or SPECIFICBUILD (mask 0x04) "
                              "flags in DB");
                realmflags &= (REALM_FLAG_OFFLINE | REALM_FLAG_NEW_PLAYERS |
                               REALM_FLAG_RECOMMENDED | REALM_FLAG_SPECIFYBUILD);
            }

            UpdateRealm(fields[ 0 ].GetUInt32(),
                        fields[ 1 ].GetCppString(),
