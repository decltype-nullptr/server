...
// GCC have alternative #pragma pack(N) syntax and old gcc version not support pack(push,N), also
// any gcc version not support it at some paltform
#if defined(__GNUC__)
#pragma pack(1)
#else
#pragma pack(push, 1)
...
// GCC have alternative #pragma pack() syntax and old gcc version not support pack(pop), also any
// gcc version not support it at some paltform
#if defined(__GNUC__)
#pragma pack()
#else
#pragma pack(pop)
...
void AuthSocket::OnRead()
{
    ...
    while (1)
    {
        ...
        for (i = 0; i < AUTH_TOTAL_COMMANDS; ++i)
        {
            ...
            {
                DEBUG_LOG(
                    "[Auth] Received unauthorized command %u length %u", _cmd, (uint32)recv_len());
                return;
            }
            ...
            {
                DEBUG_LOG("[Auth] Command handler failed for cmd %u recv length %u",
                          _cmd,
                          (uint32)recv_len());
                close_connection();
                return;
...
void AuthSocket::_SetVSFields(const std::string& rI)
{
    ...
    v_hex = v.AsHexStr();
    s_hex = s.AsHexStr();
    LoginDatabase.PExecute("UPDATE account SET v = '%s', s = '%s' WHERE username = '%s'",
                           v_hex,
                           s_hex,
                           _safelogin.c_str());
...
bool AuthSocket::_HandleLogonChallenge()
{
    ...
    auto result =
        LoginDatabase.PQuery("SELECT unbandate FROM ip_banned WHERE "
    ...
    else
    {
        ...
        result =
            LoginDatabase.PQuery("SELECT sha_pass_hash,id,locked,last_ip,v,s,security,email_verif "
        ...
        if (result)
        {
            ...
            {
                BASIC_LOG("[AuthChallenge] Account's email address requires email verification - "
                          "rejecting login");
            ...
            ///- If the IP is 'locked', check that the player comes indeed from the correct IP
            /// address
            bool locked  = false;
            ...
            if ((lockFlags & IP_LOCK) == IP_LOCK)
            {
                ...
                DEBUG_LOG(
                    "[AuthChallenge] Account '%s' is locked to IP - '%s'", _login.c_str(), last_ip);
            ...
            if (!locked ||
                (locked && ((lockFlags & FIXED_PIN) == FIXED_PIN || (lockFlags & TOTP) == TOTP)))
            {
                ...
                if (banresult)
                {
                    if ((*banresult)[ 0 ].GetUInt64() == (*banresult)[ 1 ].GetUInt64())
                    {
                        ...
                        BASIC_LOG("[AuthChallenge] Banned account %s tries to login!",
                                  _login.c_str());
                    }
                    else
                    {
                        pkt << (uint8)WOW_FAIL_SUSPENDED;
                        BASIC_LOG("[AuthChallenge] Temporarily banned account %s tries to login!",
                                  _login.c_str());
                    }
                }
                else
                {
                    ///- Get the password from the account table, upper it, and make the SRP6
                    /// calculation
                    ...
                    promptPin =
                        locked; // always prompt if the account is IP locked & 2FA is enabled

                    if (!locked && ((lockFlags & ALWAYS_ENFORCE) == ALWAYS_ENFORCE))
                    {
                        promptPin = true; // prompt if the lock hasn't been triggered but
                                          // ALWAYS_ENFORCE is set
                    }

                    if (promptPin)
                    {
                        BASIC_LOG("[AuthChallenge] account %s is using PIN authentication",
                                  _login.c_str());
                        ...
                        pkt << uint8(
                            1); // securityFlags, only '1' is available in classic (PIN input)
                        pkt << gridSeedPkt;
...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (promptPin && lp.securityFlags)
    {
        ...
        else
        {
            pinResult = false;
            sLog.outError(
                "[ERROR] Invalid PIN flags set for user %s - user cannot log-in until fixed",
                _login.c_str());
        }
    }
    ...
    if (!memcmp(M.AsByteArray().data(), lp.M1, 20) && pinResult && approvedBuild)
    {
        ...
        ///- Update the sessionkey, last_ip, last login time and reset number of failed logins in
        /// the account table for this account
        // No SQL injection (escaped user name) and IP address as received by socket
        const char* K_hex = K.AsHexStr();
        const char* os =
            reinterpret_cast<char*>(&_os); // no injection as there are only two possible values
        auto result = LoginDatabase.PQuery("UPDATE account SET sessionkey = '%s', last_ip = '%s', "
                                           "last_login = NOW(), locale = '%u', failed_logins = 0, "
    ...
    else
    {
        ...
        {
            // Increment number of failed logins by one and if it reaches the limit temporarily ban
            // that account or IP
            LoginDatabase.PExecute(
                "UPDATE account SET failed_logins = failed_logins + 1 WHERE username = '%s'",
                _safelogin.c_str());

            if (auto loginfail = LoginDatabase.PQuery(
                ...
                if (failed_logins >= MaxWrongPassCount)
                {
                    ...
                    if (WrongPassBanType)
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO account_banned VALUES "
                                               "('%u',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','"
                                               "MaNGOS realmd','Failed login autoban',1,1,0)",
                                               acc_id,
                                               WrongPassBanTime);
                        BASIC_LOG("[AuthChallenge] account %s got banned for '%u' seconds because "
                                  "it failed to authenticate '%u' times",
                                  _login.c_str(),
                                  WrongPassBanTime,
                                  failed_logins);
                    }
                    else
                    {
                        ...
                        LoginDatabase.PExecute("INSERT INTO ip_banned VALUES "
                                               "('%s',UNIX_TIMESTAMP(),UNIX_TIMESTAMP()+'%u','"
                                               "MaNGOS realmd','Failed login autoban')",
                                               current_ip.c_str(),
                                               WrongPassBanTime);
                        BASIC_LOG("[AuthChallenge] IP %s got banned for '%u' seconds because "
                                  "account %s failed to authenticate '%u' times",
                                  current_ip.c_str(),
                                  WrongPassBanTime,
                                  _login.c_str(),
                                  failed_logins);
...
bool AuthSocket::_HandleReconnectChallenge()
{
    ...
    auto result = LoginDatabase.PQuery("SELECT sessionkey,id FROM account WHERE username = '%s'",
                                       _safelogin.c_str());
...
bool AuthSocket::_HandleRealmList()
{
    ...
    {
        sLog.outError(
            "[ERROR] user %s IP %s is sending CMD_REALM_LIST too frequently.  Delay = %d seconds",
            _login.c_str(),
            get_remote_address().c_str(),
            delay);
        return false;
    ...
    ///- Circle through realms in the RealmList and construct the return packet (including # of user
    /// characters in each realm)
    ByteBuffer pkt;
    LoadRealmlist(pkt);
...
void AuthSocket::LoadRealmlist(ByteBuffer& pkt)
{
    switch (_build)
    {
        ...
        case 6141: // 1.12.3
        {
            ...
            for (RealmList::RealmMap::const_iterator i = sRealmList.begin(); i != sRealmList.end();
                 ++i)
            {
                ...
                bool ok_build =
                    std::find(i->second.realmbuilds.begin(), i->second.realmbuilds.end(), _build) !=
                ...
                // 1.x clients not support explicitly REALM_FLAG_SPECIFYBUILD, so manually form
                // similar name as show in more recent clients
                ...
                if (realmflags & REALM_FLAG_SPECIFYBUILD)
                {
                    ...
                    snprintf(buf,
                             20,
                             " (%u,%u,%u)",
                             buildInfo->major_version,
                             buildInfo->minor_version,
                             buildInfo->bugfix_version);
                    name += buf;
                }

                // Show offline state for unsupported client builds and locked realms (1.x clients
                // not support locked state show)
        ...
        default:    // and later
        {
            ...
            for (const auto& i : sRealmList)
            {
                ...
                pkt << uint8(
                    i.second.icon);       // realm type (this is second column in Cfg_Configs.dbc)
...
uint32 AuthSocket::GenerateTotpPin(const std::string& secret, int interval)
{
    ...
    int key_size =
        base32_decode((const uint8_t*)secret.data(), decoded_key.data(), decoded_key.size());
    ...
    std::uint32_t pin =
        (hmac_result[ offset ] & 0x7f) << 24 | (hmac_result[ offset + 1 ] & 0xff) << 16 |
...
{
    auto result = LoginDatabase.PQuery("SELECT gmlevel, RealmID FROM account_access WHERE id = %u",
                                       accountId);
    if (!result)
        return;
