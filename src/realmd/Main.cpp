...
extern int main(int argc, char** argv)
{
    ...
    while ((option = cmd_opts()) != EOF)
    {
        switch (option)
        {
            case 'c': cfg_file = cmd_opts.opt_arg(); break;
            case 'v': printf("Core revion: %s\n", _FULLVERSION); return 0;
            ...
            case 's':
            {
                ...
                {
                    sLog.outError(
                        "Runtime-Error: -%c unsupported argument %s", cmd_opts.opt_opt(), mode);
                    usage(argv[ 0 ]);
    ...
    if (confVersion < _REALMDCONFVERSION)
    {
        sLog.outError(
            "*****************************************************************************");
        sLog.outError(
            " WARNING: Your realmd.conf version indicates your conf file is out of date!");
        sLog.outError(
    ...
    ACE_Reactor::instance(new ACE_Reactor(new ACE_Dev_Poll_Reactor(ACE::max_handles(), 1), 1),
                          true);
#else
    ...
    LoginDatabase.Execute("UPDATE account_banned SET active = 0 WHERE unbandate<=UNIX_TIMESTAMP() "
            ...
            if (GetProcessAffinityMask(hProcess, &appAff, &sysAff))
            {
                ...
                if (!curAff)
                {
                    sLog.outError("Processors marked in UseProcessors bitmask (hex) %x not "
                                  "accessible for realmd. Accessible processors bitmask (hex): %x",
                                  Aff,
                                  appAff);
                }
                else
...
void OnSignal(int s)
{
    ...
    {
        case SIGINT:
        case SIGTERM: stopEvent = true; break;
#ifdef _WIN32
        case SIGBREAK: stopEvent = true; break;
#endif
    }
