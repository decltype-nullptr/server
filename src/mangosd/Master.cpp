...
public:
    ...
    void run(void)
    {
        if (!_delaytime)
            return;
        sLog.outString("Starting up anti-freeze thread (%u seconds max stuck time)...",
                       _delaytime / 1000);
        m_loops      = 0;
        w_loops      = 0;
        m_lastchange = 0;
        ...
        while (!World::IsStopped())
        {
            ...
            // DEBUG_LOG("anti-freeze: time=%u, counters=[%u;
            // %u]",curtime,Master::m_masterLoopCounter,World::m_worldLoopCounter);
...
int Master::Run()
{
    ...
    // set realmbuilds depend on mangosd expected builds, and set server online
    {
        ...
        LoginDatabase.PExecute("UPDATE realmlist SET realmflags = realmflags & ~(%u), population = "
...
///- Handle affinity for multiple processors and process priority on Windows
#ifdef WIN32
    {
            ...
            if (GetProcessAffinityMask(hProcess, &appAff, &sysAff))
            {
                ...
                if (!curAff)
                {
                    sLog.outError("Processors marked in UseProcessors bitmask (hex) %x not "
                                  "accessible for mangosd. Accessible processors bitmask (hex): %x",
                                  Aff,
                                  appAff);
                }
                else
        ...
        //        if(Prio && (m_ServiceStatus == -1)/* need set to default process priority class in
        //        service mode*/)
        if (Prio)
        {
    ...
    if (sConfig.GetBoolDefault("SOAP.Enabled", false))
    {
        ...
        runnable->setListenArguments(sConfig.GetStringDefault("SOAP.IP", "127.0.0.1"),
    ...
    // LoginDatabase.DirectPExecute("UPDATE realmlist SET realmflags = realmflags | %u WHERE id =
    // '%u'", REALM_FLAG_OFFLINE, realmID);
    ...
    if (cliThread)
    {
        ...
        // this only way to terminate CLI thread exist at Win32 (alt. way exist only in Windows
        // Vista API)
        //_exit(1);
...
bool StartDB(std::string name, DatabaseType& database, const char** migrations)
{
    ...
    sLog.outString("%s Database: %s, sync threads: %i, workers: %i",
                   name.c_str(),
                   dbstring.c_str(),
                   nConnections,
                   nAsyncConnections);
...
void Master::clearOnlineAccounts()
{
    ...
    /// \todo Only accounts with characters logged on *this* realm should have online status reset.
    /// Move the online column from 'account' to 'realmcharacters'?
    LoginDatabase.PExecute("UPDATE account SET current_realm = 0 WHERE current_realm = '%u'",
                           realmID);
...
void Master::_OnSignal(int s)
{
    switch (s)
    {
        case SIGINT: World::StopNow(RESTART_EXIT_CODE); break;
        case SIGTERM:
#ifdef _WIN32
        case SIGBREAK:
#endif
            ...
            if (anticrashOptions & ANTICRASH_OPTION_ANNOUNCE_PLAYERS)
            {
                if (anticrashOptions & ANTICRASH_OPTION_SAVEALL)
                    sWorld.SendWorldText(LANG_SYSTEMMESSAGE,
                                         "Server has crashed. Now saving online players ...");
                else
