...
/// \todo This function has to be enhanced to respect the login/realm split (delete char, delete
/// account chars in realm, delete account chars in realm then delete account
bool ChatHandler::HandleAccountDeleteCommand(char* args)
{
    ...
    switch (result)
    {
        case AOR_OK: PSendSysMessage(LANG_ACCOUNT_DELETED, account_name.c_str()); break;
 ...
 * @param searchString the search string which either contains a player GUID (low part) or a part of
 * the character-name
 * @return             returns false if there was a problem while selecting the characters (e.g.
 * player name not normalizeable)
 */
bool ChatHandler::GetDeletedCharacterInfoList(DeletedInfoList& foundList, std::string searchString)
{
    std::shared_ptr<QueryResult> resultChar;
    if (!searchString.empty())
    {
        // search by GUID
        if (isNumeric(searchString))
            resultChar = CharacterDatabase.PQuery(
                "SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate FROM characters "
                "WHERE deleteDate IS NOT NULL AND guid = %u LIMIT 0,50",
                uint32(atoi(searchString.c_str())));
        // search by name
        else
        {
            ...
            resultChar = CharacterDatabase.PQuery(
                "SELECT guid, deleteInfos_Name, deleteInfos_Account, deleteDate FROM characters "
                "WHERE deleteDate IS NOT NULL AND deleteInfos_Name " _LIKE_
                " " _CONCAT3_("'%%'", "'%s'", "'%%'") " LIMIT 0,50",
                searchString.c_str());
        }
    }
    else
        resultChar = CharacterDatabase.Query("SELECT guid, deleteInfos_Name, deleteInfos_Account, "
                                             "deleteDate FROM characters WHERE deleteDate IS NOT "
                                             "NULL LIMIT 0,50");
...
/**
 * Generate WHERE guids list by deleted info in way preventing return too long where list for
 * existed query string length limit.
 *
 * @param itr          a reference to an deleted info list iterator, it updated in function for
 * possible next function call if list to long
 ...
 */
std::string
ChatHandler::GenerateDeletedCharacterGUIDsWhereStr(DeletedInfoList::const_iterator& itr,
...
/**
 * Handles the '.character deleted list' command, which shows all deleted characters which matches
...
void ChatHandler::HandleCharacterDeletedRestoreHelper(DeletedInfo const& delInfo)
{
    ...
    {
        PSendSysMessage(LANG_CHARACTER_DELETED_SKIP_ACCOUNT,
                        delInfo.name.c_str(),
                        delInfo.lowguid,
                        delInfo.accountId);
        return;
    ...
    if (charcount >= 10)
    {
        PSendSysMessage(LANG_CHARACTER_DELETED_SKIP_FULL,
                        delInfo.name.c_str(),
                        delInfo.lowguid,
                        delInfo.accountId);
        return;
    ...
    {
        PSendSysMessage(LANG_CHARACTER_DELETED_SKIP_NAME,
                        delInfo.name.c_str(),
                        delInfo.lowguid,
                        delInfo.accountId);
        return;
    ...
    CharacterDatabase.PExecute("UPDATE characters SET name='%s', account='%u', deleteDate=NULL, "
                               "deleteInfos_Name=NULL, deleteInfos_Account=NULL WHERE deleteDate "
                               "IS NOT NULL AND guid = %u",
                               delInfo.name.c_str(),
                               delInfo.accountId,
                               delInfo.lowguid);
}
...
/**
 * Handles the '.character deleted restore' command, which restores all deleted characters which
 * matches the given search string
 *
 * The command automatically calls '.character deleted list' command with the search string to show
 * all restored characters.
 *
...
/**
 * Handles the '.character deleted delete' command, which completely deletes all deleted characters
 * which matches the given search string
 *
...
/**
 * Handles the '.character deleted old' command, which completely deletes all deleted characters
 * deleted with some days ago
 *
...
bool ChatHandler::HandleCharacterEraseCommand(char* args)
{
    ...
    PSendSysMessage(LANG_CHARACTER_DELETED,
                    target_name.c_str(),
...
bool ChatHandler::HandleAccountOnlineListCommand(char* args)
{
    ...
    auto result = LoginDatabase.PQuery(
        "SELECT id, username, last_ip, gmlevel, expansion FROM account WHERE current_realm = %u",
        realmID);
...
bool ChatHandler::HandleAccountCreateCommand(char* args)
{
    ...
    switch (result)
    {
        case AOR_OK: PSendSysMessage(LANG_ACCOUNT_CREATED, account_name.c_str()); break;
    ...
    if (!*args)
    {
        ...
        for (int i = 0; i < LOG_FILTER_COUNT; ++i)
            if (*logFilterData[ i ].name)
                PSendSysMessage("  %-20s = %s",
                                logFilterData[ i ].name,
                                GetOnOffStr(sLog.HasLogFilter(1 << i)));
        return true;
    }
    ...
    while (!World::IsStopped())
    {
        ...
        if (command_str != NULL)
        {
            ...
            sWorld.QueueCliCommand(new CliCommandHolder(
                0, SEC_CONSOLE, NULL, command.c_str(), &utf8print, &commandFinished));
        }
        else if (feof(stdin))
        {
            World::StopNow(SHUTDOWN_EXIT_CODE);
