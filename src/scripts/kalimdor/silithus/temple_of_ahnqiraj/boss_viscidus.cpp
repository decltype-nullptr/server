...
enum
{
    ...
    SPELL_VISCIDUS_SUICIDE =
        26003, // cast when boss explodes and is below 5% Hp - should trigger 26002
    ...
    SPELL_SUMMON_GLOBS =
        25885, // summons npc 15667 using spells from 25865 to 25884; All spells have target coords
...
static const uint32 auiGlobSummonSpells[ MAX_VISCIDUS_GLOBS ] = {
...
{
    boss_viscidusAI(Creature* pCreature)
    ...
    void MoveInLineOfSight(Unit* who) override
    {
        if (who->GetTypeId() == TYPEID_PLAYER && !m_creature->getVictim() &&
            m_creature->IsWithinDistInMap(who, 95.0f, true))
        {
            AttackStart(who);
        }
    ...
    void SummonedCreatureJustDied(Creature* pSummoned)
    {
        if (pSummoned->GetEntry() == NPC_GLOB_OF_VISCIDUS)
        {
            ...
            {
                auto health = m_creature->GetHealth() -
                              static_cast<uint32>((m_creature->GetMaxHealth() * 0.05f));
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiToxinTimer < uiDiff)
        {
            ...
            {
                m_creature->SummonCreature(NPC_VISCIDUS_TRIGGER,
...
bool EffectAuraDummy_spell_aura_dummy_viscidus_freeze(const Aura* pAura, bool bApply)
{
    ...
    if (pAura->GetId() == SPELL_VISCIDUS_FREEZE && pAura->GetEffIndex() == EFFECT_INDEX_1 &&
        !bApply)
    {
...
struct ViscidusNullAI : public ScriptedAI
{
    ...
    ViscidusNullAI(Creature* pCreature)
        : ScriptedAI(pCreature)
    {
    }
