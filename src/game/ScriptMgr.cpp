...
void ScriptMgr::LoadScripts(ScriptMapMap& scripts, const char* tablename)
{
    ...
    auto result = WorldDatabase.PQuery("SELECT id, delay, command, datalong, datalong2, "
                                       "datalong3, datalong4, data_flags, dataint, "
                                       "dataint2, dataint3, dataint4, x, y, z, o FROM %s",
                                       tablename);
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            case SCRIPT_COMMAND_TALK:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has invalid CHAT_TYPE_ (datalong = %u) in "
                ...
                if (tmp.talk.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.talk.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_TALK for "
                                    "script id %u, but this creature_template does not exist.",
                                    tablename,
                                    tmp.talk.creatureEntry,
                                    tmp.id);
                    continue;
                }
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_TALK for "
                                    "script id %u, but search radius is too small (datalong3 = "
                                    "%u).",
                                    tablename,
                                    tmp.talk.creatureEntry,
                                    tmp.id,
                                    tmp.talk.searchRadius);
                    continue;
                }
                if (!GetLanguageDescByID(tmp.talk.language))
                {
                    sLog.outErrorDb("Table `%s` has datalong4 = %u in SCRIPT_COMMAND_TALK for "
                ...
                {
                    sLog.outErrorDb("Table `%s` has invalid talk text id (dataint = %i) in "
                                    "SCRIPT_COMMAND_TALK for script id %u",
                                    tablename,
                                    tmp.talk.textId[ 0 ],
                                    tmp.id);
                ...
                {
                    if (tmp.talk.textId[ i ] && (tmp.talk.textId[ i ] < MIN_DB_SCRIPT_STRING_ID ||
                                                 tmp.talk.textId[ i ] >= MAX_DB_SCRIPT_STRING_ID))
                    {
                        sLog.outErrorDb("Table `%s` has out of range text id (dataint = %i "
                                        "expected %u-%u) in SCRIPT_COMMAND_TALK for script id %u",
                                        tablename,
                                        tmp.talk.textId[ i ],
                ...
                // if (!GetMangosStringLocale(tmp.dataint)) will be checked after db_script_string
                // loading
                break;
            }
            case SCRIPT_COMMAND_EMOTE:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has invalid emote id (datalong = %u) in "
                ...
                if (tmp.emote.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.emote.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_EMOTE for "
                                    "script id %u, but this creature_template does not exist.",
                                    tablename,
                                    tmp.emote.creatureEntry,
                                    tmp.id);
                    continue;
                }
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has invalid map (Id: %u) in "
                ...
                {
                    sLog.outErrorDb("Table `%s` has invalid coordinates (X: %f Y: %f) in "
                                    "SCRIPT_COMMAND_TELEPORT_TO for script id %u",
                                    tablename,
                                    tmp.x,
                                    tmp.y,
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                ...
                if (!quest)
                {
                    sLog.outErrorDb("Table `%s` has invalid quest (ID: %u) in "
                                    "SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u",
                                    tablename,
                ...
                {
                    sLog.outErrorDb(
                        "Table `%s` has quest (ID: %u) in SCRIPT_COMMAND_QUEST_EXPLORED in "
                        "`datalong` for script id %u, but quest not have flag "
                        "QUEST_SPECIAL_FLAG_EXPLORATION_OR_EVENT in quest flags. Script command or "
                        "quest flags wrong. Quest modified to require objective.",
                        tablename,
                        tmp.questExplored.questId,
                        tmp.id);
                ...
                {
                    sLog.outErrorDb("Table `%s` has too large distance (%u) for exploring "
                    ...
                    float(tmp.questExplored.distance) > DEFAULT_VISIBILITY_DISTANCE)
                {
                    sLog.outErrorDb("Table `%s` has too large distance (%u) for exploring "
                                    "objective complete in `datalong2` in "
                                    "SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, "
                                    "max distance is %f or 0 for disable distance check",
                                    tablename,
                                    tmp.questExplored.distance,
                                    tmp.id,
                ...
                {
                    sLog.outErrorDb("Table `%s` has too small distance (%u) for exploring "
                                    "objective complete in `datalong2` in "
                                    "SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, "
                                    "min distance is %f or 0 for disable distance check",
                                    tablename,
                                    tmp.questExplored.distance,
                                    tmp.id,
            ...
            case SCRIPT_COMMAND_KILL_CREDIT:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has invalid creature (Entry: %u) in "
            ...
            case SCRIPT_COMMAND_RESPAWN_GAMEOBJECT:
            {
                ...
                if (!data)
                {
                    sLog.outErrorDb("Table `%s` has invalid gameobject (GUID: %u) in "
                ...
                if (!info)
                {
                    sLog.outErrorDb("Table `%s` has gameobject with invalid entry (GUID: %u Entry: "
                                    "%u) in SCRIPT_COMMAND_RESPAWN_GAMEOBJECT for script id %u",
                                    tablename,
                                    tmp.GetGOGuid(),
                                    data->id,
                                    tmp.id);
                ...
                {
                    sLog.outErrorDb("Table `%s` have gameobject type (%u) unsupported by command "
                                    "SCRIPT_COMMAND_RESPAWN_GAMEOBJECT for script id %u",
                                    tablename,
                                    info->id,
                                    tmp.id);
            ...
            case SCRIPT_COMMAND_CLOSE_DOOR:
            {
                ...
                if (!data)
                {
                    sLog.outErrorDb(
                        ...
                        tablename,
                        tmp.GetGOGuid(),
                        (tmp.command == SCRIPT_COMMAND_OPEN_DOOR ? "SCRIPT_COMMAND_OPEN_DOOR" :
                ...
                {
                    sLog.outErrorDb("Table `%s` has gameobject type (%u) non supported by command "
                                    "%s for script id %u",
                                    tablename,
                                    info->id,
                                    (tmp.command == SCRIPT_COMMAND_OPEN_DOOR ?
            ...
            case SCRIPT_COMMAND_REMOVE_AURA:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` using nonexistent spell (id: %u) in "
                                    "SCRIPT_COMMAND_REMOVE_AURA or SCRIPT_COMMAND_CAST_SPELL for "
                                    "script id %u",
                                    tablename,
                ...
                {
                    sLog.outErrorDb("Table `%s` using unknown flags in datalong2 (%u)i n "
                                    "SCRIPT_COMMAND_CAST_SPELL for script id %u",
                                    tablename,
                                    tmp.removeAura.isSourceTarget,
                                    tmp.id);
                    continue;
                }
                break;
            ...
            case SCRIPT_COMMAND_CREATE_ITEM:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has nonexistent item (entry: %u) in "
            ...
            {
                if (tmp.despawn.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.despawn.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                    "SCRIPT_COMMAND_DESPAWN_CREATURE for script id %u, but this "
                                    "creature_template does not exist.",
                                    tablename,
                                    tmp.despawn.creatureEntry,
                                    tmp.id);
                    continue;
                }
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                    "SCRIPT_COMMAND_DESPAWN_CREATURE for script id %u, but search "
                                    "radius is too small (datalong3 = %u).",
                                    tablename,
                                    tmp.despawn.creatureEntry,
                                    tmp.id,
                                    tmp.despawn.searchRadius);
                    continue;
                }
                break;
            }
            ...
            {
                sLog.outErrorDb(
                    "Table `%s` use unsupported SCRIPT_COMMAND_PLAY_MOVIE for script id %u",
                    tablename,
                    tmp.id);
                continue;
            }
            case SCRIPT_COMMAND_MOVEMENT:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` SCRIPT_COMMAND_MOVEMENT has invalid MovementType "
                                    "%u for script id %u",
                                    tablename,
                                    tmp.movement.movementType,
                                    tmp.id);
                    continue;
                }
                if (tmp.movement.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.movement.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_MOVEMENT for "
                                    "script id %u, but this creature_template does not exist.",
                                    tablename,
                                    tmp.movement.creatureEntry,
                                    tmp.id);
                    continue;
                }
            ...
            {
                if (tmp.activeObject.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.activeObject.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                    "SCRIPT_COMMAND_SET_ACTIVEOBJECT for script id %u, but this "
                                    "creature_template does not exist.",
                                    tablename,
                                    tmp.activeObject.creatureEntry,
                                    tmp.id);
                    continue;
                }
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                    "SCRIPT_COMMAND_SET_ACTIVEOBJECT for script id %u, but search "
                                    "radius is too small (datalong3 = %u).",
                                    tablename,
                                    tmp.activeObject.creatureEntry,
                                    tmp.id,
                                    tmp.activeObject.searchRadius);
                    continue;
                }
            ...
            case SCRIPT_COMMAND_SET_FACTION:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_SET_FACTION "
                                    "for script id %u, but this faction does not exist.",
                                    tablename,
                                    tmp.faction.factionId,
                                    tmp.id);
                    continue;
                }

                if (tmp.faction.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.faction.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_SET_FACTION "
                                    "for script id %u, but this creature_template does not exist.",
                                    tablename,
                                    tmp.faction.creatureEntry,
                                    tmp.id);
                    continue;
                }
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_SET_FACTION "
                                    "for script id %u, but search radius is too small (datalong3 = "
                                    "%u).",
                                    tablename,
                                    tmp.faction.creatureEntry,
                                    tmp.id,
                                    tmp.faction.searchRadius);
                    continue;
                }
            ...
            case SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL:
            {
                if (tmp.morph.flags & 0x01)
                {
                    if (tmp.morph.creatureOrModelEntry &&
                        !sCreatureDisplayInfoStore.LookupEntry(tmp.morph.creatureOrModelEntry))
                    {
                        sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                        "SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL for script id %u, "
                ...
                {
                    if (tmp.morph.creatureOrModelEntry &&
                        !ObjectMgr::GetCreatureTemplate(tmp.morph.creatureOrModelEntry))
                    {
                        sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                        "SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL for script id %u, "
                                        "but this creature_template does not exist.",
                                        tablename,
                                        tmp.morph.creatureOrModelEntry,
                                        tmp.id);
                        continue;
                ...
                if (tmp.morph.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.morph.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                    "SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL for script id %u, but "
                                    "this creature_template does not exist.",
                                    tablename,
                                    tmp.morph.creatureEntry,
                                    tmp.id);
                    continue;
                }
                if (tmp.morph.creatureEntry && !tmp.morph.searchRadius)
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                    "SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL for script id %u, but "
                                    "search radius is too small (datalong3 = %u).",
                                    tablename,
                                    tmp.morph.creatureEntry,
                                    tmp.id,
                                    tmp.morph.searchRadius);
                    continue;
                }
            ...
            case SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL:
            {
                if (tmp.mount.flags & 0x01)
                {
                    if (tmp.mount.creatureOrModelEntry &&
                        !sCreatureDisplayInfoStore.LookupEntry(tmp.mount.creatureOrModelEntry))
                    {
                        sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                        "SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL for script id %u, "
                                        "but this model does not exist.",
                                        tablename,
                ...
                {
                    if (tmp.mount.creatureOrModelEntry &&
                        !ObjectMgr::GetCreatureTemplate(tmp.mount.creatureOrModelEntry))
                    {
                        sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                        "SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL for script id %u, "
                                        "but this creature_template does not exist.",
                ...
                if (tmp.mount.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.mount.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                    "SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL for script id %u, but "
                                    "this creature_template does not exist.",
                                    tablename,
                                    tmp.mount.creatureEntry,
                                    tmp.id);
                    continue;
                }
            ...
            case SCRIPT_COMMAND_SET_RUN:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_SET_RUN for "
                                    "script id %u, but this creature_template does not exist.",
                                    tablename,
                                    tmp.run.creatureEntry,
                                    tmp.id);
                    continue;
                }
            ...
            {
                if (tmp.attack.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.attack.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_ATTACK_START "
                                    "for script id %u, but this creature_template does not exist.",
                                    tablename,
                                    tmp.attack.creatureEntry,
                                    tmp.id);
                    continue;
                }
            ...
            case SCRIPT_COMMAND_GO_LOCK_STATE:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_GO_LOCK_STATE "
                                    "for script id %u, but this gameobject_template does not "
                                    "exist.",
                                    tablename,
                                    tmp.goLockState.goEntry,
                                    tmp.id);
                    continue;
                }
                if (!tmp.goLockState.searchRadius)
                {
                    sLog.outErrorDb("Table `%s` has invalid search radius (datalong3 = %u) in "
                                    "SCRIPT_COMMAND_GO_LOCK_STATE for script id %u.",
                                    tablename,
                                    tmp.goLockState.searchRadius,
                                    tmp.id);
                    continue;
                }
                ...
                {
                    sLog.outErrorDb("Table `%s` has invalid lock state (datalong = %u) in "
                                    "SCRIPT_COMMAND_GO_LOCK_STATE for script id %u.",
                                    tablename,
                                    tmp.goLockState.lockState,
                                    tmp.id);
            ...
            case SCRIPT_COMMAND_STAND_STATE:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has invalid stand state (datalong = %u) in "
                                    "SCRIPT_COMMAND_STAND_STATE for script id %u",
                                    tablename,
                                    tmp.standState.stand_state,
                                    tmp.id);
                    continue;
                }
                if (tmp.standState.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.standState.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in SCRIPT_COMMAND_STAND_STATE "
                                    "for script id %u, but this creature_template does not exist.",
                                    tablename,
                                    tmp.standState.creatureEntry,
                                    tmp.id);
                    continue;
                }
            ...
            {
                if (tmp.npcFlag.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.npcFlag.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong3 = %u in "
                                    "SCRIPT_COMMAND_MODIFY_NPC_FLAGS for script id %u, but this "
                                    "creature_template does not exist.",
                                    tablename,
                                    tmp.npcFlag.creatureEntry,
                                    tmp.id);
                    continue;
                }
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong3 = %u in "
                                    "SCRIPT_COMMAND_MODIFY_NPC_FLAGS for script id %u, but search "
                                    "radius is too small (datalong4 = %u).",
                                    tablename,
                                    tmp.npcFlag.creatureEntry,
                                    tmp.id,
                                    tmp.npcFlag.searchRadius);
                    continue;
                }
                break;
            }
            case SCRIPT_COMMAND_SEND_TAXI_PATH:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong = %u in SCRIPT_COMMAND_SEND_TAXI_PATH "
                                    "for script id %u, but this taxi path does not exist.",
                                    tablename,
                                    tmp.sendTaxiPath.taxiPathId,
                                    tmp.id);
                    continue;
                }
                break;
            ...
            {
                if (tmp.terminateScript.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.terminateScript.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong = %u in "
                                    "SCRIPT_COMMAND_TERMINATE_SCRIPT for script id %u, but this "
                                    "npc entry does not exist.",
                                    tablename,
                                    tmp.terminateScript.creatureEntry,
                                    tmp.id);
                    continue;
                }
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong = %u in  "
                                    "SCRIPT_COMMAND_TERMINATE_SCRIPT for script id %u, but search "
                                    "radius is too small (datalong2 = %u).",
                                    tablename,
                                    tmp.terminateScript.creatureEntry,
                                    tmp.id,
                                    tmp.terminateScript.searchRadius);
                    continue;
                }
                break;
            }
            ...
            {
                if (tmp.enterEvadeMode.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.enterEvadeMode.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong = %u in "
                                    "SCRIPT_COMMAND_ENTER_EVADE_MODE for script id %u, but this "
                                    "creature_template does not exist.",
                                    tablename,
                                    tmp.enterEvadeMode.creatureEntry,
                                    tmp.id);
                    continue;
                }
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong = %u in "
                                    "SCRIPT_COMMAND_ENTER_EVADE_MODE for script id %u, but search "
                                    "radius is too small (datalong2 = %u).",
                                    tablename,
                                    tmp.enterEvadeMode.creatureEntry,
                                    tmp.id,
                                    tmp.enterEvadeMode.searchRadius);
                    continue;
                }
                break;
            }
            case SCRIPT_COMMAND_TERMINATE_COND:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong = %u in SCRIPT_COMMAND_TERMINATE_COND "
                                    "for script id %u, but this condition_id does not exist.",
                                    tablename,
                                    tmp.terminateCond.conditionId,
                                    tmp.id);
                    continue;
                }
                if (tmp.terminateCond.failQuest &&
                    !sObjectMgr.GetQuestTemplate(tmp.terminateCond.failQuest))
                {
                    sLog.outErrorDb("Table `%s` has datalong2 = %u in "
                                    "SCRIPT_COMMAND_TERMINATE_COND for script id %u, but this "
                                    "questId does not exist.",
                                    tablename,
                                    tmp.terminateCond.failQuest,
                                    tmp.id);
                    continue;
                }
                break;
            ...
            case SCRIPT_COMMAND_TURN_TO:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` using unknown option in datalong (%u) in "
                                    "SCRIPT_COMMAND_TURN_TO for script id %u",
                                    tablename,
                                    tmp.turnTo.facingLogic,
                                    tmp.id);
                    continue;
                }
                if (tmp.turnTo.creatureEntry &&
                    !ObjectMgr::GetCreatureTemplate(tmp.turnTo.creatureEntry))
                {
                    sLog.outErrorDb("Table `%s` has datalong3 = %u in SCRIPT_COMMAND_TURN_TO for "
                                    "script id %u, but this npc entry does not exist.",
                                    tablename,
                                    tmp.turnTo.creatureEntry,
                                    tmp.id);
                    continue;
                }
                ...
                {
                    sLog.outErrorDb("Table `%s` has datalong3 = %u in  SCRIPT_COMMAND_TURN_TO for "
                                    "script id %u, but search radius is too small (datalong4 = "
                                    "%u).",
                                    tablename,
                                    tmp.turnTo.creatureEntry,
                                    tmp.id,
                                    tmp.turnTo.searchRadius);
                    continue;
                }
                break;
            }
...
void ScriptMgr::LoadQuestEndScripts()
{
    ...
    // check ids
    for (ScriptMapMap::const_iterator itr = sQuestEndScripts.begin(); itr != sQuestEndScripts.end();
         ++itr)
    {
...
void ScriptMgr::LoadSpellScripts()
{
    ...
    // check ids
    for (ScriptMapMap::const_iterator itr = sSpellScripts.begin(); itr != sSpellScripts.end();
         ++itr)
    {
        ...
        if (!spellInfo)
        {
            sLog.outErrorDb("Table `spell_scripts` has not existing spell (Id: %u) as script id",
                            itr->first);
            continue;
        }
        ...
        if (!found)
            sLog.outErrorDb("Table `spell_scripts` has unsupported spell (Id: %u) without "
...
void ScriptMgr::LoadEventScripts()
{
    ...
    for (ScriptMapMap::const_iterator itr = sEventScripts.begin(); itr != sEventScripts.end();
         ++itr)
    {
        std::set<uint32>::const_iterator itr2 = eventIds.find(itr->first);
        if (itr2 == eventIds.end())
            sLog.outErrorDb("Table `event_scripts` has script (Id: %u) not referring to any "
                            "gameobject_template type 10 data2 field, type 3 data6 field, type 13 "
                            "data 2 field, type 29 or any spell effect %u",
                            itr->first,
...
void ScriptMgr::LoadDbScriptStrings()
{
    sObjectMgr.LoadMangosStrings(
        WorldDatabase, "db_script_string", MIN_DB_SCRIPT_STRING_ID, MAX_DB_SCRIPT_STRING_ID, false);
...
void ScriptMgr::CheckScriptTexts(ScriptMapMap const& scripts, std::set<int32>& ids)
{
    ...
    {
        for (ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end();
             ++itrM)
        {
            if (itrM->second.command == SCRIPT_COMMAND_TALK)
            {
                for (int i = 0; i < MAX_TEXT_ID; ++i)
                {
                    if (itrM->second.talk.textId[ i ] &&
                        !sObjectMgr.GetMangosStringLocale(itrM->second.talk.textId[ i ]))
                        sLog.outErrorDb("Table `db_script_string` is missing string id %u, used in "
                                        "database script id %u.",
                                        itrM->second.talk.textId[ i ],
                                        itrMM->first);
...
void ScriptMgr::LoadAreaTriggerScripts()
{
    ...
    auto result = WorldDatabase.Query("SELECT entry, ScriptName FROM scripted_areatrigger");
    ...
    do
    {
        ...
        {
            sLog.outErrorDb("Table `scripted_areatrigger` has area trigger (ID: %u) not listed in "
                            "`AreaTrigger.dbc`.",
                            triggerId);
            continue;
        }
...
void ScriptMgr::LoadEventIdScripts()
{
    ...
    auto result = WorldDatabase.Query("SELECT id, ScriptName FROM scripted_event_id");
    ...
    do
    {
        ...
        if (itr == eventIds.end())
            sLog.outErrorDb("Table `scripted_event_id` has id %u not referring to any "
                            "gameobject_template type 10 data2 field, type 3 data6 field, type 13 "
                            "data 2 field, type 29 or any spell effect %u or path taxi node data",
                            eventId,
                            SPELL_EFFECT_SEND_EVENT);
...
bool ScriptMgr::OnGossipSelect(Player* pPlayer,
                               Creature* pCreature,
                               uint32 sender,
                               uint32 action,
                               const char* code)
{
    sLog.outDebug(
        "Gossip selection%s, sender: %d, action: %d", code ? " with code" : "", sender, action);
...
bool ScriptMgr::OnEffectDummy(Unit* pCaster,
                              uint32 spellId,
...
void ScriptMgr::LoadScriptTexts()
{
    ...
    LoadMangosStrings(
        WorldDatabase, "script_texts", TEXT_SOURCE_TEXT_START, TEXT_SOURCE_TEXT_END, true);

    auto result = WorldDatabase.PQuery("SELECT entry, sound, type, language, emote FROM "
                                       "script_texts WHERE entry BETWEEN %i AND %i",
                                       TEXT_SOURCE_GOSSIP_END,
                                       TEXT_SOURCE_TEXT_START);
        ...
        do
        {
            ...
            if (pTemp.SoundId)
            {
                if (!GetSoundEntriesStore()->LookupEntry(pTemp.SoundId))
                    sLog.outErrorDb(
                        "Entry %i in table `script_texts` has soundId %u but sound does not exist.",
                        iId,
                        pTemp.SoundId);
            }

            if (!GetLanguageDescByID(pTemp.Language))
                sLog.outErrorDb("Entry %i in table `script_texts` using Language %u but Language "
                                "does not exist.",
                                iId,
                                pTemp.Language);

            if (pTemp.Type > CHAT_TYPE_ZONE_YELL)
                sLog.outErrorDb("Entry %i in table `script_texts` has Type %u but this Chat Type "
                                "does not exist.",
                                iId,
                                pTemp.Type);
...
void ScriptMgr::LoadScriptTextsCustom()
{
    ...
    LoadMangosStrings(
        WorldDatabase, "custom_texts", TEXT_SOURCE_CUSTOM_START, TEXT_SOURCE_CUSTOM_END, true);

    auto result = WorldDatabase.PQuery("SELECT entry, sound, type, language, emote FROM "
                                       "custom_texts WHERE entry BETWEEN %i AND %i",
                                       TEXT_SOURCE_CUSTOM_END,
                                       TEXT_SOURCE_CUSTOM_START);
    ...
    if (result)
    {
        ...
        do
        {
            ...
            if (pTemp.SoundId)
            {
                if (!GetSoundEntriesStore()->LookupEntry(pTemp.SoundId))
                    sLog.outErrorDb(
                        "Entry %i in table `custom_texts` has soundId %u but sound does not exist.",
                        iId,
                        pTemp.SoundId);
            }

            if (!GetLanguageDescByID(pTemp.Language))
                sLog.outErrorDb("Entry %i in table `custom_texts` using Language %u but Language "
                                "does not exist.",
                                iId,
                                pTemp.Language);

            if (pTemp.Type > CHAT_TYPE_ZONE_YELL)
                sLog.outErrorDb("Entry %i in table `custom_texts` has Type %u but this Chat Type "
                                "does not exist.",
                                iId,
                                pTemp.Type);
...
void ScriptMgr::LoadScriptGossipTexts()
{
    ...
    LoadMangosStrings(
        WorldDatabase, "gossip_texts", TEXT_SOURCE_GOSSIP_START, TEXT_SOURCE_GOSSIP_END);
}
...
void ScriptMgr::LoadScriptWaypoints()
{
    ...
    // Load Waypoints
    auto result = WorldDatabase.PQuery("SELECT COUNT(entry) FROM script_waypoint GROUP BY entry");
    if (result)
    {
    ...
    result = WorldDatabase.PQuery("SELECT entry, pointid, location_x, location_y, location_z, "
                                  "waittime FROM script_waypoint ORDER BY pointid");
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                sLog.outErrorDb(
                    "DB table script_waypoint has waypoint for nonexistant creature entry %u",
                    pTemp.uiCreatureEntry);
                continue;
            }

            if (!pCInfo->ScriptID)
                sLog.outErrorDb("DB table script_waypoint has waypoint for creature entry %u, but "
                                "creature does not have ScriptName defined and then useless.",
                                pTemp.uiCreatureEntry);
...
void ScriptMgr::LoadEscortData()
{
    ...
    auto pResult =
        WorldDatabase.PQuery("SELECT creature_id, quest, escort_faction FROM script_escort_data");
    ...
    if (pResult)
    {
        ...
        do
        {
            ...
            {
                sLog.outErrorDb(
                    "DB table script_escort_data has data for non-existant creature entry %u",
                    pTemp.uiCreatureEntry);
                continue;
            }

            if (!pCInfo->ScriptID)
                sLog.outErrorDb("DB table script_escort_data has data for creature entry %u, but "
                                "creature does not have ScriptName defined and then useless.",
                                pTemp.uiCreatureEntry);
            ...
            if (points.empty())
            {
                sLog.outErrorDb("Le PNJ %u de script_escort_data n'a pas de donnees de Waypoints !",
                                pTemp.uiCreatureEntry);
                continue;
            }
...
void ScriptMgr::CollectPossibleEventIds(std::set<uint32>& eventIds)
{
         ...
         ++itr)
    {
        switch (itr->type)
        {
            case GAMEOBJECT_TYPE_GOOBER: eventIds.insert(itr->goober.eventId); break;
            case GAMEOBJECT_TYPE_CHEST: eventIds.insert(itr->chest.eventId); break;
            case GAMEOBJECT_TYPE_CAMERA: eventIds.insert(itr->camera.eventID); break;
...
void DoScriptText(int32 iTextEntry, WorldObject* pSource, Unit* pTarget)
{
    ...
    if (iTextEntry >= 0)
    {
        sLog.outError("DoScriptText with source entry %u (TypeId=%u, guid=%u) attempts to process "
                      "text entry %i, but text entry must be negative.",
                      pSource->GetEntry(),
                      pSource->GetTypeId(),
                      pSource->GetGUIDLow(),
                      iTextEntry);
    ...
    {
        sLog.outError(
            "DoScriptText with source entry %u (TypeId=%u, guid=%u) could not find text entry %i.",
            pSource->GetEntry(),
            pSource->GetTypeId(),
            pSource->GetGUIDLow(),
            iTextEntry);
    ...
    if (pData->SoundId)
    {
        if (GetSoundEntriesStore()->LookupEntry(pData->SoundId))
        {
            ...
            sLog.outError("DoScriptText entry %i tried to process invalid sound id %u.",
                          iTextEntry,
                          pData->SoundId);
    }
    ...
    if (pData->Emote)
    {
        if (pSource->GetTypeId() == TYPEID_UNIT || pSource->GetTypeId() == TYPEID_PLAYER)
            ...
            sLog.outError("DoScriptText entry %i tried to process emote for invalid TypeId (%u).",
                          iTextEntry,
                          pSource->GetTypeId());
    }
    ...
    switch (pData->Type)
    {
        case CHAT_TYPE_SAY: pSource->MonsterSay(iTextEntry, pData->Language, pTarget); break;
        case CHAT_TYPE_YELL: pSource->MonsterYell(iTextEntry, pData->Language, pTarget); break;
        case CHAT_TYPE_TEXT_EMOTE: pSource->MonsterTextEmote(iTextEntry, pTarget); break;
        case CHAT_TYPE_BOSS_EMOTE: pSource->MonsterTextEmote(iTextEntry, pTarget, true); break;
        ...
        case CHAT_TYPE_BOSS_WHISPER:
        {
            if (pTarget && pTarget->GetTypeId() == TYPEID_PLAYER)
                ...
                sLog.outError(
                    "DoScriptText entry %i cannot whisper without target unit (TYPEID_PLAYER).",
                    iTextEntry);
 ...
 *
 * @param iTextEntry Entry of the text, stored in SD2-database, only type CHAT_TYPE_ZONE_YELL
 * supported
 ...
 * @param pCreatureSource Can be nullptr. If pointer to Creature is given, then the creature does
 * the map-wide text
 * @param pTarget Can be nullptr. Possible target for the text
 */
void DoOrSimulateScriptTextForMap(int32 iTextEntry,
                                  uint32 uiCreatureEntry,
                                  Map* pMap,
                                  Creature* pCreatureSource /*=nullptr*/,
                                  Unit* pTarget /*=nullptr*/)
{
    ...
    if (iTextEntry >= 0)
    {
        sLog.outError("DoOrSimulateScriptTextForMap with source entry %u for map %u attempts to "
    ...
    if (!pInfo)
    {
        sLog.outError("DoOrSimulateScriptTextForMap has invalid source entry %u for map %u.",
                      uiCreatureEntry,
                      pMap->GetId());
        return;
    }
    ...
    if (!pData)
    {
        sLog.outError("DoOrSimulateScriptTextForMap with source entry %u for map %u could not find "
                      "text entry %i.",
                      uiCreatureEntry,
                      pMap->GetId(),
                      iTextEntry);
    ...
    sLog.outDebug("SD2: DoOrSimulateScriptTextForMap: text entry=%i, Sound=%u, Type=%u, "
                  "Language=%u, Emote=%u",
                  iTextEntry,
                  pData->SoundId,
                  pData->Type,
    ...
    if (pData->Type != CHAT_TYPE_ZONE_YELL)
    {
        sLog.outError("DoSimulateScriptTextForMap entry %i has not supported chat type %u.",
                      iTextEntry,
                      pData->Type);
        return;
    }
    ...
    if (pCreatureSource) // If provided pointer for sayer, use direct version
        pMap->MonsterYellToMap(
            pCreatureSource->GetObjectGuid(), iTextEntry, pData->LanguageId, pTarget);
    else // Simulate yell
...
void Script::RegisterSelf(bool bReportError)
{
    ...
    {
        sLog.outError("Script registering but ScriptName %s is not assigned in database. Script "
                      "will not be used.",
                      Name.c_str());
        delete this;
    }
...
void ScriptMgr::FillSpellSummary()
{
    ...
    for (uint32 i = 0; i < sSpellStore.GetNumRows(); ++i)
    {
        ...
        for (uint8 j = 0; j < 3; ++j)
        {
            ...
            // Make sure that this spell includes a healing effect (or an apply aura with a periodic
            // heal)
