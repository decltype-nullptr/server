...
WorldSession::WorldSession(uint32 id,
    ...
    , m_warden(nullptr)
    , m_bot(nullptr)
    , m_lastReceivedPacketTime(0)
    , _clientOS(CLIENT_OS_UNKNOWN)
    , _gameBuild(0)
    , _charactersCount(10)
    , _characterMaxLevel(0)
    , _clientHashComputeStep(HASH_NOT_COMPUTED)
    , m_masterSession(nullptr)
    , m_nodeSession(nullptr)
...
{
    sLog.outError("Client (account %u) send packet %s (%u) with size " SIZEFMTD
...
void WorldSession::SendPacket(WorldPacket const* packet)
{
    ...
    if (packet->size() > 0x8000)
    {
        ...
        sLog.outInfo("[NETWORK] Packet %s size %u is too large. Not sent [Account %u Player %s]",
                     LookupOpcodeName(packet->GetOpcode()),
                     packet->size(),
                     GetAccountId(),
                     GetPlayerName());
        ...
        if (packet->GetOpcode() == SMSG_MESSAGECHAT)
        {
            ...
            if (msgtype == CHAT_MSG_CHANNEL)
            {
                ...
                if (sObjectMgr.GetPlayerNameByGUID(guid1, name1))
                    _chatBotHistory << uint32(msgtype) << " " << name1 << " " << chanName << " "
                                    << message << std::endl;
                return;
            }
            ObjectGuid guid2;
            ...
            if (guid1.IsEmpty() || sObjectMgr.GetPlayerNameByGUID(guid1, name1))
                _chatBotHistory << uint32(msgtype) << " " << name1 << " NULL " << message
                                << std::endl;
        }
        return;
    }
    ...
    else
    {
        ...
        DETAIL_LOG("Send all time packets count: " UI64FMTD " bytes: " UI64FMTD
                   ...
                   uint32(fullTime));
        DETAIL_LOG("Send last min packets count: " UI64FMTD " bytes: " UI64FMTD
    ...
    // sLog.outString("[%s]Send packet : %u|0x%x (%s)", GetPlayerName(), packet->GetOpcode(),
    // packet->GetOpcode(), LookupOpcodeName(packet->GetOpcode()));
    if (Player* player = GetPlayer())
        DEBUG_UNIT_IF(packet->GetOpcode() != SMSG_MESSAGECHAT &&
                          packet->GetOpcode() != SMSG_WARDEN_DATA,
                      player,
                      DEBUG_PACKETS_SEND,
                      "[%s] Send packet : %u/0x%x (%s)",
                      player->GetName(),
                      packet->GetOpcode(),
                      packet->GetOpcode(),
                      LookupOpcodeName(packet->GetOpcode()));
...
void WorldSession::QueuePacket(WorldPacket* newPacket, NodeSession* from_node)
{
    ...
    if (m_nodeSession && m_nodeSession != from_node &&
        sNodesOpcodes->IsOpcodeForwardedToNode(newPacket->GetOpcode()))
    {
    ...
    if (processing != PACKET_PROCESS_WORLD && processing != PACKET_PROCESS_DB_QUERY)
        if (!IsNode() && GetMasterPlayer() &&
            sNodesOpcodes->IsOpcodeHandledByMaster(newPacket->GetOpcode()))
...
{
    sLog.outError("SESSION: opcode %s (0x%.4X) have unprocessed tail data (read stop at " SIZEFMTD
                  " from " SIZEFMTD ")",
                  LookupOpcodeName(packet->GetOpcode()),
                  packet->GetOpcode(),
                  packet->rpos(),
...
{
    if (!sWorld.IsStopped() && GetPlayer() &&
        (GetPlayer()->IsBeingTeleportedFar() ||
         GetPlayer()->FindMap() && GetPlayer()->IsInWorld()) &&
        sPlayerBotMgr.ForceLogoutDelay())
    {
        sLog.out(LOG_CHAR,
                 "Account: %d (IP: %s) Lost socket for character:[%s] (guid: %u)",
                 GetAccountId(),
...
bool WorldSession::Update(PacketFilter& updater)
{
    ...
    if (sWorld.getConfig(CONFIG_UINT32_PERFLOG_SLOW_UNIQUE_SESSION_UPDATE) &&
        sessionUpdateTime > sWorld.getConfig(CONFIG_UINT32_PERFLOG_SLOW_UNIQUE_SESSION_UPDATE))
        sLog.out(LOG_PERFORMANCE,
                 "Slow session update: %ums. Account %u on IP %s",
                 sessionUpdateTime,
                 GetAccountId(),
                 GetRemoteAddress().c_str());
            ...
            else
            {
                ...
                while (true)
                {
                    if (data.size() >= size && readValue != 256)
                    {
                        if (_player)
                            ChatHandler(_player).PSendSysMessage(
                                "[Replay] Invalid packet size [opcode %s|size %u|time %u]",
                    ...
                    if (!readCount)
                    {
                        if (_player)
                            ChatHandler(_player).PSendSysMessage(
                                "[Replay] Invalid packet (truncated) [opcode %s|size %u|time %u]",
                                LookupOpcodeName(opcode),
                                size,
                                nextTime);
                        errorWhileReading = true;
...
bool WorldSession::CanProcessPackets() const
{
    ...
    return ((m_Socket && !m_Socket->IsClosed()) ||
            (_player && sPlayerBotMgr.IsChatBot(_player->GetGUIDLow())));
}
...
void WorldSession::ProcessPackets(PacketFilter& updater)
{
    ...
    while (CanProcessPackets() && _recvQueue[ updater.PacketProcessType() ].next(packet, updater))
    {
        ...
        {
            if (!IsNode() && GetMasterPlayer() &&
                sNodesOpcodes->IsOpcodeHandledByMaster(packet->GetOpcode()))
            {
            ...
            switch (opHandle.status)
            {
                case STATUS_LOGGEDIN:
                    ...
                    if (!_player)
                    {
                        // skip STATUS_LOGGEDIN opcode unexpected errors if player logout sometime
                        // ago - this can be network lag delayed packets
                    ...
                    // lag can cause STATUS_LOGGEDIN opcodes to arrive after the player started a
                    // transfer
                    break;
                case STATUS_LOGGEDIN_OR_RECENTLY_LOGGEDOUT:
                    if (!_player && !m_playerRecentlyLogout)
                        LogUnexpectedOpcode(
                            packet, "the player has not logged in yet and not recently logout");
                    else
            ...
            if (sWorld.getConfig(CONFIG_UINT32_PERFLOG_SLOW_PACKET) &&
                packetTime > sWorld.getConfig(CONFIG_UINT32_PERFLOG_SLOW_PACKET))
                sLog.out(LOG_PERFORMANCE,
                         "Slow packet opcode %s: %ums. Account %u on IP %s",
                         opHandle.name,
                         packetTime,
                         GetAccountId(),
                         GetRemoteAddress().c_str());
        ...
        {
            sLog.outError("WorldSession::Update ByteBufferException occured while parsing a packet "
                          "(opcode:0x%x) from client %s, accountid=%i.",
                          packet->GetOpcode(),
                          GetRemoteAddress().c_str(),
                          GetAccountId());
            ...
            {
                DETAIL_LOG("Disconnecting session [account id %u / address %s] for badly formatted "
                           "packet.",
                           GetAccountId(),
                           GetRemoteAddress().c_str());
        ...
        {
            sLog.outInfo("CATCH Exception 'ASSERT' for account %u / IP %s",
                         GetAccountId(),
                         GetRemoteAddress().c_str());
            sLog.outInfo(e.what());
        ...
        {
            sLog.outInfo("CATCH Unknown exception. Account %u / IP %s",
                         GetAccountId(),
                         GetRemoteAddress().c_str());
...
bool WorldSession::UpdateDisconnected(uint32 diff)
{
    ASSERT(!m_connected);
    if (!_player ||
        !(_player->IsInWorld() || !_player->FindMap()) && !_player->IsBeingTeleportedFar())
        return false;
...
void WorldSession::LogoutPlayer(bool Save)
{
    ...
    if (_player)
    {
        ...
        if (!_player->m_InstanceValid && !_player->isGameMaster())
        {
            ...
            // this is a bad place to call for far teleport because we need player to be in world
            // for successful logout  maybe we should implement delayed far teleport logout?
        }

        // FG: finish pending transfers after starting the logout
        // this should fix players being able to logout and login back with full hp at death
        // position
        ...
        // no point resetting online in character table here as Player::SaveToDB() will set it to 1
        // since player has not been removed from world at this stage No SQL injection as AccountID
        // is uint32
        static SqlStatementID id;

        SqlStatement stmt = LoginDatabase.CreateStatement(
            id, "UPDATE account SET current_realm = ?, online = 0 WHERE id = ?");
        stmt.PExecute(uint32(0), GetAccountId());

        ///- If the player is in a guild, update the guild roster and broadcast a logout message to
        /// other guild members
        ...
        if (Map* map = _player->FindMap())
        {
            if (map->IsNonRaidDungeon() && _player->GetGroup())
            {
                ...
                if (at)
                    removedFromMap = _player->TeleportTo(at->target_mapId,
                                                         at->target_X,
                                                         at->target_Y,
                                                         at->target_Z,
        ...
        // some save parts only correctly work in case player present in map/player_lists (pets,
        // etc)
        if (Save)
        ...
        ///- If the player is in a group (or invited), remove him. If the group if then only 1
        /// person, disband the group.
        _player->UninviteFromGroup();

        // remove player from the group if he is:
        // a) in group; b) not in raid group; c) logging out normally (not being kicked or
        // disconnected)
    ...
    if (m_masterPlayer)
    {
        ...
        if (m_masterPlayer->GetSocial())
        {
            sSocialMgr.SendFriendStatus(
                m_masterPlayer, FRIEND_OFFLINE, m_masterPlayer->GetObjectGuid(), true);
...
{
    sLog.outError(
        "SESSION: received opcode %s (0x%.4X) that must be processed in WorldSocket::OnRead",
        LookupOpcodeName(recvPacket.GetOpcode()),
        recvPacket.GetOpcode());
}
...
void WorldSession::LoadTutorialsData()
{
    ...
    auto result = CharacterDatabase.PQuery("SELECT tut0,tut1,tut2,tut3,tut4,tut5,tut6,tut7 "
                                           "FROM character_tutorial WHERE account = '%u'",
                                           GetAccountId());
    ...
    switch (m_tutorialState)
    {
        case TUTORIALDATA_CHANGED:
        {
            SqlStatement stmt = CharacterDatabase.CreateStatement(
                updTutorial,
                "UPDATE character_tutorial SET tut0=?, tut1=?, tut2=?, tut3=?, tut4=?, tut5=?, "
                "tut6=?, tut7=? WHERE account = ?");
        ...
        case TUTORIALDATA_NEW:
        {
            SqlStatement stmt = CharacterDatabase.CreateStatement(
                insTutorial,
                "INSERT INTO character_tutorial (account,tut0,tut1,tut2,tut3,tut4,tut5,tut6,tut7) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        break;
        case TUTORIALDATA_UNCHANGED: break;
    }
...
void WorldSession::ExecuteOpcode(OpcodeHandler const& opHandle, WorldPacket* packet)
{
    ...
    // or call code that can do far teleports in some conditions unexpectedly for generic way work
    // code
    if (_player)
        _player->SetCanDelayTeleport(true);

    // sLog.outString("[%s] Recvd packet : %u/0x%x (%s)", GetUsername().c_str(),
    // packet->GetOpcode(), packet->GetOpcode(), LookupOpcodeName(packet->GetOpcode()));
    if (Player* player = GetPlayer())
        DEBUG_UNIT(player,
                   DEBUG_PACKETS_RECV,
                   "[%s] Recvd packet : %u/0x%x (%s)",
                   player->GetName(),
                   packet->GetOpcode(),
                   packet->GetOpcode(),
                   LookupOpcodeName(packet->GetOpcode()));
    if (_pcktRecvDump)
    {
        ...
        if (_pcktDumpFlags & PACKET_DUMP_SKIP_FREQUENT_OPCODES)
        {
            switch (packet->GetOpcode())
            {
                ...
                case CMSG_GUILD_QUERY: skipPacketAnalysis = true; break;
            }
        }
        if (!skipPacketAnalysis)
        {
            fprintf(_pcktRecvDump,
                    "%u:%s:%zu|",
...
void WorldSession::SetDumpRecvPackets(const char* file)
{
    ...
    if (_pcktRecvDump)
        fprintf(_pcktRecvDump,
                "#Begin packet dump on %s [account %s]\n",
                GetPlayerName(),
                GetUsername().c_str());
}
...
void WorldSession::ProcessAnticheatAction(const char* detector,
                                          const char* reason,
                                          uint32 cheatAction,
                                          uint32 banSeconds)
{
    ...
    if (cheatAction & CHEAT_ACTION_MUTE_PUB_CHANS)
    {
        ...
        if (GetSecurity() == SEC_PLAYER)
        {
            LoginDatabase.PExecute("UPDATE account SET flags = flags | 0x%x WHERE id = %u",
                                   GetAccountId(),
                                   ACCOUNT_FLAG_MUTED_FROM_PUBLIC_CHANNELS);
    ...
    if (cheatAction & CHEAT_ACTION_BAN_IP_ACCOUNT)
    {
        ...
        if (GetSecurity() == SEC_PLAYER)
        {
            ...
            sWorld.BanAccount(
                BAN_IP, GetRemoteAddress(), banSeconds, banIpReason.str().c_str(), detector);
        }
    }
    ...
    {
        std::stringstream oss;
        oss << "<None> [" << GetUsername() << ":" << GetAccountId() << "@"
            << GetRemoteAddress().c_str() << "]";
        playerDesc = oss.str();
    }
...
bool WorldSession::AllowPacket(uint16 opcode)
{
    ...
    switch (opcode)
    {
        ...
        case CMSG_BUY_ITEM:
        case CMSG_SELL_ITEM: _floodPacketsCount[ FLOOD_SLOW_OPCODES ]++;
        default: break;
    }
    ...
    if (reason.str() != "")
    {
        ...
        ProcessAnticheatAction(
            "AntiFlood", reason.str().c_str(), sWorld.getConfig(CONFIG_UINT32_ANTIFLOOD_SANCTION));
        return false;
    }
...
{
    std::stringstream oss;
    for (ClientIdentifiersMap::const_iterator it = _clientIdentifiers.begin();
         it != _clientIdentifiers.end();
         ++it)
    {
        Sha1Hash sha;
...
void WorldSession::LoginPlayerToNode(NodeSession* session)
{
    ...
    ///- If the player is in a group (or invited), remove him. If the group if then only 1 person,
    /// disband the group.
    _player->UninviteFromGroup();
...
bool WorldSession::CharacterScreenIdleKick(uint32 currTime)
{
    ...
    {
        DEBUG_LOG("SESSION: Kicking session [%s] from character selection",
                  GetRemoteAddress().c_str());
        return true;
    }
