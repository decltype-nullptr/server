...
void CharacterDatabaseCache::LoadCharacterPet(uint32 singlePetId)
{
    ...
    if (singlePetId)
    {
        result = CharacterDatabase.PQuery(
            "SELECT id, entry, owner, modelid, level, exp, Reactstate, loyaltypoints, loyalty, "
            "trainpoint, "
            "slot, name, renamed, curhealth, curmana, curhappiness, abdata, TeachSpelldata, "
            "savetime, resettalents_cost, "
...
void CharacterDatabaseCache::LoadPetSpell(uint32 singlePetId)
{
    ...
    if (singlePetId)
    {
        result = CharacterDatabase.PQuery("SELECT guid,spell,active "
    ...
    else
    {
        ...
        result = CharacterDatabase.Query("SELECT guid,spell,active "
...
void CharacterDatabaseCache::LoadPetSpellCooldown(uint32 singlePetId)
{
    ...
    if (singlePetId)
    {
        result = CharacterDatabase.PQuery("SELECT guid,spell,time "
    ...
    else
    {
        ...
        result = CharacterDatabase.Query("SELECT guid,spell,time "
...
void CharacterDatabaseCache::LoadPetAura(uint32 singlePetId)
{
    ...
    {
        result = CharacterDatabase.PQuery(
            "SELECT guid, caster_guid, item_guid, spell, stackcount, remaincharges, maxduration, "
            "remaintime, effIndexMask, "
    ...
    uint32 count                    = 0;
    CharacterPetCache* lastPetCache = NULL;
#define NEXT_UINT32(where)                          \
    {                                               \
        where = fields[ uiFieldCount ].GetUInt32(); \
        ++uiFieldCount;                             \
    }
#define NEXT_INT32(where)                          \
    {                                              \
        where = fields[ uiFieldCount ].GetInt32(); \
        ++uiFieldCount;                            \
    }
    do
    {
...
CharacterPetCache* CharacterDatabaseCache::GetCharacterPetCacheByOwnerAndId(uint64 owner, uint32 id)
{
    ...
    if (ownerPets == m_petsByCharacter.end())
        return NULL;
    for (CharPetVector::iterator it = ownerPets->second.begin(); it != ownerPets->second.end();
         ++it)
        if ((*it)->id == id)
            return (*it);
...
CharacterPetCache* CharacterDatabaseCache::GetCharacterPetByOwnerAndEntry(uint64 owner,
                                                                          uint32 entry)
{
    // FROM character_pet WHERE owner = '%u' AND entry = '%u' AND (slot = 'PET_SAVE_AS_CURRENT' OR
    ...
    for (CharPetVector::iterator it = ownerPets->second.begin(); it != ownerPets->second.end();
         ++it)
        if ((*it)->entry == entry &&
            ((*it)->slot == PET_SAVE_AS_CURRENT || (*it)->slot > PET_SAVE_LAST_STABLE_SLOT))
            return (*it);
...
{
    // FROM character_pet WHERE owner = '%u' AND (slot = 'PET_SAVE_AS_CURRENT' OR slot >
