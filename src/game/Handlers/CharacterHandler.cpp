...
bool LoginQueryHolder::Initialize()
{
    ...
    // NOTE: all fields in `characters` must be read to prevent lost character data at next save in
    // case wrong DB structure.
    // !!! NOTE: including unused `zone`,`online`
    res &= SetPQuery(
        PLAYER_LOGIN_QUERY_LOADFROM,
        "SELECT guid, account, name, race, class, gender, level, xp, money, playerBytes, "
        ...
        "resettalents_time, trans_x, trans_y, trans_z, trans_o, transguid, extra_flags, "
        "stable_slots, at_login, zone, online, death_expire_time, taxi_path, "
        ...
        "world_phase_mask, customFlags FROM characters WHERE guid = '%u'",
        m_guid.GetCounter());
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADBOUNDINSTANCES,
                     "SELECT id, permanent, map, resettime FROM character_instance LEFT JOIN "
                     "instance ON instance = id WHERE guid = '%u'",
                     m_guid.GetCounter());
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADAURAS,
                     "SELECT "
                     "caster_guid,item_guid,spell,stackcount,remaincharges,basepoints0,basepoints1,"
                     "basepoints2,periodictime0,periodictime1,periodictime2,maxduration,remaintime,"
                     "effIndexMask FROM character_aura WHERE guid = '%u'",
                     m_guid.GetCounter());
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADQUESTSTATUS,
                     ...
                     "itemcount1,itemcount2,itemcount3,itemcount4,reward_choice FROM "
                     "character_queststatus WHERE guid = '%u'",
                     m_guid.GetCounter());
    res &=
        SetPQuery(PLAYER_LOGIN_QUERY_LOADHONORCP,
                  "SELECT victimType,victim,cp,date,type FROM character_honor_cp WHERE guid = '%u'",
                  m_guid.GetCounter());
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADINVENTORY,
                     "SELECT * FROM (SELECT creatorGuid, giftCreatorGuid, count, duration, "
                     "charges, flags, enchantments, randomPropertyId, durability, text, bag, slot, "
                     "item, itemEntry, generated_loot FROM character_inventory JOIN item_instance "
                     "ON character_inventory.item = item_instance.guid WHERE "
                     "character_inventory.guid = '%u') as t ORDER BY bag,slot",
                     m_guid.GetCounter());
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADHOMEBIND,
                     "SELECT map,zone,position_x,position_y,position_z FROM character_homebind "
                     "WHERE guid = '%u'",
                     m_guid.GetCounter());
    res &=
        SetPQuery(PLAYER_LOGIN_QUERY_LOADSPELLCOOLDOWNS,
                  "SELECT spell,item,time,cattime FROM character_spell_cooldown WHERE guid = '%u'",
                  m_guid.GetCounter());
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADBGDATA,
                     "SELECT instance_id, team, join_x, join_y, join_z, join_o, join_map FROM "
                     "character_battleground_data WHERE guid = '%u'",
                     m_guid.GetCounter());
    ...
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADMAILS,
                     "SELECT "
                     "id,messageType,sender,receiver,subject,itemTextId,expire_time,deliver_time,"
                     "money,cod,checked,stationery,mailTemplateId,has_items FROM mail WHERE "
                     "receiver = '%u' ORDER BY id DESC",
                     m_guid.GetCounter());
    res &= SetPQuery(PLAYER_LOGIN_QUERY_LOADMAILEDITEMS,
                     "SELECT creatorGuid, giftCreatorGuid, count, duration, charges, flags, "
                     "enchantments, randomPropertyId, durability, text, mail_id, item_guid, "
                     "itemEntry, generated_loot FROM mail_items JOIN item_instance ON item_guid = "
                     "guid WHERE receiver = '%u'",
                     m_guid.GetCounter());
...
{
public:
    void HandleCharEnumCallback(const std::shared_ptr<QueryResult>& result, uint32 account)
    {
    ...
    void HandlePlayerLoginCallback(const std::shared_ptr<QueryResult>& /*dummy*/,
...
void WorldSession::HandleCharEnum(const std::shared_ptr<QueryResult>& result)
{
    ...
    if (result)
    {
        do
        {
            ...
            DETAIL_LOG(
                "Build enum data for char guid %u from account %u.", guidlow, GetAccountId());
...
void WorldSession::HandleCharEnumOpcode(WorldPacket& /*recv_data*/)
{
    ...
    CharacterDatabase.AsyncPQuery(
        {//           0               1                2                3
         ...
         "characters.position_z, guild_member.guildid, characters.playerFlags, "
         //  15                    16                   17                     18 19
         "characters.at_login, character_pet.entry, character_pet.modelid, character_pet.level, "
         "characters.equipmentCache "
         "FROM characters LEFT JOIN character_pet ON characters.guid=character_pet.owner AND "
         "character_pet.slot='%u' "
...
void WorldSession::HandleCharCreateOpcode(WorldPacket& recv_data)
{
    ...
    if (!classEntry || !raceEntry || !sWorld.getConfig(CONFIG_BOOL_ENABLE_CHAR_CREATION))
    {
        ...
        SendPacket(&data);
        sLog.outError(
            "Class: %u or Race %u not found in DBC (Wrong DBC files?) or Cheater?", class_, race_);
        return;
    }
    ...
    if (!normalizePlayerName(name))
    {
        ...
        SendPacket(&data);
        sLog.outError("Account:[%d] but tried to Create character with empty [name]",
                      GetAccountId());
        return;
    }
    ...
    bool AllowTwoSideAccounts = !sWorld.IsPvPRealm() ||
                                sWorld.getConfig(CONFIG_BOOL_ALLOW_TWO_SIDE_ACCOUNTS) ||
                                GetSecurity() > SEC_PLAYER;
    CinematicsSkipMode skipCinematics =
        CinematicsSkipMode(sWorld.getConfig(CONFIG_UINT32_SKIP_CINEMATICS));

    bool have_same_race = false;
    if (!AllowTwoSideAccounts || skipCinematics == CINEMATICS_SKIP_SAME_RACE)
    {
        auto result2 = CharacterDatabase.PQuery(
    ...
    if (!pNewChar->Create(sObjectMgr.GeneratePlayerLowGuid(),
                          name,
                          race_,
                          class_,
                          gender,
    ...
    BASIC_LOG("Account: %d (IP: %s) Create Character:[%s] (guid: %u)",
              GetAccountId(),
              IP_str.c_str(),
              name.c_str(),
              pNewChar->GetGUIDLow());
...
void WorldSession::HandleCharDeleteOpcode(WorldPacket& recv_data)
{
    ...
    std::string IP_str = GetRemoteAddress();
    BASIC_LOG("Account: %d (IP: %s) Delete Character:[%s] (guid: %u)",
              GetAccountId(),
              IP_str.c_str(),
              name.c_str(),
              lowguid);
    sLog.out(LOG_CHAR,
             "Account: %d (IP: %s) Delete Character:[%s] (guid: %u)",
             GetAccountId(),
             IP_str.c_str(),
             name.c_str(),
             lowguid);
...
void WorldSession::HandlePlayerLoginOpcode(WorldPacket& recv_data)
{
    ...
    CharacterDatabase.DelayQueryHolderUnsafe(
...
void WorldSession::HandlePlayerLogin(LoginQueryHolder* holder)
{
    ...
    {
        sLog.outInfo("[CRASH] HandlePlayerLogin on session %u with player %s [loading=%u]",
                     GetAccountId(),
                     GetPlayerName(),
                     m_playerLoading);
        delete holder;
    ...
    if (pCurrChar && (pCurrChar->IsInWorld() || pCurrChar->IsBeingTeleportedFar()))
    {
        ...
        alreadyOnline = true;
        // If the character had a logout request, then he is articifially stunned (cf
        // CMSG_LOGOUT_REQUEST handler). Fix it here.
    ...
    else
    {
        ...
        {
            sLog.outInfo("[CRASH] Trying to login already ingame character guid %u",
    ...
    // "GetAccountId()==db stored account id" checked in LoadFromDB (prevent login not own character
    // using cheating tools)
    if (alreadyOnline)
        pCurrChar->SendPacketsAtRelogin();
    else if (!pCurrChar->LoadFromDB(playerGuid, holder))
    {
        KickPlayer();     // disconnect client, player no set to session and it will not deleted or
                          // saved at kick
    ...
    if (!alreadyOnline)
    {
        // QueryResult *result = CharacterDatabase.PQuery("SELECT guildid,rank FROM guild_member
        // WHERE guid = '%u'",pCurrChar->GetGUIDLow());
        ...
        else if (pCurrChar->GetGuildId()) // clear guild related fields in case wrong data about
        ...
        if (pCurrChar->GetGuildId() != 0)
        {
            ...
            else
            {
                ...
                sLog.outError("Player %s (GUID: %u) marked as member of nonexistent guild (id: "
                              "%u), removing guild membership for player.",
                              pCurrChar->GetName(),
    ...
    {
        // normal delayed teleport protection not applied (and this correct) for this case (Player
        // object just created)
    ...
    stmt = LoginDatabase.CreateStatement(
        updAccount, "UPDATE account SET current_realm = ?, online = 1 WHERE id = ?");
    stmt.PExecute(realmID, GetAccountId());

    pCurrChar->SetInGameTime(WorldTimer::getMSTime());

    // announce group about member online (must be after add to player list to receive announce to
    // self)
    ...
    if (GetMasterPlayer())
        sSocialMgr.SendFriendStatus(
            GetMasterPlayer(), FRIEND_ONLINE, GetMasterPlayer()->GetObjectGuid(), true);
    ...
    if (!alreadyOnline)
    {
        ...
        if (pCurrChar->m_deathState != ALIVE)
        {
            ...
            if (pCurrChar->getRace() == RACE_NIGHTELF)
                pCurrChar->CastSpell(
                    pCurrChar, 20584, true); // auras SPELL_AURA_INCREASE_SPEED(+speed in wisp
                                             // form), SPELL_AURA_INCREASE_SWIM_SPEED(+swim speed in
                                             // wisp form), SPELL_AURA_TRANSFORM (to wisp form)
            pCurrChar->CastSpell(pCurrChar, 8326, true); // auras SPELL_AURA_GHOST,
    ...
    // Load pet if any (if player not alive and in taxi flight or another then pet will remember as
    // temporary unsummoned)
    if (alreadyOnline)
    ...
    sLog.out(LOG_CHAR,
             "Account: %d (IP: %s) Login Character:[%s] (guid: %u)%s",
             GetAccountId(),
             IP_str.c_str(),
             pCurrChar->GetName(),
             pCurrChar->GetGUIDLow(),
             alreadyOnline ? " Player was already online" : "");
...
void WorldSession::HandleTutorialFlagOpcode(WorldPacket& recv_data)
{
    ...
    if (wInt >= 8)
    {
        // sLog.outError("CHEATER? Account:[%d] Guid[%u] tried to send wrong CMSG_TUTORIAL_FLAG",
        // GetAccountId(),GetGUID());
        return;
    }
...
void WorldSession::HandleCharRenameOpcode(WorldPacket& recv_data)
{
    ...
    CharacterDatabase.AsyncPQuery(
        {"SELECT guid, name FROM characters WHERE guid = %u AND account = "
         "%u AND (at_login & %u) = %u AND NOT EXISTS (SELECT NULL FROM "
        ...
        &WorldSession::HandleChangePlayerNameOpcodeCallBack,
        GetAccountId(),
        newname);
}

void WorldSession::HandleChangePlayerNameOpcodeCallBack(const std::shared_ptr<QueryResult>& result,
                                                        uint32 accountId,
                                                        std::string newname)
{
    ...
    CharacterDatabase.PExecute(
        "UPDATE characters set name = '%s', at_login = at_login & ~ %u WHERE guid ='%u'",
        newname.c_str(),
