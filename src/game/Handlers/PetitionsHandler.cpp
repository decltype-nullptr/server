...
void WorldSession::HandlePetitionBuyOpcode(WorldPacket& recv_data)
{
    ...
    DEBUG_LOG(
        "Petitioner %s tried sell petition: name %s", guidNPC.GetString().c_str(), name.c_str());

    // prevent cheating
    Creature* pCreature = GetPlayer()->GetNPCIfCanInteractWith(guidNPC, UNIT_NPC_FLAG_PETITIONER);
    if (!pCreature)
    {
        DEBUG_LOG("WORLD: HandlePetitionBuyOpcode - %s not found or you can't interact with him.",
    ...
    InventoryResult msg =
        _player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, charterid, pProto->BuyCount);
    if (msg != EQUIP_ERR_OK)
    {
    ...
    auto result = CharacterDatabase.PQuery(
        "SELECT petitionguid FROM petition WHERE ownerguid = '%u'", _player->GetGUIDLow());
    ...
    CharacterDatabase.PExecute("DELETE FROM petition WHERE petitionguid IN ( %s )",
                               ssInvalidPetitionGUIDs.str().c_str());
    CharacterDatabase.PExecute("DELETE FROM petition_sign WHERE petitionguid IN ( %s )",
                               ssInvalidPetitionGUIDs.str().c_str());
    CharacterDatabase.PExecute(
        "INSERT INTO petition (ownerguid, petitionguid, name) VALUES ('%u', '%u', '%s')",
        _player->GetGUIDLow(),
        charter->GetGUIDLow(),
        name.c_str());
...
void WorldSession::HandlePetitionShowSignOpcode(WorldPacket& recv_data)
{
    ...
    // solve (possible) some strange compile problems with explicit use GUID_LOPART(petitionguid) at
    // some GCC versions (wrong code optimization in compiler?)
    ...
    data << uint32(
        petitionguid_low); // guild guid (in mangos always same as GUID_LOPART(petitionguid)
...
void WorldSession::HandlePetitionQueryOpcode(WorldPacket& recv_data)
{
    ...
    recv_data >> petitionguid; // petition guid
    DEBUG_LOG("CMSG_PETITION_QUERY Petition %s Guild GUID %u",
...
void WorldSession::SendPetitionQueryOpcode(ObjectGuid petitionguid)
{
    ...
    auto result = CharacterDatabase.PQuery("SELECT ownerguid, name, "
    ...
    data << uint32(
        petitionLowGuid); // guild/team guid (in mangos always same as GUID_LOPART(petition guid)
    ...
    data << uint32(9);
    data << uint32(
        9); // bypass client - side limitation, a different value is needed here for each petition
    data << uint32(0); // 5
    data << uint32(0); // 6
    data << uint32(0); // 7
    data << uint32(0); // 8
...
void WorldSession::HandlePetitionSignOpcode(WorldPacket& recv_data)
{
    ...
    auto result = CharacterDatabase.PQuery("SELECT ownerguid, "
                                           "  (SELECT COUNT(playerguid) FROM petition_sign WHERE "
                                           "petition_sign.petitionguid = '%u') AS signs "
                                           "FROM petition WHERE petitionguid = '%u'",
                                           petitionLowGuid,
    ...
    // client doesn't allow to sign petition two times by one character, but not check sign by
    // another character from same account  not allow sign another player from already sign player
    // account
    ...
    CharacterDatabase.PExecute("INSERT INTO petition_sign (ownerguid,petitionguid, playerguid, "
    ...
    DEBUG_LOG(
        "PETITION SIGN: %s by %s", petitionGuid.GetString().c_str(), _player->GetGuidStr().c_str());
...
void WorldSession::HandlePetitionDeclineOpcode(WorldPacket& recv_data)
{
    ...
    DEBUG_LOG("Petition %s declined by %s",
              petitionGuid.GetString().c_str(),
    ...
    auto result = CharacterDatabase.PQuery(
        "SELECT ownerguid FROM petition WHERE petitionguid = '%u'", petitionLowGuid);
    if (!result)
        return;
...
void WorldSession::HandleOfferPetitionOpcode(WorldPacket& recv_data)
{
    ...
    DEBUG_LOG("OFFER PETITION: petition %s to %s",
              petitionGuid.GetString().c_str(),
              playerGuid.GetString().c_str());
    ...
    uint8 signs = 0;
    auto result =
        CharacterDatabase.PQuery("SELECT playerguid FROM petition_sign WHERE petitionguid = '%u'",
                                 petitionGuid.GetCounter());
    ...
    data << uint32(
        petitionGuid
            .GetCounter()); // guild guid (in mangos always same as low part of petition guid)
    data << uint8(signs);   // sign's count
...
void WorldSession::HandleTurnInPetitionOpcode(WorldPacket& recv_data)
{
    ...
    DEBUG_LOG("Petition %s turned in by %s",
              petitionGuid.GetString().c_str(),
              _player->GetGuidStr().c_str());
    ...
    // data
    auto result =
        CharacterDatabase.PQuery("SELECT ownerguid, name FROM petition WHERE petitionguid = '%u'",
                                 petitionGuid.GetCounter());
    if (result)
    {
        Field* fields = result->Fetch();
    ...
    {
        sLog.outError("CMSG_TURN_IN_PETITION: petition table not have data for guid %u!",
                      petitionGuid.GetCounter());
        return;
    }
    ...
    CharacterDatabase.PExecute("DELETE FROM petition WHERE petitionguid = '%u'",
                               petitionGuid.GetCounter());
    CharacterDatabase.PExecute("DELETE FROM petition_sign WHERE petitionguid = '%u'",
                               petitionGuid.GetCounter());
    CharacterDatabase.CommitTransaction();
...
void WorldSession::SendPetitionShowList(ObjectGuid guid)
{
    ...
    {
        DEBUG_LOG(
            "WORLD: HandlePetitionShowListOpcode - %s not found or you can't interact with him.",
