...
void BattleGroundAV::setChallengeInvocationCounter(uint32 faction_id,
...
void BattleGroundAV::initializeChallengeInvocationGoals(void)
{
    ...
    for (int i = 0; i < BG_TEAMS_COUNT; ++i)
    {
        ...
        m_challengeGoals[ BG_TEAM_ALLIANCE ][ BG_AV_IRONDEEP_GROUND_ASSAULT ] =
            280; /** Real value 280 */
        m_challengeGoals[ BG_TEAM_ALLIANCE ][ BG_AV_COLDTOOTH_GROUND_ASSAULT ] =
            70; /** Real value 70  */
        m_challengeGoals[ BG_TEAM_HORDE ][ BG_AV_IRONDEEP_GROUND_ASSAULT ] =
            70; /** Real value 70 */
        m_challengeGoals[ BG_TEAM_HORDE ][ BG_AV_COLDTOOTH_GROUND_ASSAULT ] =
            280;                                                      /** Real value 280  */
    ...
    m_challengeTimerStart[ BG_AV_AIR_ASSAULT_BEACON_LIEUTENANT ] =
        1800000;                                                           /** Real value 1800000 */
...
void BattleGroundAV::HandleKillUnit(Creature* creature, Player* killer)
{
    ...
    switch (creature->GetEntry())
    {
        ...
        case NPC_LANDMINES_LAYER_H2:
        {
            ...
            uint8 eventIdx = creature->GetEntry() == NPC_LANDMINES_LAYER_A2 ?
                                 BG_AV_LANDMINES_ALLIANCE :
                                 BG_AV_LANDMINES_HORDE;
        ...
        case NPC_LANDMINES_EXPERT_H2:
        {
            ...
            uint8 eventIdx = creature->GetEntry() == NPC_LANDMINES_EXPERT_A2 ?
                                 BG_AV_LANDMINES_ALLIANCE :
                                 BG_AV_LANDMINES_HORDE;
            Map* map = GetBgMap();
            BGObjects::const_iterator itr =
                m_EventObjects[ MAKE_PAIR32(eventIdx, 0) ].gameobjects.begin();
    ...
    {
        case BG_AV_BOSS_A:
            CastSpellOnTeam(BG_AV_BOSS_KILL_QUEST_SPELL, HORDE); // this is a spell which finishes a
                                                                 // quest where a player has to kill
                                                                 // the boss
            RewardReputationToTeam(BG_AV_FACTION_H, m_RepBoss, HORDE);
            RewardHonorToTeam(GetBonusHonorFromKill(BG_AV_KILL_BOSS), HORDE);
            SendYellToAll(
                LANG_BG_AV_A_GENERAL_DEAD, LANG_UNIVERSAL, GetSingleCreatureGuid(BG_AV_HERALD, 0));
            EndBattleGround(HORDE);
            break;
        case BG_AV_BOSS_H:
            CastSpellOnTeam(BG_AV_BOSS_KILL_QUEST_SPELL, ALLIANCE); // this is a spell which
                                                                    // finishes a quest where a
                                                                    // player has to kill the boss
            RewardReputationToTeam(BG_AV_FACTION_A, m_RepBoss, ALLIANCE);
            RewardHonorToTeam(GetBonusHonorFromKill(BG_AV_KILL_BOSS), ALLIANCE);
            SendYellToAll(
                LANG_BG_AV_H_GENERAL_DEAD, LANG_UNIVERSAL, GetSingleCreatureGuid(BG_AV_HERALD, 0));
        ...
        case BG_AV_MINE_BOSSES_NORTH:
            ...
            else // Never happens, this function HandleKillUnit is only called if a player kill a
                 // unit
...
void BattleGroundAV::UpgradeArmor(Object* questGiver, Player* player)
{
    ...
    if (m_Team_QuestStatus[ teamIdx ][ 0 ] >= 500 &&
        getReinforcementLevelGroundUnit(m_faction_id) == AV_NPC_BASIC)
        ressources = 500;
    else if (m_Team_QuestStatus[ teamIdx ][ 0 ] >= 1000 &&
             getReinforcementLevelGroundUnit(m_faction_id) == AV_NPC_SEASONED)
        ressources = 1000;
    else if (m_Team_QuestStatus[ teamIdx ][ 0 ] >= 1500 &&
             getReinforcementLevelGroundUnit(m_faction_id) == AV_NPC_VETERAN)
        ressources = 1500;

    setReinforcementLevelGroundUnit(teamIdx, ressources);

    if (ressources % 500 == 0 && m_Team_QuestStatus[ teamIdx ][ 0 ] != 0 &&
        questGiver->GetTypeId() == TYPEID_UNIT)
    {
    ...
    if (m_Team_QuestStatus[ teamIdx ][ 0 ] == 500 || m_Team_QuestStatus[ teamIdx ][ 0 ] == 1000 ||
        m_Team_QuestStatus[ teamIdx ][ 0 ] == 1500) // 25,50,75 turn ins
    {
...
void BattleGroundAV::HandleQuestComplete(WorldObject* questGiver, uint32 questid, Player* player)
{
    ...
    // TODO add events (including quest not available anymore, next quest availabe, go/npc
    // de/spawning)
    switch (questid)
    {
        ...
        case BG_AV_QUEST_H_SCRAPS2:
            ...
            if ((m_Team_QuestStatus[ teamIdx ][ 0 ] % 100) == 0 &&
                (m_Team_QuestStatus[ teamIdx ][ 0 ] % 500) != 0 &&
                questGiver->GetTypeId() == TYPEID_UNIT)
                ((Creature*)questGiver)
                    ->MonsterSay("Great! Let's keep those supplies coming, people!", 0, 0);

            /*            if(m_Team_QuestStatus[teamIdx][0]%500 == 0 &&
               m_Team_QuestStatus[teamIdx][0] != 0 && questGiver->GetTypeId() == TYPEID_UNIT)
                        {
                            sprintf(sMessageRemaining,"Thanks for the supplies,
               %s",player->GetName());
                            ((Creature*)questGiver)->MonsterSay(sMessageRemaining, 0, 0);
                            ...
                            else if(m_Team_QuestStatus[teamIdx][0] == 1500)
                            {
                                ...
                                sprintf(sMessageRemaining,"Champion units are entering the
               battle!");
                                ((Creature*)questGiver)->MonsterYell(sMessageRemaining, 0, 0);
                            }
            ...
            */
            ...
            else if ((m_Team_QuestStatus[ teamIdx ][ 0 ] % 500) == 0 &&
                     m_Team_QuestStatus[ teamIdx ][ 0 ] != 0)
            {
            ...
            reputation = 1;
            /*           if (m_Team_QuestStatus[teamIdx][0] == 500 || m_Team_QuestStatus[teamIdx][0]
               == 1000 || m_Team_QuestStatus[teamIdx][0] == 1500)  //25,50,75 turn ins
                        {
                            DEBUG_LOG("BattleGroundAV: Quest %i completed starting with unit
               upgrading..", questid); for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i <=
               BG_AV_NODES_FROSTWOLF_HUT; ++i) if (m_Nodes[i].Owner == teamIdx && m_Nodes[i].State
               == POINT_CONTROLLED) PopulateNode(i);
                        }
            */
            break;
        ...
        case BG_AV_QUEST_H_BOSS1:
            m_Team_QuestStatus[ teamIdx ][ 4 ] += 4; // there are 2 quests where you can turn in 5
                                                     // or 1 item.. ( + 4 cause +1 will be done some
                                                     // lines below)
            reputation = 4;
        // no break
        ...
        case BG_AV_QUEST_H_BOSS2:
            ...
            if (m_Team_QuestStatus[ teamIdx ][ 4 ] == 200)
            {
                if (teamIdx == 0)
                    sprintf(sMessageRemaining,
                            "Soldiers of Stormpike, aid and protect us! The Forest Lord has "
                            "granted us his protection. The portal must now be opened!");
                else
                    sprintf(sMessageRemaining,
                            "Soldiers of Frostwolf, come to my aid! The Ice Lord has granted us "
                            "his protection. He's accepted the offering! The time has come to "
                            "unleash him upon the Stormpike Army!");
        ...
        case BG_AV_QUEST_H_NEAR_MINE:
            ...
            {
                DEBUG_LOG("BattleGroundAV: Quest %i completed (need to implement some events here",
                          questid);
                if (m_Team_QuestStatus[ teamIdx ][ 6 ] == 7)
                    DEBUG_LOG("BattleGroundAV: Quest %i completed (need to implement some events "
                              "here - ground assault ready",
                              questid);
            }
            break;
        ...
        case BG_AV_QUEST_H_RIDER_TAME:
            ...
            if (m_Team_QuestStatus[ teamIdx ][ 8 ] % 25 == 0 &&
                m_Team_QuestStatus[ teamIdx ][ 8 ] != 0)
    ...
    if (reputation)
        RewardReputationToTeam((player->GetTeam() == ALLIANCE) ? BG_AV_FACTION_A : BG_AV_FACTION_H,
                               reputation,
                               player->GetTeam());
}
...
{
    // note: to remove reinforcements points must be negative, for adding reinforcements points must
    // be positive
    ...
    if (points < 0)
    {
        ...
        else if (!m_IsInformedNearLose[ teamIdx ] &&
                 m_TeamScores[ teamIdx ] < BG_AV_SCORE_NEAR_LOSE)
        {
    ...
    UpdateWorldState(((teamIdx == BG_TEAM_HORDE) ? BG_AV_Horde_Score : BG_AV_Alliance_Score),
                     m_TeamScores[ teamIdx ]);
}
...
void BattleGroundAV::Update(uint32 diff)
{
    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        if (GetStartDelayTime() <= m_StartDelayTimes[ BG_STARTING_EVENT_THIRD ] &&
        ...
        // looks for all timers of the nodes and destroy the building (for graveyards the building
        // wont get destroyed, it goes just to the other team
...
void BattleGroundAV::EndBattleGround(Team winner)
{
    ...
    for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
    {
        if (tower_survived[ i ])
        {
            ...
            RewardHonorToTeam(
                GetBonusHonorFromKill(tower_survived[ i ] * BG_AV_KILL_SURVIVING_TOWER), team[ i ]);
        }
        DEBUG_LOG("BattleGroundAV: EndbattleGround: bgteam: %u towers:%u honor:%u rep:%u",
                  i,
                  tower_survived[ i ],
                  GetBonusHonorFromKill(tower_survived[ i ] * BG_AV_KILL_SURVIVING_TOWER),
...
void BattleGroundAV::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    ...
    switch (Trigger)
    {
        ...
        case 3331:
            ...
            break;
        default:
            DEBUG_LOG("BattleGroundAV: WARNING: Unhandled AreaTrigger in Battleground: %u",
                      Trigger);
            //            Source->GetSession()->SendAreaTriggerMessage("Warning: Unhandled
            //            AreaTrigger in Battleground: %u", Trigger);
            break;
    }
}
...
{
    const auto& itr = m_PlayerScores.find(Source->GetObjectGuid());
    ...
    switch (type)
    {
        ...
        case SCORE_SECONDARY_OBJECTIVES:
            ...
            break;
        default: BattleGround::UpdatePlayerScore(Source, type, value); break;
    }
}
...
void BattleGroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        ...
        RewardReputationToTeam((ownerTeam == ALLIANCE) ? BG_AV_FACTION_A : BG_AV_FACTION_H,
...
void BattleGroundAV::ChangeMineOwner(uint8 mine, BattleGroundAVTeamIndex teamIdx)
{
    ...
    // TODO changing the owner should result in setting respawntime to infinite for current
    // creatures (they should fight the new ones), spawning new mine owners creatures and changing
    // the chest - objects so that the current owning team can use them
    ...
    {
        PlaySoundToAll((teamIdx == BG_AV_TEAM_ALLIANCE) ? BG_AV_SOUND_ALLIANCE_GOOD :
                                                          BG_AV_SOUND_HORDE_GOOD);
        m_Mine_Reclaim_Timer[ mine ] = BG_AV_MINE_RECLAIM_TIMER;
        SendYell2ToAll(LANG_BG_AV_MINE_TAKEN,
                       LANG_UNIVERSAL,
                       GetSingleCreatureGuid(BG_AV_HERALD, 0),
...
void BattleGroundAV::PopulateMineNode(uint8 mine,
                                      BattleGroundAVTeamIndex teamIdx,
                                      uint32 oldUpgradeAdvance)
{
    ...
    if (ActualFactionUpgrade == ActualMineOwner)
    {
        ...
        SetSpawnEventMode(
            BG_AV_MINE_EVENT + mine, ActualMineOwner + mineDefender * 3, RESPAWN_FORCED);
        // En cas de prise du flag (destroy node)
        SpawnEvent(BG_AV_MINE_EVENT + mine, ActualMineOwner + mineDefender * 3, true, true);
    }
    else
    {
        // En cas de capture du flag (assault node) -> on ne despawn pas l event -> juste stop le
        // repop
        SetSpawnEventMode(
            BG_AV_NODES_MAX + mine, ActualMineOwner + oldMineDefender * 3, RESPAWN_STOP);
    }
}
...
void BattleGroundAV::PopulateNode(BG_AV_Nodes node)
{
    ...
    if (IsGrave(node))
    {
        ...
        {
            // En cas de defense : l event est déjà spawn -> juste set le mode.
            SetSpawnEventMode(BG_AV_NODES_MAX + node,
                              NewteamIdx * BG_AV_MAX_GRAVETYPES + graveDefenderTypeNew,
                              RESPAWN_FORCED);
            // En cas de prise du flag (destroy node)
        ...
        {
            // En cas de capture du flag (assault node) -> on ne despawn pas l event -> juste stop
            // le repop
            SetSpawnEventMode(BG_AV_NODES_MAX + node,
                              OldteamIdx * BG_AV_MAX_GRAVETYPES + graveDefenderTypeOld,
                              RESPAWN_STOP);
            delay = 1;
        }
    }
    if (IsTower(node) && NewteamIdx != BG_AV_TEAM_NEUTRAL)
    {
        ...
        {
            SetSpawnEventMode(
                BG_AV_NODES_MAX + node, (NewteamIdx * BG_AV_MAX_STATES) + 1, RESPAWN_FORCED);
        ...
        {
            SetSpawnEventMode(
                BG_AV_NODES_MAX + node, (OldteamIdx * BG_AV_MAX_STATES) + 1, RESPAWN_STOP);
            delay = 1;
        }
    }
...
void BattleGroundAV::EventPlayerClickedOnFlag(Player* source, GameObject* target_obj)
{
    ...
    switch ((sBattleGroundMgr.GetGameObjectEventIndex(target_obj->GetGUIDLow())).event2 %
            BG_AV_MAX_STATES)
    {
        case POINT_CONTROLLED: EventPlayerAssaultsPoint(source, node); break;
        case POINT_ASSAULTED: EventPlayerDefendsPoint(source, node); break;
        default: break;
    }
}
...
void BattleGroundAV::EventPlayerDefendsPoint(Player* player, BG_AV_Nodes node)
{
    ...
    {
        // until snowfall doesn't belong to anyone it is better handled in assault - code (best
        // would be to have a special function for neutral nodes.. but doing this just for snowfall
        // will be a bit to much i think
    ...
    {
        sLog.outError("BattleGroundAV: player defends point which doesn't belong to his team %i",
                      node);
        return;
    }
    ...
    if (IsTower(node))
    {
        SendYell2ToAll(LANG_BG_AV_TOWER_DEFENDED,
                       LANG_UNIVERSAL,
                       GetSingleCreatureGuid(BG_AV_HERALD, 0),
                       GetNodeName(node),
    ...
    {
        SendYell2ToAll(LANG_BG_AV_GRAVE_DEFENDED,
                       LANG_UNIVERSAL,
                       GetSingleCreatureGuid(BG_AV_HERALD, 0),
                       GetNodeName(node),
                       (teamIdx == BG_TEAM_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE);
        ...
        PlaySoundToAll((teamIdx == BG_TEAM_ALLIANCE) ? BG_AV_SOUND_ALLIANCE_GOOD :
                                                       BG_AV_SOUND_HORDE_GOOD);
    }
}
...
void BattleGroundAV::EventPlayerAssaultsPoint(Player* player, BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        SendYell2ToAll(LANG_BG_AV_TOWER_ASSAULTED,
                       LANG_UNIVERSAL,
                       GetSingleCreatureGuid(BG_AV_HERALD, 0),
                       GetNodeName(node),
                       (teamIdx == BG_TEAM_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE);
    ...
    {
        SendYell2ToAll(LANG_BG_AV_GRAVE_ASSAULTED,
                       LANG_UNIVERSAL,
                       GetSingleCreatureGuid(BG_AV_HERALD, 0),
                       GetNodeName(node),
                       (teamIdx == BG_TEAM_ALLIANCE) ? LANG_BG_ALLY : LANG_BG_HORDE);
    ...
    PlaySoundToAll((teamIdx == BG_TEAM_ALLIANCE) ? BG_AV_SOUND_ALLIANCE_ASSAULTS :
...
void BattleGroundAV::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    ...
    for (uint32 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
    {
        for (uint8 j = 0; j < BG_AV_MAX_STATES; j++)
        {
            ...
            FillInitialWorldState(
                data,
                count,
                BG_AV_NodeWorldStates[ i ][ GetWorldStateType(j, BG_AV_TEAM_ALLIANCE) ],
                m_Nodes[ i ].Owner == BG_AV_TEAM_ALLIANCE && stateok);
            FillInitialWorldState(
                data,
                count,
                BG_AV_NodeWorldStates[ i ][ GetWorldStateType(j, BG_AV_TEAM_HORDE) ],
    ...
    if (m_Mine_Owner[ BG_AV_NORTH_MINE ] != m_Mine_PrevOwner[ BG_AV_NORTH_MINE ])
        FillInitialWorldState(
            data,
            count,
            BG_AV_MineWorldStates[ BG_AV_NORTH_MINE ][ m_Mine_PrevOwner[ BG_AV_NORTH_MINE ] ],
            0);
    ...
    if (m_Mine_Owner[ BG_AV_SOUTH_MINE ] != m_Mine_PrevOwner[ BG_AV_SOUTH_MINE ])
        FillInitialWorldState(
            data,
            count,
            BG_AV_MineWorldStates[ BG_AV_SOUTH_MINE ][ m_Mine_PrevOwner[ BG_AV_SOUTH_MINE ] ],
            0);
}

void BattleGroundAV::UpdateNodeWorldState(BG_AV_Nodes node)
{
    UpdateWorldState(BG_AV_NodeWorldStates[ node ][ GetWorldStateType(m_Nodes[ node ].State,
                                                                      m_Nodes[ node ].Owner) ],
                     1);
    if (m_Nodes[ node ].PrevOwner ==
        BG_AV_TEAM_NEUTRAL) // currently only snowfall is supported as neutral node
...
WorldSafeLocsEntry const* BattleGroundAV::GetClosestGraveYard(Player* plr)
{
    ...
    {
        if (WorldSafeLocsEntry const* gEntry =
                sWorldSafeLocsStore.LookupEntry(plr->GetTeam() == ALLIANCE ? 611 : 610))
            return gEntry;
    }
    ...
    WorldSafeLocsEntry const* good_entry =
        sWorldSafeLocsStore.LookupEntry(BG_AV_GraveyardIds[ teamIdx + 7 ]);
    if (good_entry)
        mindist =
            (good_entry->x - x) * (good_entry->x - x) + (good_entry->y - y) * (good_entry->y - y);
...
void BattleGroundAV::AssaultNode(BG_AV_Nodes node, BattleGroundTeamIndex teamIdx)
{
    ...
    m_Nodes[ node ].Timer =
        (m_Nodes[ node ].PrevOwner != BG_AV_TEAM_NEUTRAL) ? BG_AV_CAPTIME : BG_AV_SNOWFALL_FIRSTCAP;
...
void BattleGroundAV::Reset()
{
    ...
    m_RepSurviveCaptain =
        (isBGWeekend) ? BG_AV_REP_SURVIVING_CAPTAIN_HOLIDAY : BG_AV_REP_SURVIVING_CAPTAIN;
    m_RepSurviveTower =
        (isBGWeekend) ? BG_AV_REP_SURVIVING_TOWER_HOLIDAY : BG_AV_REP_SURVIVING_TOWER;
    ...
    for (BG_AV_Nodes i = BG_AV_NODES_DUNBALDAR_SOUTH; i <= BG_AV_NODES_FROSTWOLF_WTOWER;
         ++i) // towers
        m_ActiveEvents[ BG_AV_COMMANDER_A_MORTIMER + i - BG_AV_NODES_DUNBALDAR_SOUTH ] =
            0; // Commanders are alive

    for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_STONEHEART_GRAVE;
         ++i) // alliance graves
    ...
    for (BG_AV_Nodes i = BG_AV_NODES_ICEBLOOD_GRAVE; i <= BG_AV_NODES_FROSTWOLF_HUT;
         ++i) // horde graves
        InitNode(i, BG_AV_TEAM_HORDE, false);
    for (BG_AV_Nodes i = BG_AV_NODES_ICEBLOOD_TOWER; i <= BG_AV_NODES_FROSTWOLF_WTOWER;
         ++i) // horde towers
...
SetQuestSlotCounter-> Visuel "Complete Quest" dans le journal de quête. (Deco reco disparais il faut
donc modifier la fonction pour que sa le modifie dans `character_queststatus`) SetQuestStatus->
Validation dans la bdd. SendQuestCompleteEvent -> Affiche au joueur que une quête est validée.
*/
...
void BattleGroundAV::HandleCommand(Player* player, ChatHandler* handler, char* args)
{
    ...
    if (commandType == "reinforcement")
    {
        ...
        handler->PSendSysMessage("Reinforcement %s: added %i. Value now %u",
                                 factionId == BG_TEAM_ALLIANCE ? "Alliance" : "Horde",
                                 value,
                                 getReinforcementLevelGroundUnit(factionId));
    }
    else if (commandType == "quest")
    {
        ...
        if (Creature* target = player->getSelectedCreature())
        {
            ...
            handler->PSendSysMessage(
                "Quest #%u completed on target %s", questId, target->GetName());
        }
        else
...
SpellCastResult BattleGroundAV::CheckSpellCast(Player* caster, SpellEntry const* spell)
{
    switch (spell->Id)
    {
        ...
        {
            BattleGroundTeamIndex teamId =
                (spell->Id == SPELL_SUMMON_SHREDDER_A2) ? BG_TEAM_ALLIANCE : BG_TEAM_HORDE;
            ...
            if (Player* shredderOwner = caster->GetMap()->GetPlayer(shredderOwnerGuid))
                if (Unit* charm = shredderOwner->GetCharm())
                    switch (charm->GetEntry())
                    {
                        ...
                        case NPC_SHREDDER_A2: return SPELL_FAILED_SPELL_UNAVAILABLE;
                    }
