    ...
    if (GetStatus() == STATUS_IN_PROGRESS)
    {
        ...
        for (int node = 0; node < BG_AB_NODES_MAX; ++node)
        {
            ...
            if (m_NodeTimers[ node ])
            {
                ...
                else
                {
                    ...
                    if (teamIndex == 0)
                    {
                        SendMessage2ToAll(LANG_BG_AB_NODE_TAKEN,
        ...
        for (int team = 0; team < BG_TEAMS_COUNT; ++team)
        {
            ...
            if (m_lastTick[ team ] > BG_AB_TickIntervals[ points ])
            {
                ...
                {
                    (team == BG_TEAM_ALLIANCE) ? RewardReputationToTeam(509, 10, ALLIANCE) :
                                                 RewardReputationToTeam(510, 10, HORDE);
                    m_ReputationScoreTics[ team ] -= m_ReputationTics;
                }
                if (m_HonorScoreTics[ team ] >= m_HonorTics)
                {
                    RewardHonorToTeam(BG_AB_PerTickHonor[ GetBracketId() ],
                                      (team == BG_TEAM_ALLIANCE) ? ALLIANCE : HORDE);
                    m_HonorScoreTics[ team ] -= m_HonorTics;
                }
                if (!m_IsInformedNearVictory &&
                    m_TeamScores[ team ] > BG_AB_WARNING_NEAR_VICTORY_SCORE)
                {
...
void BattleGroundAB::StartingEventOpenDoors()
{
    for (int i = 0; i < BG_AB_NODES_MAX; ++i)
    {
        ...
        SpawnBGObject(m_BgObjects[ BG_AB_OBJECT_SPEEDBUFF_STABLES + buff + i * 3 ],
                      RESPAWN_IMMEDIATELY);
    }
...
void BattleGroundAB::HandleAreaTrigger(Player* Source, uint32 Trigger)
{
    switch (Trigger)
    {
        ...
        default:
            ...
            // Source->GetSession()->SendAreaTriggerMessage("Warning: Unhandled AreaTrigger in
            // Battleground: %u", Trigger);
            break;
    }
}
...
void BattleGroundAB::_CreateBanner(uint8 node, uint8 type, uint8 teamIndex)
{
    ...
    switch (type)
    {
        case BG_AB_NODE_TYPE_CONTESTED: delay = 1; break;
        case BG_AB_NODE_TYPE_OCCUPIED: delay = 5; break;
    }
...
void BattleGroundAB::FillInitialWorldStates(WorldPacket& data, uint32& count)
{
    ...
    for (uint8 node = 0; node < BG_AB_NODES_MAX; ++node)
        for (uint8 i = 1; i < BG_AB_NODES_MAX; ++i)
            FillInitialWorldState(
                data, count, BG_AB_OP_NODESTATES[ node ] + plusArray[ i ], m_Nodes[ node ] == i);
    ...
    FillInitialWorldState(
        data, count, BG_AB_OP_RESOURCES_WARNING, BG_AB_WARNING_NEAR_VICTORY_SCORE);
...
void BattleGroundAB::_NodeOccupied(uint8 node, Team team)
{
    ...
    for (uint8 i = 0; i < BG_AB_NODES_MAX; ++i)
    {
        if (m_Nodes[ node ] == GetTeamIndexByTeamId(team) + BG_AB_NODE_TYPE_OCCUPIED &&
            !m_NodeTimers[ i ])
            ++capturedNodes;
    }
    if (capturedNodes >= 5)
...
void BattleGroundAB::EventPlayerClickedOnFlag(Player* source, GameObject* target_obj)
{
    ...
    uint32 killCredits[ BG_AB_NODES_MAX ] = {BG_AB_NODE_STABLES_CREDIT,
    ...
    if (m_Nodes[ node ] == BG_AB_NODE_TYPE_NEUTRAL)
    {
        ...
        if (teamIndex == 0)
            SendMessage2ToAll(LANG_BG_AB_NODE_CLAIMED,
                              CHAT_MSG_BG_SYSTEM_ALLIANCE,
                              source,
                              _GetNodeNameId(node),
                              LANG_BG_ALLY);
             ...
             (m_Nodes[ node ] == BG_AB_NODE_STATUS_HORDE_CONTESTED))
    {
        ...
        if (m_prevNodes[ node ] < BG_AB_NODE_TYPE_OCCUPIED)
        {
            ...
            if (teamIndex == BG_TEAM_ALLIANCE)
                SendMessage2ToAll(LANG_BG_AB_NODE_ASSAULTED,
                                  CHAT_MSG_BG_SYSTEM_ALLIANCE,
                                  source,
                                  _GetNodeNameId(node));
            else
        ...
        else
        {
            ...
            if (teamIndex == BG_TEAM_ALLIANCE)
                SendMessage2ToAll(LANG_BG_AB_NODE_DEFENDED,
                                  CHAT_MSG_BG_SYSTEM_ALLIANCE,
                                  source,
                                  _GetNodeNameId(node));
            else
        ...
        sound = (teamIndex == BG_TEAM_ALLIANCE) ? BG_AB_SOUND_NODE_ASSAULTED_ALLIANCE :
    ...
    else
    {
        ...
        if (teamIndex == BG_TEAM_ALLIANCE)
            ...
            SendMessage2ToAll(
                LANG_BG_AB_NODE_ASSAULTED, CHAT_MSG_BG_SYSTEM_HORDE, source, _GetNodeNameId(node));
...
bool BattleGroundAB::SetupBattleGround()
{
    ...
    for (int i = 0; i < BG_AB_NODES_MAX; ++i)
    {
        if (!AddObject(BG_AB_OBJECT_SPEEDBUFF_STABLES + 3 * i,
                       Buff_Entries[ 0 ],
...
void BattleGroundAB::Reset()
{
    ...
    m_ReputationTics =
        (isBGWeekend) ? BG_AB_ABBGWeekendReputationTicks : BG_AB_NotABBGWeekendReputationTicks;
...
WorldSafeLocsEntry const* BattleGroundAB::GetClosestGraveYard(Player* player)
{
    ...
    if (GetStatus() != STATUS_IN_PROGRESS && !player->isGameMaster())
    {
        if (WorldSafeLocsEntry const* gEntry =
                sWorldSafeLocsStore.LookupEntry(player->GetTeam() == ALLIANCE ? 890 : 889))
            return gEntry;
    }
    ...
    if (!nodes.empty())
    {
        ...
        for (uint8 i = 0; i < nodes.size(); ++i)
        {
            WorldSafeLocsEntry const* entry =
                sWorldSafeLocsStore.LookupEntry(BG_AB_GraveyardIds[ nodes[ i ] ]);
            if (!entry)
                continue;
            float dist =
                (entry->x - plr_x) * (entry->x - plr_x) + (entry->y - plr_y) * (entry->y - plr_y);
            if (mindist > dist)
            {
                mindist    = dist;
                good_entry = entry;
