...
BattleGroundQueue::~BattleGroundQueue()
{
    ...
    for (int i = 0; i < MAX_BATTLEGROUND_BRACKETS; ++i)
    {
        ...
        {
            for (GroupsQueueType::iterator itr = m_QueuedGroups[ i ][ j ].begin();
                 itr != m_QueuedGroups[ i ][ j ].end();
                 ++itr)
                delete (*itr);
                                            ...
                                            bool isPremade)
{
    ...
    DEBUG_LOG("Adding Group to BattleGroundQueue bgTypeId : %u, bracket_id : %u, index : %u",
              BgTypeId,
              bracketId,
              index);
    ...
    // add players from group to ginfo
    {
        ...
        if (grp)
        {
            for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                sLog.out(LOG_BG,
                         "%s:%u [%u:%s] tag BG=%u (groupLeader '%s')",

                         member->GetName(),
                         member->GetGUIDLow(),
                         member->GetSession()->GetAccountId(),
                         member->GetSession()->GetRemoteAddress().c_str(),
                         BgTypeId,
                         leader->GetName());
            }
        }
        else
        {
            ...
            sLog.out(LOG_BG,
                     ...
                     leader->GetSession()->GetRemoteAddress().c_str(),
                     BgTypeId);
        }
        ...
        if (!isPremade && sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_QUEUE_ANNOUNCER_JOIN))
        {
            ...
            if (bg)
            {
                ...
                uint32 qAlliance   = 0;
                uint32 q_min_level =
                    leader->GetMinLevelForBattleGroundBracketId(bracketId, BgTypeId);
                uint32 q_max_level =
                    leader->GetMaxLevelForBattleGroundBracketId(bracketId, BgTypeId);
                GroupsQueueType::const_iterator itr;
                for (itr = m_QueuedGroups[ bracketId ][ BG_QUEUE_NORMAL_ALLIANCE ].begin();
                     itr != m_QueuedGroups[ bracketId ][ BG_QUEUE_NORMAL_ALLIANCE ].end();
                     ++itr)
                    if (!(*itr)->IsInvitedToBGInstanceGUID)
                        qAlliance += (*itr)->Players.size();
                for (itr = m_QueuedGroups[ bracketId ][ BG_QUEUE_NORMAL_HORDE ].begin();
                     itr != m_QueuedGroups[ bracketId ][ BG_QUEUE_NORMAL_HORDE ].end();
                     ++itr)
                    if (!(*itr)->IsInvitedToBGInstanceGUID)
                ...
                {
                    ChatHandler(leader).PSendSysMessage(
                ...
                else
                {
                    sWorld.SendWorldText(LANG_BG_QUEUE_ANNOUNCE_WORLD,
                                         ...
                                         q_max_level - 1,
                                         qAlliance,
                                         (MinPlayers > qAlliance) ? MinPlayers - qAlliance :
                                                                    (uint32)0,
                                         qHorde,
                                         (MinPlayers > qHorde) ? MinPlayers - qHorde : (uint32)0);
                }
                                                               ...
                                                               BattleGroundBracketId bracket_id)
{
    ...
    m_SumOfWaitTimes[ team_index ][ bracket_id ] -=
        m_WaitTimes[ team_index ][ bracket_id ][ (*lastPlayerAddedPointer) ];
                                                  ...
                                                  BattleGroundBracketId bracket_id)
{
    ...
    if (m_WaitTimes[ team_index ][ bracket_id ][ COUNT_OF_PLAYERS_TO_AVERAGE_WAIT_TIME - 1 ])
        return (m_SumOfWaitTimes[ team_index ][ bracket_id ] /
...
void BattleGroundQueue::LogQueueInscription(Player* plr,
                                            BattleGroundTypeId BgTypeId,
                                            uint32 uiAction)
{
    ...
    CharacterDatabase.PExecute(
        "INSERT INTO character_bgqueue (playerGUID, playerName, playerIP, BGtype, action, time) "
        " VALUES ('%u', '%s', '%s', '%u', '%u', '%u')",
        plr->GetGUIDLow(),
        sPName.c_str(),
        last_ip.c_str(),
        ...
        uiAction,
        queuing_time);
    // CharacterDatabase.CommitTransaction(); // Pas de commit de transaction sans un
    // BeginTransaction avant.
}
...
void BattleGroundQueue::RemovePlayer(ObjectGuid guid, bool decreaseInvitedCount)
{
    ...
    for (int32 bracket_id_tmp = MAX_BATTLEGROUND_BRACKETS - 1;
         bracket_id_tmp >= 0 && bracket_id == -1;
         --bracket_id_tmp)
    {
        // we must check premade and normal team's queue - because when players from premade are
        // joining bg,  they leave groupinfo so we can't use its players size to find out index
        for (uint32 j = index; j < BG_QUEUE_GROUP_TYPES_COUNT; j += BG_QUEUE_NORMAL_ALLIANCE)
        {
            for (group_itr_tmp = m_QueuedGroups[ bracket_id_tmp ][ j ].begin();
                 group_itr_tmp != m_QueuedGroups[ bracket_id_tmp ][ j ].end();
                 ++group_itr_tmp)
            {
    ...
    if (bracket_id == -1)
    {
        sLog.outError("BattleGroundQueue: ERROR Cannot find groupinfo for %s",
                      guid.GetString().c_str());
        return;
    }
    DEBUG_LOG("BattleGroundQueue: Removing %s, from bracket_id %u",
              guid.GetString().c_str(),
              (uint32)bracket_id);
    ...
    {
        BattleGround* bg =
            sBattleGroundMgr.GetBattleGround(group->IsInvitedToBGInstanceGUID, group->BgTypeId);
        if (bg)
...
bool BattleGroundQueue::InviteGroupToBG(GroupQueueInfo* ginfo, BattleGround* bg, Team side)
{
    ...
    if (!ginfo->IsInvitedToBGInstanceGUID)
    {
        ...
        for (GroupQueueInfoPlayers::iterator itr = ginfo->Players.begin();
             itr != ginfo->Players.end();
             ++itr)
        {
            ...
            BGQueueInviteEvent* inviteEvent =
                new BGQueueInviteEvent(plr->GetObjectGuid(),
            ...
            BGQueueRemoveEvent* removeEvent =
                new BGQueueRemoveEvent(plr->GetObjectGuid(),
                                       ginfo->IsInvitedToBGInstanceGUID,
                                       bgTypeId,
                                       bgQueueTypeId,
                                       ginfo->RemoveInviteTime);
            plr->m_Events.AddEvent(removeEvent,
            ...
            DEBUG_LOG("Battleground: invited %s to BG instance %u queueindex %u bgtype %u, I can't "
                      "help it if they don't press the enter battle button.",
            ...
            sBattleGroundMgr.BuildBattleGroundStatusPacket(
                &data, bg, queueSlot, STATUS_WAIT_JOIN, INVITE_ACCEPT_WAIT_TIME, 0);
...
void BattleGroundQueue::FillPlayersToBG(BattleGround* bg, BattleGroundBracketId bracket_id)
{
    ...
    uint32 aliIndex = 0;
    for (;
         aliIndex < aliCount && m_SelectionPools[ BG_TEAM_ALLIANCE ].AddGroup((*Ali_itr), aliFree);
         aliIndex++)
        ++Ali_itr;
    ...
    for (; hordeIndex < hordeCount &&
           m_SelectionPools[ BG_TEAM_HORDE ].AddGroup((*Horde_itr), hordeFree);
         hordeIndex++)
        ++Horde_itr;
    ...
    1. Last time when BattleGroundQueue::Update was executed we invited all possible players - so
    there is only small possibility that we will invite now whole queue, because only 1 change has
    been made to queues from the last BattleGroundQueue::Update call 2. Other thing we should
    consider is group order in queue
    */
    ...
    while (abs(diffAli - diffHorde) > bg_differential &&
           (m_SelectionPools[ BG_TEAM_HORDE ].GetPlayerCount() > 0 ||
            m_SelectionPools[ BG_TEAM_ALLIANCE ].GetPlayerCount() > 0))
    {
        ...
        if (diffAli < diffHorde)
        {
            ...
            if (m_SelectionPools[ BG_TEAM_ALLIANCE ].KickGroup(diffHorde - diffAli))
            {
                for (; aliIndex < aliCount &&
                       m_SelectionPools[ BG_TEAM_ALLIANCE ].AddGroup(
                           (*Ali_itr), (aliFree >= diffHorde) ? aliFree - diffHorde : 0);
                     aliIndex++)
                    ++Ali_itr;
            }
            // if ali selection is already empty, then kick horde group, but if there are less horde
            // than ali in bg - break;
        ...
        else
        {
            ...
            {
                for (; hordeIndex < hordeCount &&
                       m_SelectionPools[ BG_TEAM_HORDE ].AddGroup(
                           (*Horde_itr), (hordeFree >= diffAli) ? hordeFree - diffAli : 0);
                     hordeIndex++)
                    ++Horde_itr;
            }
...
// it tries to invite as much players as it can - to MaxPlayersPerTeam, because premade groups have
// more than MinPlayersPerTeam players
                                          ...
                                          uint32 MaxPlayersPerTeam)
{
    ...
    for (uint32 queueType = 0; queueType < 2; ++queueType)
        for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
        {
            ...
            for (; itr_team[ i ] != m_QueuedGroups[ bracket_id ][ 2 * queueType + i ].end();
                 ++(itr_team[ i ]))
            {
                if (!(*(itr_team[ i ]))->IsInvitedToBGInstanceGUID &&
                    (*(itr_team[ i ]))->Players.size() >=
                        sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PREMADE_QUEUE_GROUP_MIN_SIZE))
                {
    ...
    // try to invite same number of players - this cycle may cause longer wait time even if there
    // are enough players in queue, but we want ballanced bg
    ...
    // return true if there are enough players in selection pools - enable to work .debug bg command
    // correctly
    ...
    // now check if we can move group from Premade queue to normal queue (timer has expired) or
    // group size lowered!! this could be 2 cycles but i'm checking only first team in queue - it
    // can cause problem - if first is invited to BG and seconds timer expired, but we can ignore
    // it, because players have only 80 seconds to click to enter bg and when they click or after 80
    // seconds the queue info is removed from queue
    uint32 time_before = WorldTimer::getMSTime() -
                         sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_PREMADE_GROUP_WAIT_FOR_MATCH);
    for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
    {
        if (!m_QueuedGroups[ bracket_id ][ BG_QUEUE_PREMADE_ALLIANCE + i ].empty())
        {
            GroupsQueueType::iterator itr =
                m_QueuedGroups[ bracket_id ][ BG_QUEUE_PREMADE_ALLIANCE + i ].begin();
            if (!(*itr)->IsInvitedToBGInstanceGUID &&
                ((*itr)->JoinTime < time_before || (*itr)->Players.size() < MinPlayersPerTeam))
            {
                                         ...
                                         uint32 maxPlayers)
{
    ...
    for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
    {
        ...
        for (; itr_team[ i ] != m_QueuedGroups[ bracket_id ][ BG_QUEUE_NORMAL_ALLIANCE + i ].end();
             ++(itr_team[ i ]))
        {
            if (!(*(itr_team[ i ]))->IsInvitedToBGInstanceGUID)
            {
    ...
    if ((sWorld.getConfig(CONFIG_UINT32_BATTLEGROUND_INVITATION_TYPE) != 0) &&
        ...
        m_SelectionPools[ BG_TEAM_ALLIANCE ].GetPlayerCount() >= minPlayers)
    {
        // we will try to invite more groups to team with less players indexed by j
        ++(itr_team[ j ]); // this will not cause a crash, because for cycle above reached break;
        for (; itr_team[ j ] != m_QueuedGroups[ bracket_id ][ BG_QUEUE_NORMAL_ALLIANCE + j ].end();
             ++(itr_team[ j ]))
        {
            if (!(*(itr_team[ j ]))->IsInvitedToBGInstanceGUID)
                if (!m_SelectionPools[ j ].AddGroup(
                        *(itr_team[ j ]),
                        m_SelectionPools[ (j + 1) % BG_TEAMS_COUNT ].GetPlayerCount()))
                    break;
        }
        // do not allow to start bg with more than 2 players more on 1 faction
        if (abs((int32)(m_SelectionPools[ BG_TEAM_HORDE ].GetPlayerCount() -
                        m_SelectionPools[ BG_TEAM_ALLIANCE ].GetPlayerCount())) > 2)
            return false;
    }
    // allow 1v0 if debug bg
    if (sBattleGroundMgr.isTesting() && (m_SelectionPools[ BG_TEAM_ALLIANCE ].GetPlayerCount() ||
    ...
    // return true if there are enough players in selection pools - enable to work .debug bg command
    // correctly
    return m_SelectionPools[ BG_TEAM_ALLIANCE ].GetPlayerCount() >= minPlayers &&
           m_SelectionPools[ BG_TEAM_HORDE ].GetPlayerCount() >= minPlayers;
}

/*
this method is called when group is inserted, or player / group is removed from BG Queue - there is
only one player's status changed, so we don't use while(true) cycles to invite whole queue it must
be called after fully adding the members of a group to ensure group joining should be called from
BattleGround::RemovePlayer function in some cases
*/
void BattleGroundQueue::Update(BattleGroundTypeId bgTypeId, BattleGroundBracketId bracket_id)
{
    ...
    while (itrOffline != m_QueuedPlayers.end())
    {
        if (!itrOffline->second.online &&
            WorldTimer::getMSTimeDiffToNow(itrOffline->second.LastOnlineTime) >
                OFFLINE_BG_QUEUE_TIME)
        {
    ...
    for (itr = sBattleGroundMgr.BGFreeSlotQueue[ bgTypeId ].begin();
         itr != sBattleGroundMgr.BGFreeSlotQueue[ bgTypeId ].end();
         itr = next)
    {
        next = itr;
        ++next;
        ...
        {
            BattleGround* bg = *itr; // we have to store battleground pointer here, because when
                                     // battleground is full, it is removed from free queue (not yet
                                     // implemented!!)
            ...
            for (GroupsQueueType::const_iterator citr =
                     m_SelectionPools[ BG_TEAM_ALLIANCE ].SelectedGroups.begin();
                 citr != m_SelectionPools[ BG_TEAM_ALLIANCE ].SelectedGroups.end();
                 ++citr)
                InviteGroupToBG((*citr), bg, (*citr)->GroupTeam);
            for (GroupsQueueType::const_iterator citr =
                     m_SelectionPools[ BG_TEAM_HORDE ].SelectedGroups.begin();
                 citr != m_SelectionPools[ BG_TEAM_HORDE ].SelectedGroups.end();
                 ++citr)
                InviteGroupToBG((*citr), bg, (*citr)->GroupTeam);
    ...
    if (bgTypeId == BATTLEGROUND_AV && sWorld.getConfig(CONFIG_UINT32_AV_MIN_PLAYERS_IN_QUEUE) &&
        !sBattleGroundMgr.isTesting())
    {
        ...
        for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
        {
            ...
            for (; itr != m_QueuedGroups[ bracket_id ][ BG_QUEUE_NORMAL_ALLIANCE + i ].end(); ++itr)
                if (!(*itr)->IsInvitedToBGInstanceGUID)
                    playersInQueuePerTeam[ i ]++; // Only one player, because premades are not
                                                  // allowed in AV.
        }
        ...
        {
            // Now randomize
            std::random_shuffle(m_QueuedGroups[ bracket_id ][ BG_QUEUE_NORMAL_ALLIANCE ].begin(),
                                m_QueuedGroups[ bracket_id ][ BG_QUEUE_NORMAL_ALLIANCE ].begin() +
                                    minPlayersInQueue);
            std::random_shuffle(m_QueuedGroups[ bracket_id ][ BG_QUEUE_NORMAL_HORDE ].begin(),
                                m_QueuedGroups[ bracket_id ][ BG_QUEUE_NORMAL_HORDE ].begin() +
                                    minPlayersInQueue);
            sLog.out(LOG_BG,
                     "Alterac queue randomized (%u alliance vs %u horde)",
                     m_QueuedGroups[ bracket_id ][ BG_QUEUE_NORMAL_ALLIANCE ].size(),
                     m_QueuedGroups[ bracket_id ][ BG_QUEUE_NORMAL_HORDE ].size());
        }
    }
    if (bgTypeId == BATTLEGROUND_AV && sWorld.getConfig(CONFIG_UINT32_AV_INITIAL_MAX_PLAYERS) &&
        !sBattleGroundMgr.isTesting() && normalMatchesCreationAttempts)
    {
    ...
    uint32 q_max_level = Player::GetMaxLevelForBattleGroundBracketId(bracket_id, bgTypeId);
    {
        ...
        if (CheckPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam))
        {
            ...
            if (!bg2)
            {
                sLog.outError("BattleGroundQueue::Update - Cannot create battleground: %u",
                              bgTypeId);
                return;
            }
            ...
            for (uint32 i = 0; i < BG_TEAMS_COUNT; i++)
                for (GroupsQueueType::const_iterator citr =
                         m_SelectionPools[ BG_TEAM_ALLIANCE + i ].SelectedGroups.begin();
                     citr != m_SelectionPools[ BG_TEAM_ALLIANCE + i ].SelectedGroups.end();
                     ++citr)
...
bool BGQueueInviteEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    // player logged off (we should do nothing, he is correctly removed from queue in another
    // procedure)
    if (!plr)
        return true;
...
/*
    ...
    4. player left queue and he joined again and he has been invited to same battleground again ->
   we should not remove him from queue yet 5. player is invited to bg and he didn't choose what to
   do and timer expired - only in this condition we should call queue::RemovePlayer we must remove
   player in the 5. case even if battleground object doesn't exist!
*/
bool BGQueueRemoveEvent::Execute(uint64 /*e_time*/, uint32 /*p_time*/)
{
    ...
    if (queueSlot < PLAYER_MAX_BATTLEGROUND_QUEUES) // player is in queue, or in Battleground
    {
        ...
        if (bgQueue.IsPlayerInvited(m_PlayerGuid, m_BgInstanceGUID, m_RemoveTime))
        {
            DEBUG_LOG("Battleground: removing player %u from bg queue for instance %u because of "
                      "not pressing enter battle in time.",
                      plr->GetGUIDLow(),
                      m_BgInstanceGUID);
            ...
            if (bg && bg->GetStatus() != STATUS_WAIT_LEAVE)
                sBattleGroundMgr.ScheduleQueueUpdate(
                    m_BgQueueTypeId, m_BgTypeId, bg->GetBracketId());
...
void BattleGroundMgr::DeleteAllBattleGrounds()
{
    ...
    for (uint32 i = BATTLEGROUND_TYPE_NONE; i < MAX_BATTLEGROUND_TYPE_ID; ++i)
    {
        for (BattleGroundSet::iterator itr = m_BattleGrounds[ i ].begin();
             itr != m_BattleGrounds[ i ].end();)
        {
            BattleGround* bg = itr->second;
            ++itr; // step from invalidate iterator pos in result element remove in ~BattleGround
                   // call
            delete bg;
        }
    }
    ...
    *data << uint32(StatusID); // status
    switch (StatusID)
    {
        ...
        case STATUS_IN_PROGRESS: // status_in_progress
            *data << uint32(
                Time1); // time to bg auto leave, 0 at bg start, 120000 after bg end, milliseconds
            *data << uint32(Time2); // time from bg start, milliseconds
            break;
        default: sLog.outError("Unknown BG status!"); break;
    }
}
...
void BattleGroundMgr::BuildPvpLogDataPacket(WorldPacket* data, BattleGround* bg)
{
    ...
    if (count >= 80) // Client has a hard limit to 80. If we go beyond (but it should not happen
                     // ?!), WoW Error (happening !)
        count = 80;
    *data << (uint32)(count);

    for (BattleGround::BattleGroundScoreMap::const_iterator itr = bg->GetPlayerScoresBegin();
         itr != bg->GetPlayerScoresEnd();
         ++itr)
    {
        if (!count)
            break;
        ...
        *data << uint32(itr->second->BonusHonor);
        ...
        switch (bg->GetTypeID()) // battleground specific things
        {
            case BATTLEGROUND_AV:
                ...
                *data << (uint32)((BattleGroundAVScore*)itr->second)
                             ->SecondaryObjectives; // Mines Taken
                *data << (uint32)((BattleGroundAVScore*)itr->second)
                             ->LieutnantCount; // Lieutnant kills
                *data << (uint32)((BattleGroundAVScore*)itr->second)
                             ->SecondaryNPC; // Secondary unit summons
                break;
            ...
            case BATTLEGROUND_AB:
                ...
                *data
                    << (uint32)((BattleGroundABScore*)itr->second)->BasesDefended; // bases defended
                break;
            default:
                                                                    ...
                                                                    BattleGroundTypeId bgTypeId)
{
    ...
    for (BattleGroundSet::iterator itr = m_BattleGrounds[ bgTypeId ].begin();
         itr != m_BattleGrounds[ bgTypeId ].end();
         ++itr)
    {
                                                      ...
                                                      BattleGroundBracketId bracket_id)
{
    ...
    // the optimalization would be to use as bitmask std::vector<uint32> - but that would only make
    // code unreadable
    uint32 lastId                = 0;
                                                     ...
                                                     BattleGroundBracketId bracket_id)
{
    ...
    if (!bg_template)
    {
        sLog.outError("BattleGround: CreateNewBattleGround - bg template not found for %u",
                      bgTypeId);
        return NULL;
    }
    ...
    switch (bgTypeId)
    {
        case BATTLEGROUND_AV: bg = new BattleGroundAV(*(BattleGroundAV*)bg_template); break;
        case BATTLEGROUND_WS: bg = new BattleGroundWS(*(BattleGroundWS*)bg_template); break;
        case BATTLEGROUND_AB: bg = new BattleGroundAB(*(BattleGroundAB*)bg_template); break;
        default:
...
uint32 BattleGroundMgr::CreateBattleGround(BattleGroundTypeId bgTypeId,
                                           uint32 MinPlayersPerTeam,
                                           uint32 MaxPlayersPerTeam,
                                           uint32 LevelMin,
                                           uint32 LevelMax,
...
void BattleGroundMgr::CreateInitialBattleGrounds()
{
    ...
    //                                                4      5                6              7 8
    auto result =
        WorldDatabase.Query("SELECT id, "
                            "MinPlayersPerTeam,MaxPlayersPerTeam,MinLvl,MaxLvl,AllianceStartLoc,"
                            "AllianceStartO,HordeStartLoc,HordeStartO FROM battleground_template");
    ...
    do
    {
        ...
        {
            sLog.outErrorDb("Table `battleground_template` for id %u have nonexistent "
                            "WorldSafeLocs.dbc id %u in field `AllianceStartLoc`. BG not created.",
                            bgTypeID,
                            start1);
            continue;
        }
        ...
        {
            sLog.outErrorDb("Table `battleground_template` for id %u have non-existed "
                            "WorldSafeLocs.dbc id %u in field `HordeStartLoc`. BG not created.",
                            bgTypeID,
                            start2);
            continue;
        }
        ...
        {
            sLog.outErrorDb(
                "Table `battleground_template` for id %u associated with nonexistent map id %u.",
                bgTypeID,
                mapId);
            continue;
        }

        // DETAIL_LOG("Creating battleground %s, %u-%u", bl->name[sWorld.GetDBClang()], MinLvl,
        // MaxLvl);
        if (!CreateBattleGround(bgTypeID,
                                MinPlayersPerTeam,
                                MaxPlayersPerTeam,
                                MinLvl,
                                MaxLvl,
...
void BattleGroundMgr::SendToBattleGround(Player* pl, uint32 instanceId, BattleGroundTypeId bgTypeId)
{
    ...
    if (bg)
    {
        ...
        DETAIL_LOG("BATTLEGROUND: Sending %s to map %u, X %f, Y %f, Z %f, O %f",
                   pl->GetName(),
                   mapid,
                   x,
                   y,
        ...
        sLog.outError(
            "player %u trying to port to nonexistent bg instance %u", pl->GetGUIDLow(), instanceId);
}
...
BattleGroundQueueTypeId BattleGroundMgr::BGQueueTypeId(BattleGroundTypeId bgTypeId)
{
    switch (bgTypeId)
    {
        case BATTLEGROUND_WS: return BATTLEGROUND_QUEUE_WS;
        case BATTLEGROUND_AB: return BATTLEGROUND_QUEUE_AB;
        case BATTLEGROUND_AV: return BATTLEGROUND_QUEUE_AV;
...
BattleGroundTypeId BattleGroundMgr::BGTemplateId(BattleGroundQueueTypeId bgQueueTypeId)
{
    ...
    {
        case BATTLEGROUND_QUEUE_WS: return BATTLEGROUND_WS;
        case BATTLEGROUND_QUEUE_AB: return BATTLEGROUND_AB;
        case BATTLEGROUND_QUEUE_AV: return BATTLEGROUND_AV;
        default:
...
void BattleGroundMgr::ScheduleQueueUpdate(BattleGroundQueueTypeId bgQueueTypeId,
                                          BattleGroundTypeId bgTypeId,
                                          BattleGroundBracketId bracket_id)
{
    // ACE_Guard<ACE_Thread_Mutex> guard(SchedulerLock);
...
void BattleGroundMgr::LoadBattleMastersEntry()
{
    ...
    do
    {
        ...
        {
            sLog.outErrorDb("Table `battlemaster_entry` contain entry %u for nonexistent "
...
void BattleGroundMgr::LoadBattleEventIndexes()
{
    ...
    auto result =
        ...
        //                           5           6
        WorldDatabase.Query(
            "SELECT data.typ, data.guid1, data.ev1 AS ev1, data.ev2 AS ev2, data.map AS m, "
            "data.guid2, description.map, "
            //                              7                  8                   9
            "description.event1, description.event2, description.description "
            "FROM "
            "(SELECT '1' AS typ, a.guid AS guid1, a.event1 AS ev1, a.event2 AS ev2, b.map AS map, "
            "b.guid AS guid2 "
            "FROM gameobject_battleground AS a "
            "LEFT OUTER JOIN gameobject AS b ON a.guid = b.guid "
            "UNION "
            "SELECT '2' AS typ, a.guid AS guid1, a.event1 AS ev1, a.event2 AS ev2, b.map AS map, "
            "b.guid AS guid2 "
            ...
            // full outer join doesn't work in mysql :-/ so just UNION-select the same again and add
            // a left outer join
            "UNION "
            "SELECT data.typ, data.guid1, data.ev1, data.ev2, data.map, data.guid2, "
            "description.map, "
            "description.event1, description.event2, description.description "
            "FROM "
            "(SELECT '1' AS typ, a.guid AS guid1, a.event1 AS ev1, a.event2 AS ev2, b.map AS map, "
    ...
    do
    {
        ...
        {
            sLog.outErrorDb("BattleGroundEvent: %s with nonexistent guid %u for event: map:%u, "
                            "event1:%u, event2:%u (\"%s\")",
        ...
        if (desc_map != map)
        {
            ...
            {
                sLog.outErrorDb(
                    "BattleGroundEvent: missing db-data for map:%u, event1:%u, event2:%u (\"%s\")",
                    desc_map,
                    desc_event1,
                    desc_event2,
                    description);
            ...
            {
                sLog.outErrorDb("BattleGroundEvent: %s with guid %u is registered, for a "
...
void BattleGroundMgr::PlayerLoggedIn(Player* player)
{
    for (int i = 0; i < PLAYER_MAX_BATTLEGROUND_QUEUES; ++i)
        if (m_BattleGroundQueues[ i ].PlayerLoggedIn(player))
        {
            ...
            uint32 avgTime = m_BattleGroundQueues[ i ].GetAverageQueueWaitTime(
                &groupInfo, player->GetBattleGroundBracketIdFromLevel(groupInfo.BgTypeId));
            ...
            BuildBattleGroundStatusPacket(
                ...
                STATUS_WAIT_QUEUE,
                avgTime,
                WorldTimer::getMSTimeDiff(groupInfo.JoinTime, WorldTimer::getMSTime()));
            player->GetSession()->SendPacket(&data);

            if (groupInfo.IsInvitedToBGInstanceGUID)
                player->SetInviteForBattleGroundQueueType(BattleGroundQueueTypeId(i),
                                                          groupInfo.IsInvitedToBGInstanceGUID);
        }
}
