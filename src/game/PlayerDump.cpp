...
// Low level functions
static bool
findtoknth(std::string& str, int n, std::string::size_type& s, std::string::size_type& e)
{
    int i;
    s = e                       = 0;
...
std::string CreateDumpString(char const* tableName, const std::shared_ptr<QueryResult>& result)
{
    if (!tableName || !result)
        return "";
...
void StoreGUID(const std::shared_ptr<QueryResult>& result, uint32 field, std::set<uint32>& guids)
{
    Field* fields = result->Fetch();
...
void StoreGUID(const std::shared_ptr<QueryResult>& result,
               uint32 data,
               uint32 field,
...
void PlayerDumpWriter::DumpTableContent(std::string& dump,
                                        uint32 guid,
                                        ...
                                        DumpTableType type)
{
    ...
    do
    {
        ...
        auto result =
            CharacterDatabase.PQuery("SELECT * FROM %s WHERE %s", tableFrom, wherestr.c_str());
        if (!result)
            return;
        ...
        do
        {
            ...
            dump += "\n";
             ...
             guids_itr !=
                 guids->end()); // not set case iterate single time, set case iterate for all guids
}
...
std::string PlayerDumpWriter::GetDump(uint32 guid)
{
    ...
    dump += "IMPORTANT NOTE: This sql queries not created for apply directly, use '.pdump load' "
            "command in console or client chat instead.\n";
    dump += "IMPORTANT NOTE: NOT APPLY ITS DIRECTLY to character DB or you will DAMAGE and CORRUPT "
            "character DB\n\n";

    // revision check guard
    auto result = CharacterDatabase.QueryNamed("SELECT * FROM character_db_version LIMIT 1");
    if (result)
    {
        ...
        if (!reqName.empty())
        {
            ...
            sLog.outError("Table 'character_db_version' not have revision guard field, revision "
                          "guard query not added to pdump.");
    }
    else
        sLog.outError("Character DB not have 'character_db_version' table, revision guard query "
                      "not added to pdump.");
...
DumpReturn
PlayerDumpReader::LoadDump(const std::string& file, uint32 account, std::string name, uint32 guid)
{
    ...
    if (ObjectMgr::CheckPlayerName(name, true) == CHAR_NAME_SUCCESS)
    {
        ...
        result =
            CharacterDatabase.PQuery("SELECT * FROM characters WHERE name = '%s'", name.c_str());
        if (result)
        {
    ...
    while (!feof(fin))
    {
        ...
        if (tn.empty())
        {
            sLog.outError("LoadPlayerDump: Can't extract table name from line: '%s'!",
                          line.c_str());
            ROLLBACK(DUMP_FILE_BROKEN);
        }
        ...
        switch (type)
        {
            ...
            case DTT_CHARACTER:
            {
                ...
                if (name == "")
                {
                    ...
                    result = CharacterDatabase.PQuery("SELECT * FROM characters WHERE name = '%s'",
                                                      name.c_str());
                    if (result)
                    {

                        if (!changenth(
                                line, 35, "1")) // characters.at_login set to "rename on login"
                            ROLLBACK(DUMP_FILE_BROKEN);
                    }
                }
                else
            ...
            case DTT_ITEM:
            {
                ...
                if (!changeGuid(line,
                                13,
                                itemTexts,
                                sObjectMgr.m_ItemTextIds.GetNextAfterMaxUsed(),
                                true)) // item_instance.text update
                    ROLLBACK(DUMP_FILE_BROKEN);
                break;
            }
            case DTT_ITEM_GIFT:
            ...
            default: sLog.outError("Unknown dump table type: %u", type); break;
        }
