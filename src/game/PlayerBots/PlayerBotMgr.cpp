...
void PlayerBotMgr::load()
{
    ...
    auto result = LoginDatabase.PQuery("SELECT MAX(id)"
                                       " FROM account");
    if (!result)
    {
    ...
    result = CharacterDatabase.PQuery("SELECT char_guid, chance, ai"
                                      " FROM playerbot");
    if (!result)
...
void PlayerBotMgr::OnBotLogin(PlayerBotEntry* e)
{
    ...
    if (confDebug)
        sLog.outString("[PlayerBot][Login]  '%s' GUID:%u Acc:%u",
                       e->name.c_str(),
                       e->playerGUID,
                       e->accountId);
}
void PlayerBotMgr::OnBotLogout(PlayerBotEntry* e)
{
    ...
    if (confDebug)
        sLog.outString("[PlayerBot][Logout] '%s' GUID:%u Acc:%u",
                       e->name.c_str(),
                       e->playerGUID,
                       e->accountId);
}
...
void PlayerBotMgr::update(uint32 diff)
{
    ...
    while (it != m_tempBots.end())
    {
        if (!it->second)
        {
            ...
            for (std::map<uint32, PlayerBotEntry*>::iterator iter = m_bots.begin();
                 iter != m_bots.end();
                 ++iter)
                if (iter->second->accountId == it->first)
                {
                    iter->second->state =
                        PB_STATE_OFFLINE; // Will get logged out at next WorldSession::Update call
                    m_bots.erase(iter);
                    break;
                }
            m_tempBots.erase(it);
    ...
    for (iter = m_bots.begin(); iter != m_bots.end(); ++iter)
    {
        ...
        if (!sess)
        {
            ...
            // sLog.outString("/!\\ PlayerBot in queue but Session not in World ... Account : %u,
            // GUID : %u", iter->second->accountId, iter->second->playerGUID);
            continue;
        }
...
bool PlayerBotMgr::addBot(uint32 playerGUID, bool chatBot)
{
    ...
    WorldSession* session =
        new WorldSession(accountId, NULL, sAccountMgr.GetSecurity(accountId), 0, LOCALE_enUS);
    session->SetBot(e);
...
bool PlayerBotMgr::deleteRandomBot()
{
    ...
    for (iter = m_bots.begin(); iter != m_bots.end(); iter++)
    {
        if (!iter->second->customBot && !iter->second->isChatBot &&
            iter->second->state == PB_STATE_ONLINE)
        {
            onlinePassed++;
...
bool ChatHandler::HandleBotInfoCommand(char* args)
{
    ...
    PSendSysMessage(
        "Min:%u Max:%u Total:%u", stats.confMinOnline, stats.confMaxOnline, stats.totalBots);
    PSendSysMessage("Loading : %u, Online : %u, Chat : %u",
                    stats.loadingCount,
                    stats.onlineCount,
                    stats.onlineChat);
