...
struct HonorStanding
{
    ...
    bool operator<(const HonorStanding& hs) { return cp > hs.cp; }
};
...
public:
    HonorMaintenancer()
        ...
        , m_lastMaintenanceDay(0)
        , m_nextMaintenanceDay(0)
    {
    }
    ~HonorMaintenancer() {}
...
{
public:
    explicit HonorMgr(Player* owner)
        : m_owner(owner)
    {
    }
    ~HonorMgr() {}
    ...
    void Save();
    void SaveStoredData();
    void Load(const std::shared_ptr<QueryResult>& result);
