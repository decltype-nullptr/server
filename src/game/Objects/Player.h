...
// 2^n values, Player::m_isunderwater is a bitmask. These are mangos internal values, they are never
// send to any client
...
struct PlayerSpell
{
    ...
    bool disabled : 1;  // first rank has been learned in result talent learn but currently talent
                        // unlearned, save max learned ranks
};
...
struct SpellModifier
{
    SpellModifier()
        ...
        , spellId(0)
        , op(MAX_SPELLMOD)
        , type(SPELLMOD_TYPE_NONE)
    {
    }

    SpellModifier(SpellModOp _op,
                  SpellModType _type,
                  int32 _value,
                  uint32 _spellId,
                  uint64 _mask,
...
enum TrainerSpellState
{
    ...
    TRAINER_SPELL_GREEN_DISABLED =
        10 // custom value, not send to client: formally green but learn not allowed
};
...
enum InstanceResetWarningType
{
    ...
    RAID_INSTANCE_WARNING_MIN_SOON = 3, // WARNING! %s is scheduled to reset in %d minute(s). Please
                                        // exit the zone or you will be returned to your bind
                                        // location!
...
struct BGData
{
    ...
    bool m_needSave; ///< true, if saved to DB fields modified after prev. save (marked as "saved"
                     /// above)
};
...
public: // constructors
    TradeData(Player* player, Player* trader)
        ...
        , m_trader(trader)
        , m_accepted(false)
        , m_acceptProccess(false)
        , m_money(0)
        , m_spell(0)
        , m_lastModificationTime(0)
        , m_scamPreventionDelay(0)
    {
    }
...
private:              // fields
    ...
    ObjectGuid
        m_items[ TRADE_SLOT_COUNT ]; // traded itmes from m_player side including non-traded slot

    time_t m_lastModificationTime; // to prevent scam (change gold before the other validates)
    time_t m_scamPreventionDelay;  // to prevent scam, set a delay in milliseconds (CANNOT be less
                                   // than or equal to 10ms) before accepting trade.
};
...
public:
    ...
    {
        return TeleportTo(
            loc.mapid, loc.coord_x, loc.coord_y, loc.coord_z, loc.orientation, options, recover);
    }
    ...
    static bool BuildEnumData(const std::shared_ptr<QueryResult>& result, WorldPacket* p_data);
    ...
    GameObject* GetGameObjectIfCanInteractWith(ObjectGuid guid,
                                               uint32 gameobject_type = MAX_GAMEOBJECT_TYPE) const;
    ...
    {
        return GetSession()->GetSecurity() >= SEC_GAMEMASTER &&
    ...
    bool isEnabledWhisperRestriction() const
    {
        return m_ExtraFlags & PLAYER_EXTRA_WHISP_RESTRICTION;
    }
    ...
    bool isGMChat() const
    {
        return GetSession()->GetSecurity() >= SEC_MODERATOR &&
    ...
    void BuildPlayerChat(WorldPacket* data,
                         uint8 msgtype,
                         const std::string& text,
                         uint32 language) const;
    static void BuildPlayerChat(ObjectGuid senderGuid,
                                uint8 senderChatTag,
                                WorldPacket* data,
                                uint8 msgtype,
                                const std::string& text,
    ...
    InventoryResult
    CanStoreItem(uint8 bag, uint8 slot, ItemPosCountVec& dest, Item* pItem, bool swap = false) const
    {
        if (!pItem)
    ...
    InventoryResult
    CanEquipItem(uint8 slot, uint16& dest, Item* pItem, bool swap, bool not_loading = true) const;

    InventoryResult CanEquipUniqueItem(Item* pItem, uint8 except_slot = NULL_SLOT) const;
    InventoryResult CanEquipUniqueItem(ItemPrototype const* itemProto,
    ...
    Item*
    StoreNewItem(ItemPosCountVec const& pos, uint32 item, bool update, int32 randomPropertyId = 0);
    ...
    void
    AutoStoreLoot(Loot& loot, bool broadcast = false, uint8 bag = NULL_BAG, uint8 slot = NULL_SLOT);

    InventoryResult _CanTakeMoreSimilarItems(uint32 entry,
                                             uint32 count,
                                             Item* pItem,
                                             uint32* no_space_count = NULL) const;
    InventoryResult _CanStoreItem(uint8 bag,
                                  uint8 slot,
                                  ItemPosCountVec& dest,
                                  uint32 entry,
                                  uint32 count,
    ...
    void
    SendEquipError(InventoryResult msg, Item* pItem, Item* pItem2 = NULL, uint32 itemid = 0) const;
    ...
    void
    RewardQuest(Quest const* pQuest, uint32 reward, WorldObject* questGiver, bool announce = true);
    ...
    uint8 GetQuestSlotState(uint16 slot) const
    {
        return GetByteValue(
            PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET + QUEST_COUNT_STATE_OFFSET, 3);
    }
    ...
    void SetQuestSlot(uint16 slot, uint32 quest_id, uint32 timer = 0)
    {
        ...
        SetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET + QUEST_COUNT_STATE_OFFSET,
                       0);
        SetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET + QUEST_TIME_OFFSET, timer);
    }
    void SetQuestSlotCounter(uint16 slot, uint8 counter, uint8 count)
    {
        uint32 val = GetUInt32Value(PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET +
                                    QUEST_COUNT_STATE_OFFSET);
        val &= ~(0x3F << (counter * 6));
    ...
    {
        SetByteFlag(
            PLAYER_QUEST_LOG_1_1 + slot * MAX_QUEST_OFFSET + QUEST_COUNT_STATE_OFFSET, 3, state);
    }
    ...
    void
    CastedCreatureOrGO(uint32 entry, ObjectGuid guid, uint32 spell_id, bool original_caster = true);
    ...
    void SendQuestUpdateAddCreatureOrGo(Quest const* pQuest,
                                        ObjectGuid guid,
    ...
    static bool LoadPositionFromDB(ObjectGuid guid,
                                   uint32& mapid,
                                   float& x,
                                   float& y,
                                   float& z,
    ...
    static void SavePositionInDB(ObjectGuid guid,
                                 uint32 mapid,
                                 float x,
                                 float y,
                                 float z,
    ...
    static void DeleteFromDB(ObjectGuid playerguid,
                             uint32 accountId,
    ...
    void
    LogModifyMoney(int32 d, const char* type, ObjectGuid fromGuid = ObjectGuid(), uint32 data = 0);
    ...
    bool IsImmuneToSpellEffect(SpellEntry const* spellInfo,
    ...
    void _LoadSpellCooldowns(const std::shared_ptr<QueryResult>& result);
    ...
    bool
    UpdateGatherSkill(uint32 SkillId, uint32 SkillValue, uint32 RedLevel, uint32 Multiplicator = 1);
    ...
    void ApplyStatBuffMod(Stats stat, float val, bool apply)
    {
        ApplyModSignedFloatValue(
            (val > 0 ? PLAYER_FIELD_POSSTAT0 + stat : PLAYER_FIELD_NEGSTAT0 + stat), val, apply);
    }
    ...
    {
        return GetFloatValue(positive ? PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE + school :
                                        PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE + school);
    }
    void SetResistanceBuffMods(SpellSchools school, bool positive, float val)
    {
        SetFloatValue(positive ? PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE + school :
                                 PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE + school,
                      val);
    }
    void ApplyResistanceBuffModsMod(SpellSchools school, bool positive, float val, bool apply)
    {
        ApplyModSignedFloatValue(positive ? PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE + school :
                                            PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE + school,
                                 val,
                                 apply);
    }
    void
    ApplyResistanceBuffModsPercentMod(SpellSchools school, bool positive, float val, bool apply)
    {
        ApplyPercentModFloatValue(positive ? PLAYER_FIELD_RESISTANCEBUFFMODSPOSITIVE + school :
                                             PLAYER_FIELD_RESISTANCEBUFFMODSNEGATIVE + school,
                                  val,
                                  apply);
    }
    ...
    void
    ApplyEquipSpell(SpellEntry const* spellInfo, Item* item, bool apply, bool form_change = false);
    ...
    // returns true if the player is in active state for outdoor pvp objective capturing, false
    // otherwise
    ...
    bool
    HasMovementFlag(MovementFlags f) const; // for script access to m_movementInfo.HasMovementFlag
    ...
    {
        return TeleportTo(
            m_homebindMapId, m_homebindX, m_homebindY, m_homebindZ, GetOrientation(), options);
    }
    ...
    bool IsInVisibleList_Unsafe(WorldObject const* u) const
    {
        return this == u || m_visibleGUIDs.find(u->GetObjectGuid()) != m_visibleGUIDs.end();
    }
    ...
    template <class T>
    void UpdateVisibilityOf(WorldObject const* viewPoint,
                            T* target,
                            UpdateData& data,
    ...
    InstancePlayerBind*
    BindToInstance(DungeonPersistentState* save, bool permanent, bool load = false);
    void SendRaidInfo();
    void SendSavedInstances();
    static void ConvertInstancesToGroup(Player* player,
                                        Group* group           = NULL,
                                        ObjectGuid player_guid = ObjectGuid());
...
public:
    ...
    void SetSmartInstanceBindingMode(bool smartRebinding)
    {
        m_smartInstanceRebind = smartRebinding;
    }
...
protected:
    ...
    */
    ...
    // We allow only one timed quest active at the same time. Below can then be simple value instead
    // of set.
    ...
    void _LoadActions(const std::shared_ptr<QueryResult>& result);
    void _LoadAuras(const std::shared_ptr<QueryResult>& result, uint32 timediff);
    void _LoadBoundInstances(const std::shared_ptr<QueryResult>& result);
    void _LoadInventory(const std::shared_ptr<QueryResult>& result, uint32 timediff);
    void _LoadItemLoot(const std::shared_ptr<QueryResult>& result);
    void _LoadMails(const std::shared_ptr<QueryResult>& result);
    void _LoadMailedItems(const std::shared_ptr<QueryResult>& result);
    void _LoadQuestStatus(const std::shared_ptr<QueryResult>& result);
    void _LoadGroup(const std::shared_ptr<QueryResult>& result);
    void _LoadSkills(const std::shared_ptr<QueryResult>& result);
    void LoadSkillsFromFields();
    void _LoadSpells(const std::shared_ptr<QueryResult>& result);
    void _LoadFriendList(const std::shared_ptr<QueryResult>& result);
    bool _LoadHomeBind(const std::shared_ptr<QueryResult>& result);
    void _LoadBGData(const std::shared_ptr<QueryResult>& result);
...
private:
    ...
    InventoryResult _CanStoreItem_InSpecificSlot(uint8 bag,
                                                 uint8 slot,
                                                 ItemPosCountVec& dest,
                                                 ...
                                                 bool swap,
                                                 Item* pSrcItem) const;
    InventoryResult _CanStoreItem_InBag(uint8 bag,
                                        ItemPosCountVec& dest,
                                        ItemPrototype const* pProto,
                                        uint32& count,
                                        bool merge,
    ...
    InventoryResult _CanStoreItem_InInventorySlots(uint8 slot_begin,
                                                   uint8 slot_end,
                                                   ItemPosCountVec& dest,
                                                   ItemPrototype const* pProto,
                                                   uint32& count,
    ...
    {
        // we should not execute delayed teleports for now dead players but has been alive at
        // teleport because we don't want player's ghost teleported from graveyard
...
T Player::ApplySpellMod(uint32 spellId, SpellModOp op, T& basevalue, Spell* spell)
{
    ...
    int32 totalpct  = 0;
    int32 totalflat = 0;
    for (SpellModList::iterator itr = m_spellMods[ op ].begin(); itr != m_spellMods[ op ].end();
         ++itr)
    {
        ...
        else if (mod->type == SPELLMOD_PCT)
        {
            ...
            if (mod->op == SPELLMOD_CASTING_TIME && basevalue >= T(10 * IN_MILLISECONDS) &&
                mod->value <= -100)
                continue;
        ...
        // Nostalrius : fix ecorce (22812 - +1sec incant) + rapidite nature (17116 - sorts instant)
        // = 0sec de cast
