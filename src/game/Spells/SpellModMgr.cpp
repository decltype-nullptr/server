...
void SpellModMgr::LoadSpellMods()
{
    ...
    auto result = WorldDatabase.Query(
        ...
        "Stances, StancesNot, SpellVisual, ManaCostPercentage, StartRecoveryCategory, "
        "StartRecoveryTime, MaxTargetLevel, MaxAffectedTargets, DmgClass, "
        "rangeIndex, RecoveryTime, CategoryRecoveryTime, procCharges, SpellFamilyName, "
        "SpellFamilyFlags, Mechanic "
        "FROM spell_mod");
    uint32 total_count = 0;
    if (!result)
    {
        OUT_LOG();
        OUT_LOG(">> Loaded %u spell modifications. Table spell_mod vide ou inexistante ?",
                total_count);
    }
    else
    {
    ...
    result = WorldDatabase.Query(
        "SELECT Id, EffectIndex, Effect, EffectApplyAuraName, EffectMechanic, "
        "EffectImplicitTargetA, EffectImplicitTargetB, "
        "EffectRadiusIndex, EffectItemType, EffectMiscValue, EffectTriggerSpell, "
        "EffectDieSides, EffectBaseDice, EffectBasePoints, EffectAmplitude, "
        "EffectChainTarget, " // Int
        "EffectDicePerLevel, EffectRealPointsPerLevel, EffectPointsPerComboPoint, "
    ...
    if (!result)
    {
        OUT_LOG();
        OUT_LOG(
            ">> Loaded %u spell effect modifications. Table spell_effect_mod vide ou inexistante ?",
            total_count);
    }
    else
    {
        ...
        do
        {
            ...
            if (!spell)
            {
                OUT_ERR("Spell entry %u from `spell_effect_mod` doesn't exist in dbc, ignoring.",
                        spellid);
                continue;
            }
            if (effect_idx >= MAX_EFFECT_INDEX)
            {
                OUT_ERR("Erreur : Le sort %u a une definition pour l'effet %u, alors que le max "
                        "est %u.",
                        spellid,
                        effect_idx,
                        (MAX_EFFECT_INDEX - 1));
            ...
            // 0   1            2       3                    4               5 6  Id, EffectIndex,
            // Effect, EffectApplyAuraName, EffectMechanic, EffectImplicitTargetA,
