...
uint32 GetSpellCastTime(SpellEntry const* spellInfo, Spell* spell)
{
    ...
    if (spell)
    {
        // Nostalrius: ne pas consommer les SpellModifier si 'castTime' = 0 (Eclat lunaire / Grace
        // de la nature 16886)
        if (castTime)
        ...
        else
        {
            if (spell->IsRangedSpell() && !spell->IsAutoRepeat())
                castTime =
                    int32(castTime * spell->GetCaster()->m_modAttackSpeedPct[ RANGED_ATTACK ]);
        }
    }
...
{
    uint32 CastingTime =
        !IsChanneledSpell(spellProto) ? GetSpellCastTime(spellProto) : GetSpellDuration(spellProto);
    ...
    for (uint32 i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        switch (spellProto->Effect[ i ])
        {
            ...
            case SPELL_EFFECT_HEAL: DirectDamage = true; break;
            case SPELL_EFFECT_APPLY_AURA:
                switch (spellProto->EffectApplyAuraName[ i ])
                {
                    ...
                    case SPELL_AURA_DUMMY: ++effects; break;
                    case SPELL_AURA_MOD_DECREASE_SPEED: ++effects; break;
...
float CalculateCustomCoefficient(SpellEntry const* spellProto,
                                 Unit const* caster,
                                 DamageEffectType const damageType,
                                 float coeff,
                                 Spell* spell)
{
    if (!caster)
    ...
    switch (spellProto->SpellFamilyName)
    {
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (spellProto->IsFitToFamilyMask(UI64LIT(0x0000000008000000)) &&
    ...
    switch (spellInfo->DmgClass)
    {
        case SPELL_DAMAGE_CLASS_MELEE:
            if (spellInfo->AttributesEx3 & SPELL_ATTR_EX3_REQ_OFFHAND)
                ...
                return BASE_ATTACK;
            break;
        case SPELL_DAMAGE_CLASS_RANGED: return RANGED_ATTACK; break;
        default:
            // Wands
...
SpellSpecific GetSpellSpecific(uint32 spellId)
{
    ...
    switch (spellInfo->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            if (spellInfo->AuraInterruptFlags & AURA_INTERRUPT_FLAG_NOT_SEATED)
            {
                ...
                for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
                {
                    switch (spellInfo->EffectApplyAuraName[ i ])
                    {
                        ...
                        case SPELL_AURA_OBS_MOD_MANA: drink = true; break;
                        default: break;
                    }
                }
            ...
            {
                // Well Fed buffs (must be exclusive with Food / Drink replenishment effects, or
                // else Well Fed will cause them to be removed) SpellIcon 2560 is Spell 46687, does
                // not have this flag
        ...
        case SPELLFAMILY_MAGE:
        {
            ...
            if (spellInfo->EffectApplyAuraName[ EFFECT_INDEX_0 ] == SPELL_AURA_MOD_CONFUSE &&
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if ((spellInfo->IsFitToFamilyMask(UI64LIT(0x0000000020180400))) &&
        ...
        case SPELLFAMILY_POTION: return sSpellMgr.GetSpellElixirSpecific(spellInfo->Id);
    }
    ...
    if ((IsSpellHaveAura(spellInfo, SPELL_AURA_TRACK_CREATURES) ||
        ...
        ((spellInfo->AttributesEx & SPELL_ATTR_EX_NOT_RESET_AUTO_ACTIONS) ||
...
bool IsSingleFromSpellSpecificPerTargetPerCaster(SpellSpecific spellSpec1, SpellSpecific spellSpec2)
{
    switch (spellSpec1)
    {
        ...
        case SPELL_JUDGEMENT: return spellSpec1 == spellSpec2;
        default: return false;
    }
}

// target not allow have more one ranks from spell from spell specific per target
bool IsSingleFromSpellSpecificSpellRanksPerTarget(SpellSpecific spellSpec1,
                                                  SpellSpecific spellSpec2)
{
    switch (spellSpec1)
    {
        ...
        case SPELL_AURA:
        case SPELL_CURSE:
        case SPELL_ASPECT: return spellSpec1 == spellSpec2;
        default: return false;
    }
}
...
bool IsSingleFromSpellSpecificPerTarget(SpellSpecific spellSpec1, SpellSpecific spellSpec2)
{
    switch (spellSpec1)
    {
        ...
        case SPELL_WELL_FED: return spellSpec1 == spellSpec2;
        ...
        case SPELL_FLASK_ELIXIR:
            return spellSpec2 == SPELL_BATTLE_ELIXIR || spellSpec2 == SPELL_GUARDIAN_ELIXIR ||
                   spellSpec2 == SPELL_FLASK_ELIXIR;
        case SPELL_FOOD: return spellSpec2 == SPELL_FOOD || spellSpec2 == SPELL_FOOD_AND_DRINK;
        case SPELL_DRINK: return spellSpec2 == SPELL_DRINK || spellSpec2 == SPELL_FOOD_AND_DRINK;
        case SPELL_FOOD_AND_DRINK:
            return spellSpec2 == SPELL_FOOD || spellSpec2 == SPELL_DRINK ||
    ...
    switch (targetA)
    {
        ...
        case TARGET_CASTER_COORDINATES:
            return (targetB == TARGET_ALL_PARTY ||
                    ...
                    targetB == TARGET_AREAEFFECT_INSTANT);
        default: break;
    }
    if (targetB)
...
{
    // positive targets that in target selection code expect target in m_targers, so not that
    // auto-select target by spell data by m_caster and etc
    switch (targetA)
    {
...
{
    // non-positive targets that in target selection code expect target in m_targers, so not that
    // auto-select target by spell data by m_caster and etc
    switch (targetA)
    {
        case TARGET_CHAIN_DAMAGE:
        case TARGET_CURRENT_ENEMY_COORDINATES: return true;
        default: break;
    }
    return false;
}

bool IsPositiveEffect(SpellEntry const* spellproto,
                      ...
                      Unit* victim)
{
    ...
    if (spellproto->IsFitToFamily<SPELLFAMILY_WARLOCK, CF_WARLOCK_HELLFIRE>() &&
        spellproto->SpellIconID == 937 && spellproto->SpellVisual == 5423)
        return true;
    ...
    switch (spellproto->Effect[ effIndex ])
    {
        case SPELL_EFFECT_DUMMY:
            ...
            break;
        // always positive effects (check before target checks that provided non-positive result in
        // some case for positive effects)
        ...
        case SPELL_EFFECT_INSTAKILL:
            ...
            if (spellproto->EffectImplicitTargetA[ effIndex ] == TARGET_SELF &&
                spellproto->EffectImplicitTargetB[ effIndex ] == TARGET_NONE)
                return true;
            ...
            switch (spellproto->EffectApplyAuraName[ effIndex ])
            {
                case SPELL_AURA_DUMMY:
                {
                    ...
                    switch (spellproto->Id)
                    {
                        ...
                        case 23445: // evil twin
                            return false;
                        // some spells have unclear target modes for selection, so just make effect
                        // positive
                        case 27184:
                        case 27190:
                        case 27191:
                        case 27201:
                        case 27202:
                        case 27203: return true;
                        default: break;
                    }
                }
                break;
                case SPELL_AURA_MOD_DAMAGE_DONE: // dependent from base point sign (negative ->
                                                 // negative)
                ...
                case SPELL_AURA_MOD_HEALING_DONE:
                    if (spellproto->CalculateSimpleValue(effIndex) < 0)
                        return false;
                    break;
                case SPELL_AURA_MOD_DAMAGE_TAKEN: // dependent from bas point sign (positive ->
                                                  // negative)
                    if (spellproto->CalculateSimpleValue(effIndex) < 0)
                        return true;
                ...
                case SPELL_AURA_MOD_DAMAGE_PERCENT_DONE:
                    if (spellproto->CalculateSimpleValue(effIndex) > 0)
                        return true; // some expected positive spells have unclear target modes //
                                     // maybe don't need this at all now that we don't check for
                                     // what was SPELL_ATTR_EX_NEGATIVE
                    break;
                case SPELL_AURA_ADD_TARGET_TRIGGER: return true;
                case SPELL_AURA_PERIODIC_TRIGGER_SPELL:
                    if (spellproto->Id != spellproto->EffectTriggerSpell[ effIndex ])
                    {
                        uint32 spellTriggeredId = spellproto->EffectTriggerSpell[ effIndex ];
                        SpellEntry const* spellTriggeredProto =
                            sSpellMgr.GetSpellEntry(spellTriggeredId);
                        ...
                        if (spellTriggeredProto)
                        {
                            ...
                            for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
                            {
                                // if non-positive trigger cast targeted to positive target this
                                // main cast is non-positive this will place this spell auras as
                                // debuffs
                                if (spellTriggeredProto->Effect[ i ] &&
                                    IsPositiveTarget(
                                        spellTriggeredProto->EffectImplicitTargetA[ i ],
                                        spellTriggeredProto->EffectImplicitTargetB[ i ]) &&
                ...
                case SPELL_AURA_PROC_TRIGGER_SPELL:
                    // many positive auras have negative triggered spells at damage for example and
                    // this not make it negative (it can be canceled for example)
                    break;
                case SPELL_AURA_MOD_STUN: // have positive and negative spells, we can't sort its
                                          // correctly at this moment.
                    if (effIndex == EFFECT_INDEX_0 && spellproto->Effect[ EFFECT_INDEX_1 ] == 0 &&
                ...
                case SPELL_AURA_MOD_DETECT_RANGE: return false;
                ...
                case SPELL_AURA_MECHANIC_IMMUNITY:
                {
                    ...
                    switch (spellproto->EffectMiscValue[ effIndex ])
                    {
                        ...
                        case MECHANIC_MOUNT:
                        case MECHANIC_INVULNERABILITY: return false;
                        default: break;
                    }
                }
                break;
...
bool IsPositiveSpell(SpellEntry const* spellproto, Unit* caster, Unit* victim)
{
    ...
    // some self-applied spells have negative effects but in self casting case negative check
    // ignored.
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
        if (spellproto->Effect[ i ] &&
            !IsPositiveEffect(spellproto, SpellEffectIndex(i), caster, victim))
            return false;
    return true;
}
...
bool IsSingleTargetSpell(SpellEntry const* spellInfo)
{
    ...
    // Entangling roots -> Can still have on several targets using [Nature's Grasp] (fixed in patch
    // 2.2)
    if (spellInfo->IsFitToFamily<SPELLFAMILY_DRUID, CF_DRUID_ENTANGLING_ROOTS>() &&
        spellInfo->Id != 19975)
        return true;
    ...
    switch (GetSpellSpecific(spellInfo->Id))
    {
        ...
        case SPELL_MAGE_POLYMORPH: return true;
        default: break;
    }
...
SpellCastResult GetErrorAtShapeshiftedCast(SpellEntry const* spellInfo, uint32 form)
{
    ...
    {
        // needs shapeshift
        if (!(spellInfo->AttributesEx2 & SPELL_ATTR_EX2_NOT_NEED_SHAPESHIFT) &&
            spellInfo->Stances != 0)
            return SPELL_FAILED_ONLY_SHAPESHIFT;
    }
...
void SpellMgr::LoadSpellTargetPositions()
{
    ...
    //                                                4                  5
    auto result =
        WorldDatabase.Query("SELECT id, target_map, target_position_x, target_position_y, "
    ...
    do
    {
        ...
        if (!mapEntry)
        {
            sLog.outErrorDb("Spell (ID:%u) target map (ID: %u) does not exist in `Map.dbc`.",
                            Spell_ID,
                            st.target_mapId);
            continue;
        }
        ...
        if (!spellInfo)
        {
            sLog.outErrorDb("Spell (ID:%u) listed in `spell_target_position` does not exist.",
                            Spell_ID);
            continue;
        }
        ...
        if (!found)
        {
            sLog.outErrorDb("Spell (Id: %u) listed in `spell_target_position` does not have target "
...
struct SpellRankHelper
{
    ...
    void FillHigherRanks()
    {
        ...
        for (std::set<uint32>::const_iterator itr = firstRankSpellsWithCustomRanks.begin();
             itr != firstRankSpellsWithCustomRanks.end();
             ++itr)
            if (!worker.HasEntry(*itr))
                sLog.outErrorDb("Spell %u must be listed in `%s` as first rank for listed custom "
                                "ranks of spell but not found!",
                                *itr,
                                worker.TableName());

        // fill absent non first ranks data base at first rank data
        for (std::set<uint32>::const_iterator itr = firstRankSpells.begin();
             itr != firstRankSpells.end();
             ++itr)
        {
...
struct DoSpellProcEvent
{
    ...
    void operator()(uint32 spell_id)
    {
        ...
        // add ranks only for not filled data (some ranks have ppm data different for ranks for
        // example)
        ...
        else
        {
            ...
            if (spe.schoolMask != r_spe.schoolMask)
                sLog.outErrorDb("Spell %u listed in `spell_proc_event` as custom rank have "
            ...
            for (int32 i = 0; i < MAX_EFFECT_INDEX; ++i)
            {
                ...
                {
                    sLog.outErrorDb("Spell %u listed in `spell_proc_event` as custom rank have "
                                    "different spellFamilyMask from first rank in chain",
                                    spell_id);
                    break;
                }
            }
    ...
    {
        // let have independent data in table for spells with ppm rates (exist rank dependent ppm
        // rate spells)
        if (!spe.ppmRate)
        {
            sLog.outErrorDb("Spell %u listed in `spell_proc_event` is not first rank (%u) in chain",
                            entry,
                            first_id);
    ...
    void AddEntry(SpellProcEventEntry const& spe, SpellEntry const* spell)
    {
        ...
        if (spe.procFlags == 0)
        {
            if (spell->procFlags == 0)
                sLog.outErrorDb("Spell %u listed in `spell_proc_event` probally not triggered "
                                "spell (no proc flags)",
                                spell->Id);
        }
        else
        {
            if (spell->procFlags == spe.procFlags)
                sLog.outErrorDb("Spell %u listed in `spell_proc_event` has exactly same proc flags "
                                "as in spell.dbc, field value redundant",
                                spell->Id);
            else
                isCustom = true;
        }
        ...
        {
            /* enable for re-check cases, 0 chance ok for some cases because in some cases it set by
            another spell/talent spellmod) if (spell->procChance==0 && !spe.ppmRate)
                sLog.outErrorDb("Spell %u listed in `spell_proc_event` probally not triggered spell
            (no chance or ppm)", spell->Id);
            */
        }
        else
        {
            if (spell->procChance == spe.customChance)
                sLog.outErrorDb("Spell %u listed in `spell_proc_event` has exactly same custom "
                                "chance as in spell.dbc, field value redundant",
                                spell->Id);
            else
                isCustom = true;
        }
        ...
        if (!spe.schoolMask && !spe.procFlags && !spe.procEx && !spe.ppmRate && !spe.customChance &&
            !spe.cooldown)
        {
            ...
            if (empty)
                sLog.outErrorDb("Spell %u listed in `spell_proc_event` not have any useful data",
                                spell->Id);
        }
...
void SpellMgr::LoadSpellProcEvents()
{
    ...
    //                                                7       8        9             10
    auto result =
        WorldDatabase.Query("SELECT entry, SchoolMask, SpellFamilyName, SpellFamilyMask0, "
    ...
    SpellRankHelper<SpellProcEventEntry, DoSpellProcEvent, SpellProcEventMap> rankHelper(
        *this, mSpellProcEventMap);
    ...
    sLog.outString();
    sLog.outString(
        ">> Loaded %u extra spell proc event conditions +%u custom proc (inc. +%u custom ranks)",
        rankHelper.worker.count,
        rankHelper.worker.customProc,
        rankHelper.customRank);
}

struct DoSpellProcItemEnchant
{
    DoSpellProcItemEnchant(SpellProcItemEnchantMap& _procMap, float _ppm)
        : procMap(_procMap)
        , ppm(_ppm)
    {
    }
...
void SpellMgr::LoadSpellProcItemEnchant()
{
    ...
    //                                                0      1
    auto result = WorldDatabase.Query("SELECT entry, ppmRate FROM spell_proc_item_enchant");
    if (!result)
    {
    ...
    do
    {
        ...
        {
            sLog.outErrorDb(
                "Spell %u listed in `spell_proc_item_enchant` is not first rank (%u) in chain",
                entry,
                first_id);
            // prevent loading since it won't have an effect anyway
            continue;
...
{
    DoSpellBonuses(SpellBonusMap& _spellBonusMap, SpellBonusEntry const& _spellBonus)
        : spellBonusMap(_spellBonusMap)
...
void SpellMgr::LoadSpellBonuses()
{
    ...
    do
    {
        ...
        if (first_id != entry)
        {
            sLog.outErrorDb("Spell %u listed in `spell_bonus_data` is not first rank (%u) in chain",
                            entry,
                            first_id);
            // prevent loading since it won't have an effect anyway
            continue;
        ...
        if (direct_diff < 0.02f && !need_dot && !sbe.ap_bonus && !sbe.ap_dot_bonus)
            sLog.outErrorDb("`spell_bonus_data` entry for spell %u `direct_bonus` not needed (data "
                            "from table: %f, calculated %f, difference of %f) and `dot_bonus` also "
                            "not used",
                            entry,
                            sbe.direct_damage,
                            direct_calc,
        ...
        {
            sLog.outErrorDb("`spell_bonus_data` entry for spell %u `direct_bonus` not needed (data "
                            "from table: %f, calculated %f, difference of %f) and ",
                            entry,
                            sbe.direct_damage,
                            direct_calc,
                            direct_diff);
            sLog.outErrorDb("                                  ... `dot_bonus` not needed (data "
                            "from table: %f, calculated %f, difference of %f)",
                            sbe.dot_damage,
                            dot_calc,
                            dot_diff);
        }
        else if (!need_direct && dot_diff < 0.02f && !sbe.ap_bonus && !sbe.ap_dot_bonus)
            sLog.outErrorDb("`spell_bonus_data` entry for spell %u `dot_bonus` not needed (data "
                            "from table: %f, calculated %f, difference of %f) and direct also not "
                            "used",
                            entry,
                            sbe.dot_damage,
                            dot_calc,
            ...
            sLog.outErrorDb("`spell_bonus_data` entry for spell %u `direct_bonus` not used (spell "
                            "not have non-periodic affects)",
                            entry);
        else if (!need_dot && sbe.dot_damage)
            sLog.outErrorDb("`spell_bonus_data` entry for spell %u `dot_bonus` not used (spell not "
                            "have periodic affects)",
                            entry);

        if (!need_direct && sbe.ap_bonus)
            sLog.outErrorDb("`spell_bonus_data` entry for spell %u `ap_bonus` not used (spell not "
                            "have non-periodic affects)",
                            entry);
        else if (!need_dot && sbe.ap_dot_bonus)
            sLog.outErrorDb("`spell_bonus_data` entry for spell %u `ap_dot_bonus` not used (spell "
                            "not have periodic affects)",
                            entry);
                                              ...
                                              uint32 procExtra)
{
    ...
    if (spellProcEvent) // Exist event data
    {
        ...
        else // For spells need check school/spell family/family mask
        {
            ...
            if (spellProcEvent->schoolMask &&
                (spellProcEvent->schoolMask & GetSchoolMask(procSpell->School)) == 0)
                return false;
    ...
    if (procEvent_procEx == PROC_EX_NONE)
    {
        ...
        if (EventProcFlag & (PROC_FLAG_ON_DO_PERIODIC | PROC_FLAG_ON_TAKE_PERIODIC) &&
    ...
    else // all spells hits here only if resist/reflect/immune/evade/periodic
    {
        ...
        if ((procEvent_procEx & PROC_EX_NO_PERIODIC) &&
            (procFlags & (PROC_FLAG_ON_DO_PERIODIC | PROC_FLAG_ON_TAKE_PERIODIC)))
            return false;
        // Check Extra Requirement like
        // (hit/crit/miss/resist/parry/dodge/block/immune/reflect/absorb and other)
...
void SpellMgr::LoadSpellGroups()
{
    ...
    //                                                0         1
    auto result = WorldDatabase.Query(
        "SELECT group_id, spell_id FROM spell_group ORDER BY group_id, group_spell_id, spell_id");
    if (!result)
    {
        sLog.outString();
    ...
    for (SpellGroupSpellMap::iterator itr = mSpellGroupSpell.begin();
         itr != mSpellGroupSpell.end();)
    {
        if (itr->second < 0)
        {
            if (groups.find(abs(itr->second)) == groups.end())
            {
                sLog.outErrorDb("SpellGroup id %u listed in `spell_groups` does not exist",
                                abs(itr->second));
        ...
        else
        {
            ...
            {
                sLog.outErrorDb("Spell %u listed in `spell_group` is not first rank of spell",
            itr->second); mSpellGroupSpell.erase(itr++);
            }*/
            else
                ++itr;
        }
    ...
    for (std::set<uint32>::iterator groupItr = groups.begin(); groupItr != groups.end(); ++groupItr)
    {
        ...
        for (std::set<uint32>::iterator spellItr = spells.begin(); spellItr != spells.end();
             ++spellItr)
        {
            ++count;
            mSpellSpellGroup.insert(
                SpellSpellGroupMap::value_type(*spellItr, SpellGroup(*groupItr)));
        }
    }
...
void SpellMgr::LoadSpellGroupStackRules()
{
    ...
    //                                                       0         1
    auto result = WorldDatabase.Query("SELECT group_id, stack_rule FROM spell_group_stack_rules");
    if (!result)
    {
        sLog.outString();
    ...
    do
    {
        ...
        {
            sLog.outErrorDb(
                "SpellGroupStackRule %u listed in `spell_group_stack_rules` does not exist",
                stack_rule);
            continue;
        }
        ...
        {
            sLog.outErrorDb("SpellGroup id %u listed in `spell_group_stack_rules` does not exist",
                            group_id);
            continue;
        }
...
bool SpellMgr::ListMorePowerfullSpells(uint32 spellId, std::list<uint32>& list) const
{
    ...
    for (SpellGroupSpellMap::const_iterator itr = mSpellGroupSpell.begin();
         itr != mSpellGroupSpell.end();
         ++itr)
    {
        if (itr->second == spellId)
        {
            // Un sort peut etre dans plusieurs groupes. On s'interesse au groupe
            // 'SPELL_GROUP_STACK_RULE_POWERFULL_CHAIN'
...
void SpellMgr::LoadSpellElixirs()
{
    ...
    //                                                0      1
    auto result = WorldDatabase.Query("SELECT entry, mask FROM spell_elixir");
    if (!result)
    {
...
{
    DoSpellThreat(SpellThreatMap& _threatMap)
    ...
    void operator()(uint32 spell_id)
    {
        ...
        // add ranks only for not filled data (spells adding flat threat are usually different for
        // ranks)
        ...
        else
        {
            ...
            if (ste.threat == r_ste.threat && ste.multiplier == r_ste.multiplier &&
                ste.ap_bonus == r_ste.ap_bonus)
                sLog.outErrorDb("Spell %u listed in `spell_threat` as custom rank has same data as "
                                "Rank 1, so redundant",
                                spell_id);
        }
    }
    ...
    bool IsValidCustomRank(SpellThreatEntry const& ste, uint32 entry, uint32 first_id)
    {
        if (!ste.threat)
        {
            sLog.outErrorDb("Spell %u listed in `spell_threat` is not first rank (%u) in chain and "
                            "has no threat",
                            entry,
                            first_id);
    ...
    void AddEntry(SpellThreatEntry const& ste, SpellEntry const* spell)
    {
        ...
        if (ste.threat || ste.ap_bonus != 0.f)
        {
            ...
            if ((targetA[ EFFECT_INDEX_1 ] &&
                ...
                sLog.outErrorDb("Spell %u listed in `spell_threat` has effects with different "
                                "targets, threat may be assigned incorrectly",
                                spell->Id);
        }
        ++count;
    }
...
void SpellMgr::LoadSpellThreats()
{
    ...
    //                                                0      1       2           3
    auto result =
        WorldDatabase.Query("SELECT entry, Threat, multiplier, ap_bonus FROM spell_threat");
    if (!result)
    {
        BarGoLink bar(1);
        bar.step();
    ...
    SpellRankHelper<SpellThreatEntry, DoSpellThreat, SpellThreatMap> rankHelper(*this,
                                                                                mSpellThreatMap);
...
bool SpellMgr::IsNoStackSpellDueToSpell(uint32 spellId_1, uint32 spellId_2) const
{
    ...
    if ((spellInfo_1->Id == SPELL_ID_PASSIVE_RESURRECTION_SICKNESS) !=
        (spellInfo_2->Id == SPELL_ID_PASSIVE_RESURRECTION_SICKNESS))
        return false;

    // Allow stack passive and not passive spells
    if ((spellInfo_1->Attributes & SPELL_ATTR_PASSIVE) !=
        (spellInfo_2->Attributes & SPELL_ATTR_PASSIVE))
        return false;
    ...
    switch (spellInfo_1->SpellFamilyName)
    {
        case SPELLFAMILY_GENERIC:
            switch (spellInfo_2->SpellFamilyName)
            {
                case SPELLFAMILY_GENERIC: // same family case
                {
                    ...
                    // Heart of the Wild and (Primal Instinct (Idol of Terror) triggering spell or
                    // Agility)
                ...
                case SPELLFAMILY_WARRIOR:
                {
                    ...
                    if (spellInfo_1->SpellIconID == 276 && spellInfo_1->SpellVisual == 196 &&
                        spellInfo_2->Id == 71)
                        return false;
                ...
                case SPELLFAMILY_DRUID:
                {
                    ...
                    if (spellInfo_1->SpellIconID == 312 && spellInfo_1->SpellVisual == 216 &&
                        spellInfo_2->Id == 24932)
                        return false;
                    break;
                }
                case SPELLFAMILY_ROGUE:
                ...
                case SPELLFAMILY_PALADIN:
                {
                    ...
                    if (spellInfo_2->SpellIconID == 502 && spellInfo_1->SpellIconID == 502 &&
                        spellInfo_1->SpellVisual == 969)
                        return false;
                    break;
                }
            }
        ...
        case SPELLFAMILY_MAGE:
            if (spellInfo_2->SpellFamilyName == SPELLFAMILY_MAGE)
            {
                ...
                if (((spellInfo_1->SpellFamilyFlags & UI64LIT(0x80)) &&
                     (spellInfo_2->SpellFamilyFlags & UI64LIT(0x100000))) ||
                    ((spellInfo_2->SpellFamilyFlags & UI64LIT(0x80)) &&
                     (spellInfo_1->SpellFamilyFlags & UI64LIT(0x100000))))
                    return false;

                // Blink & Improved Blink
                if (((spellInfo_1->SpellFamilyFlags & UI64LIT(0x0000000000010000)) &&
                     (spellInfo_2->SpellVisual == 72 && spellInfo_2->SpellIconID == 1499)) ||
                    ((spellInfo_2->SpellFamilyFlags & UI64LIT(0x0000000000010000)) &&
                     (spellInfo_1->SpellVisual == 72 && spellInfo_1->SpellIconID == 1499)))
                    return false;

                // Fireball & Pyroblast (Dots)
                if (((spellInfo_1->SpellFamilyFlags & UI64LIT(0x1)) &&
                     (spellInfo_2->SpellFamilyFlags & UI64LIT(0x400000))) ||
                    ((spellInfo_2->SpellFamilyFlags & UI64LIT(0x1)) &&
                     (spellInfo_1->SpellFamilyFlags & UI64LIT(0x400000))))
                    return false;
            }
            // Detect Invisibility and Mana Shield (multi-family check)
            if (spellInfo_2->Id == 132 && spellInfo_1->SpellIconID == 209 &&
                spellInfo_1->SpellVisual == 968)
                return false;

            // Combustion and Fire Protection Aura (multi-family check)
            if (spellInfo_1->Id == 11129 && spellInfo_2->SpellIconID == 33 &&
                spellInfo_2->SpellVisual == 321)
                return false;
        ...
        case SPELLFAMILY_WARLOCK:
            if (spellInfo_2->SpellFamilyName == SPELLFAMILY_WARLOCK)
            {
                ...
                if ((spellInfo_1->SpellIconID == 313 &&
                     (spellInfo_2->SpellIconID == 544 || spellInfo_2->SpellIconID == 91)) ||
                    (spellInfo_2->SpellIconID == 313 &&
                     (spellInfo_1->SpellIconID == 544 || spellInfo_1->SpellIconID == 91)))
                    return false;
            }
            // Detect Invisibility and Mana Shield (multi-family check)
            if (spellInfo_1->Id == 132 && spellInfo_2->SpellIconID == 209 &&
                spellInfo_2->SpellVisual == 968)
                return false;
            break;
        case SPELLFAMILY_WARRIOR:
            ...
            {
                // Rend and Deep Wound
                if (((spellInfo_1->SpellFamilyFlags & UI64LIT(0x20)) &&
                     (spellInfo_2->SpellFamilyFlags & UI64LIT(0x1000000000))) ||
                    ((spellInfo_2->SpellFamilyFlags & UI64LIT(0x20)) &&
                     (spellInfo_1->SpellFamilyFlags & UI64LIT(0x1000000000))))
                    return false;
                ...
                if ((spellInfo_2->SpellVisual == 246 && spellInfo_1->SpellVisual == 210) ||
                    (spellInfo_1->SpellVisual == 246 && spellInfo_2->SpellVisual == 210))
                    return false;
            }
            ...
            if (spellInfo_1->Id == 71 && spellInfo_2->SpellIconID == 276 &&
                spellInfo_2->SpellVisual == 196)
                return false;

            // Bloodlust and Bloodthirst (multi-family check)
            if (spellInfo_2->Id == 2825 && spellInfo_1->SpellIconID == 38 &&
                spellInfo_1->SpellVisual == 0)
                return false;
        ...
        case SPELLFAMILY_PRIEST:
            if (spellInfo_2->SpellFamilyName == SPELLFAMILY_PRIEST)
            {
                ...
                if (((spellInfo_1->SpellFamilyFlags & UI64LIT(0x2000000)) &&
                     (spellInfo_2->SpellFamilyFlags & UI64LIT(0x800000000))) ||
                    ((spellInfo_2->SpellFamilyFlags & UI64LIT(0x2000000)) &&
                     (spellInfo_1->SpellFamilyFlags & UI64LIT(0x800000000))))
                    return false;

                // StarShards and Shadow Word: Pain
                if (((spellInfo_1->SpellFamilyFlags & UI64LIT(0x200000)) &&
                     (spellInfo_2->SpellFamilyFlags & UI64LIT(0x8000))) ||
                    ((spellInfo_2->SpellFamilyFlags & UI64LIT(0x200000)) &&
                     (spellInfo_1->SpellFamilyFlags & UI64LIT(0x8000))))
                    return false;
            }
            break;
        case SPELLFAMILY_DRUID:
            if (spellInfo_2->SpellFamilyName == SPELLFAMILY_DRUID)
            {
                ...
                if (((spellInfo_1->SpellFamilyFlags == UI64LIT(0x0) &&
                      spellInfo_1->SpellIconID == 108) &&
                     (spellInfo_2->SpellFamilyFlags & UI64LIT(0x20000000000000))) ||
                    ((spellInfo_2->SpellFamilyFlags == UI64LIT(0x0) &&
                      spellInfo_2->SpellIconID == 108) &&
                     (spellInfo_1->SpellFamilyFlags & UI64LIT(0x20000000000000))))
                    return false;
            }

            // Leader of the Pack and Scroll of Stamina (multi-family check)
            if (spellInfo_1->Id == 24932 && spellInfo_2->SpellIconID == 312 &&
                spellInfo_2->SpellVisual == 216)
                return false;
        ...
        case SPELLFAMILY_HUNTER:
            if (spellInfo_2->SpellFamilyName == SPELLFAMILY_HUNTER)
            {
                ...
                if (((spellInfo_1->SpellFamilyFlags & UI64LIT(0x20)) &&
                     (spellInfo_2->SpellFamilyFlags & UI64LIT(0x20000000000))) ||
                    ((spellInfo_2->SpellFamilyFlags & UI64LIT(0x20)) &&
                     (spellInfo_1->SpellFamilyFlags & UI64LIT(0x20000000000))))
                    return false;

                // Serpent Sting & (Immolation/Explosive Trap Effect)
                if (((spellInfo_1->SpellFamilyFlags & UI64LIT(0x4)) &&
                     (spellInfo_2->SpellFamilyFlags & UI64LIT(0x00000004000))) ||
                    ((spellInfo_2->SpellFamilyFlags & UI64LIT(0x4)) &&
                     (spellInfo_1->SpellFamilyFlags & UI64LIT(0x00000004000))))
                    return false;
        ...
        case SPELLFAMILY_PALADIN:
            ...
            // Combustion and Fire Protection Aura (multi-family check)
            if (spellInfo_2->Id == 11129 && spellInfo_1->SpellIconID == 33 &&
                spellInfo_1->SpellVisual == 321)
                return false;

            // *Sanctity Aura -> Unstable Currents and other (multi-family check)
            if (spellInfo_1->SpellIconID == 502 &&
                spellInfo_2->SpellFamilyName == SPELLFAMILY_GENERIC &&
                spellInfo_2->SpellIconID == 502 && spellInfo_2->SpellVisual == 969)
                return false;

            // *Seal of Command and Band of Eternal Champion (multi-family check)
            if (spellInfo_1->SpellIconID == 561 && spellInfo_1->SpellVisual == 7992 &&
                spellId_2 == 35081)
                return false;
            break;
        case SPELLFAMILY_SHAMAN:
            ...
            // Bloodlust and Bloodthirst (multi-family check)
            if (spellInfo_1->Id == 2825 && spellInfo_2->SpellIconID == 38 &&
                spellInfo_2->SpellVisual == 0)
                return false;
            break;
        default: break;
    ...
    // more generic checks
    if (spellInfo_1->SpellIconID == spellInfo_2->SpellIconID && spellInfo_1->SpellIconID != 0 &&
        spellInfo_1->SpellIconID != 1 && // SpellIconID 1 se stack avec tout ...
        ...
        IsPositiveSpell(spellInfo_1) ==
            IsPositiveSpell(spellInfo_2) // Jugement n'ecrase pas le sceau en dudu pala-pala
    )
    {
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        ...
        if (spellInfo_1->Effect[ i ] && spellInfo_1->Effect[ i ] != SPELL_EFFECT_DUMMY &&
...
SpellEntry const* SpellMgr::SelectAuraRankForLevel(SpellEntry const* spellInfo, uint32 level) const
{
    ...
    for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
    {
        // for simple aura in check apply to any non caster based targets, in rank search mode to
...
static void LoadSpellChains_AbilityHelper(SpellChainMap& chainMap,
                                          AbilitySpellPrevMap const& prevRanks,
                                          uint32 spell_id,
                                          uint32 prev_id,
                                          uint32 deep = 30)
{
    ...
    {
        MANGOS_ASSERT(
            chain_itr->second.prev == prev_id &&
            "LoadSpellChains_AbilityHelper: Conflicting data in talents or spell abilities dbc");
        return;
    }
    ...
    {
        MANGOS_ASSERT(false &&
                      "LoadSpellChains_AbilityHelper: Infinity cycle in spell ability data");
        return;
    }
...
void SpellMgr::LoadSpellChains()
{
    ...
    for (unsigned int i = 0; i < sTalentStore.GetNumRows(); ++i)
    {
        ...
        // not add ranks for 1 ranks talents (if exist non ranks spells then it will included in
        // table data)
    ...
    // load known data from spell abilities
    {
        ...
        for (SkillLineAbilityMap::const_iterator ab_itr = mSkillLineAbilityMap.begin();
             ab_itr != mSkillLineAbilityMap.end();
             ++ab_itr)
        {
            ...
            // some forward spells still exist but excluded from real use as ranks and not listed in
            // skill abilities now
            ...
            if (chain_itr != mSpellChains.end())
            {
                MANGOS_ASSERT(chain_itr->second.prev == spell_id &&
                              "Conflicting data in talents or spell abilities dbc");
                continue;
            }
    ...
    do
    {
        ...
        if (chain_itr != mSpellChains.end())
        {
            ...
            {
                sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                "`spell_chain` expected rank %u by DBC data.",
                                spell_id,
                                node.prev,
                                node.first,
                                node.rank,
                                node.req,
                                chain_itr->second.rank);
                continue;
            }
            ...
            {
                sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                "`spell_chain` expected prev %u by DBC data.",
                                spell_id,
                                node.prev,
                                node.first,
                                node.rank,
                                node.req,
                                chain_itr->second.prev);
                continue;
            }
            ...
            {
                sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                "`spell_chain` expected first %u by DBC data.",
                                spell_id,
                                node.prev,
                                node.first,
                                node.rank,
                                node.req,
                                chain_itr->second.first);
                continue;
            }
            ...
            sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) already added "
                            "(talent or spell ability with forward) and non need in `spell_chain`",
                            spell_id,
                            node.prev,
                            node.first,
                            node.rank,
        ...
        {
            sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                            "`spell_chain` has nonexistent previous rank spell.",
                            spell_id,
                            node.prev,
                            node.first,
                            node.rank,
        ...
        {
            sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                            "`spell_chain` has not existing first rank spell.",
                            spell_id,
                            node.prev,
                            node.first,
                            node.rank,
        ...
        // check basic spell chain data integrity (note: rank can be equal 0 or 1 for first/single
        // spell)
        if ((spell_id == node.first) != (node.rank <= 1) ||
            (spell_id == node.first) != (node.prev == 0) || (node.rank <= 1) != (node.prev == 0))
        {
            sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                            "`spell_chain` has not compatible chain data.",
                            spell_id,
                            node.prev,
                            node.first,
                            node.rank,
        ...
        {
            sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                            "`spell_chain` has not existing required spell.",
                            spell_id,
                            node.prev,
                            node.first,
                            node.rank,
        ...
        // talents not required data in spell chain for work, but must be checked if present for
        // integrity
        if (TalentSpellPos const* pos = GetTalentSpellPos(spell_id))
        {
            if (node.rank != pos->rank + 1)
            {
                sLog.outErrorDb("Talent %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                "`spell_chain` has wrong rank.",
                                spell_id,
                                node.prev,
                                node.first,
                                node.rank,
            ...
            if (TalentEntry const* talentEntry = sTalentStore.LookupEntry(pos->talent_id))
            {
                ...
                {
                    sLog.outErrorDb("Talent %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                    "`spell_chain` has wrong first rank spell.",
                                    spell_id,
                                    node.prev,
                                    node.first,
                                    node.rank,
                ...
                {
                    sLog.outErrorDb("Talent %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                    "`spell_chain` has wrong prev rank spell.",
                                    spell_id,
                                    node.prev,
                                    node.first,
                                    node.rank,
    ...
    for (SpellChainMap::const_iterator i = mSpellChains.begin(); i != mSpellChains.end(); ++i)
    {
        if (i->second.prev)
        {
            ...
            {
                sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                "`spell_chain` has different first spell in chain compared to "
                                "previous rank spell (prev: %u, first: %u, rank: %d, req: %u).",
                                i->first,
                                i->second.prev,
                                i->second.first,
                                i->second.rank,
            ...
            {
                sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                "`spell_chain` has different rank compared to previous rank spell "
                                "(prev: %u, first: %u, rank: %d, req: %u).",
                                i->first,
                                i->second.prev,
                                i->second.first,
                                i->second.rank,
        ...
        if (i->second.req)
        {
            ...
            {
                sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                "`spell_chain` has not found required rank spell in table.",
                                i->first,
                                i->second.prev,
                                i->second.first,
                                i->second.rank,
            ...
            {
                sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                "`spell_chain` has required rank spell from same spell chain "
                                "(prev: %u, first: %u, rank: %d, req: %u).",
                                i->first,
                                i->second.prev,
                                i->second.first,
            ...
            {
                sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                                "`spell_chain` has required rank spell with required spell (prev: "
                                "%u, first: %u, rank: %d, req: %u).",
                                i->first,
                                i->second.prev,
                                i->second.first,
                                i->second.rank,
    ...
    // check single rank redundant cases (single rank talents not added by default so this can be
    // only custom cases)
    for (SpellChainMap::const_iterator i = mSpellChains.begin(); i != mSpellChains.end(); ++i)
    {
        ...
        {
            sLog.outErrorDb("Spell %u (prev: %u, first: %u, rank: %d, req: %u) listed in "
                            "`spell_chain` has single rank data, so redundant.",
                            i->first,
                            i->second.prev,
                            i->second.first,
                            i->second.rank,
    ...
    sLog.outString();
    sLog.outString(">> Loaded %u spell chain records (%u from DBC data with %u req field updates, "
                   "and %u loaded from table)",
                   dbc_count + new_count,
                   dbc_count,
                   req_count,
                   new_count);
...
void SpellMgr::LoadSpellLearnSpells()
{
    ...
    //                                                0      1        2
    auto result = WorldDatabase.Query("SELECT entry, SpellID, Active FROM spell_learn_spell");
    if (!result)
    {
        BarGoLink bar(1);
        bar.step();
    ...
    do
    {
        ...
        {
            sLog.outErrorDb("Spell %u listed in `spell_learn_spell` learning nonexistent spell %u",
                            spell_id,
                            node.spell);
            continue;
        }
        ...
        {
            sLog.outErrorDb(
                "Spell %u listed in `spell_learn_spell` attempt learning talent spell %u, skipped",
                spell_id,
                node.spell);
            continue;
        }
    ...
    for (uint32 spell = 0; spell < sSpellStore.GetNumRows(); ++spell)
    {
        ...
        for (int i = 0; i < MAX_EFFECT_INDEX; ++i)
        {
            if (entry->Effect[ i ] == SPELL_EFFECT_LEARN_SPELL)
            {
                ...
                dbc_node.active = true; // all dbc based learned spells is active (show in spell
                                        // book or hide by client itself)
                ...
                // talent or passive spells or skill-step spells auto-casted and not need dependent
                // learning, pet teaching spells don't must be dependent learning (casted) other
                // required explicit dependent learning
                dbc_node.autoLearned = entry->EffectImplicitTargetA[ i ] == TARGET_PET ||
                ...
                bool found = false;
                for (SpellLearnSpellMap::const_iterator itr = db_node_bounds.first;
                     itr != db_node_bounds.second;
                     ++itr)
                {
                    if (itr->second.spell == dbc_node.spell)
                    {
                        sLog.outErrorDb("Spell %u auto-learn spell %u in spell.dbc then the record "
                                        "in `spell_learn_spell` is redundant, please fix DB.",
                                        spell,
                                        dbc_node.spell);
                        found = true;
                        break;
...
void SpellMgr::LoadSpellScriptTarget()
{
    ...
    auto result = WorldDatabase.Query("SELECT entry,type,targetEntry FROM spell_script_target");
    ...
    do
    {
        ...
        {
            sLog.outErrorDb(
                "Table `spell_script_target`: spellId %u listed for TargetEntry %u does not exist.",
                spellId,
                targetEntry);
            continue;
        }
        ...
        if (!targetfound)
        {
            sLog.outErrorDb("Table `spell_script_target`: spellId %u listed for TargetEntry %u "
                            "does not have any implicit target TARGET_SCRIPT(38) or "
                            "TARGET_SCRIPT_COORDINATES (46) or TARGET_FOCUS_OR_SCRIPTED_GAMEOBJECT "
                            "(40).",
                            spellId,
                            targetEntry);
            continue;
        ...
        {
            sLog.outErrorDb(
                "Table `spell_script_target`: target type %u for TargetEntry %u is incorrect.",
                type,
                targetEntry);
            continue;
        }
        ...
        switch (type)
        {
            case SPELL_TARGET_TYPE_GAMEOBJECT:
            {
                ...
                {
                    sLog.outErrorDb(
                        "Table `spell_script_target`: gameobject template entry %u does not exist.",
                        targetEntry);
                    continue;
                }
                break;
            ...
            default:
                if (!targetEntry)
                {
                    sLog.outErrorDb("Table `spell_script_target`: target entry == 0 for not GO "
                                    "target type (%u).",
                                    type);
                    continue;
                }
                        ...
                        sCreatureStorage.LookupEntry<CreatureInfo>(targetEntry))
                {
                    ...
                    {
                        sLog.outErrorDb("Table `spell_script_target` has creature %u as a target "
                                        "of spellid 30427, but this creature has no skinlootid. "
                                        "Gas extraction will not work!",
                                        cInfo->Entry);
                        continue;
                    }
                }
                ...
                {
                    sLog.outErrorDb(
                        "Table `spell_script_target`: creature template entry %u does not exist.",
                        targetEntry);
                    continue;
                }
                break;
    ...
    spellInfo->EffectImplicitTargetA[j] != TARGET_SELF && spellInfo->EffectImplicitTargetB[j] ==
    TARGET_SCRIPT )
            {
                ...
                if(lower==upper)
                {
                    sLog.outErrorDb("Spell (ID: %u) has effect
    EffectImplicitTargetA/EffectImplicitTargetB = %u (TARGET_SCRIPT), but does not have record in
    `spell_script_target`",spellInfo->Id,TARGET_SCRIPT); break;                                  //
    effects of spell
                }
            }
        }
...
void SpellMgr::LoadSpellPetAuras()
{
    ...
    //                                                0      1    2
    auto result = WorldDatabase.Query("SELECT spell, pet, aura FROM spell_pet_auras");
    if (!result)
    {
    ...
    do
    {
        ...
        else
        {
            ...
            {
                sLog.outError(
                    "Spell %u listed in `spell_pet_auras` does not have dummy aura or dummy effect",
                    spell);
                continue;
            }
        ...
        switch (spellInfo->Effect[ i ])
        {
            ...
            case SPELL_EFFECT_CREATE_ITEM:
            {
                if (!ObjectMgr::GetItemPrototype(spellInfo->EffectItemType[ i ]))
                {
                    ...
                    {
                        if (pl)
                            ChatHandler(pl).PSendSysMessage("Craft spell %u create item (Entry: "
                                                            "%u) but item does not exist in "
                                                            "item_template.",
                                                            spellInfo->Id,
                                                            spellInfo->EffectItemType[ i ]);
                        else
                            sLog.outErrorDb("Craft spell %u create item (Entry: %u) but item does "
            ...
            case SPELL_EFFECT_LEARN_SPELL:
            {
                SpellEntry const* spellInfo2 =
                    sSpellMgr.GetSpellEntry(spellInfo->EffectTriggerSpell[ i ]);
                if (!IsSpellValid(spellInfo2, pl, msg))
                {
                    if (msg)
                    {
                        if (pl)
                            ...
                            sLog.outErrorDb("Spell %u learn to invalid spell %u, and then...",
                                            spellInfo->Id,
                                            spellInfo->EffectTriggerSpell[ i ]);
                    }
                    return false;
    ...
    if (need_check_reagents)
    {
        for (int j = 0; j < MAX_SPELL_REAGENTS; ++j)
        {
                ...
                !ObjectMgr::GetItemPrototype(spellInfo->Reagent[ j ]))
            {
                ...
                {
                    if (pl)
                        ChatHandler(pl).PSendSysMessage("Craft spell %u requires reagent item "
                                                        "(Entry: %u) but item does not exist in "
                                                        "item_template.",
                                                        spellInfo->Id,
                                                        spellInfo->Reagent[ j ]);
                    else
                        sLog.outErrorDb("Craft spell %u requires reagent item (Entry: %u) but item "
...
void SpellMgr::LoadSpellAreas()
{
    ...
    auto result =
        WorldDatabase.Query("SELECT spell, area, quest_start, quest_start_active, quest_end, "
    ...
    do
    {
...

        {
            ...
            {
                sLog.outErrorDb(
                    "Spell %u listed in `spell_area` already listed with similar requirements.",
                    spell);
                continue;
            }
        }
        ...
        {
            sLog.outErrorDb("Spell %u listed in `spell_area` have wrong area (%u) requirement",
                            spell,
                            spellArea.areaId);
            continue;
        }
        ...
        {
            sLog.outErrorDb(
                "Spell %u listed in `spell_area` have wrong start quest (%u) requirement",
                spell,
        ...
        if (spellArea.questEnd)
        {
            ...
            {
                sLog.outErrorDb(
                    "Spell %u listed in `spell_area` have wrong end quest (%u) requirement",
                    spell,
                    spellArea.questEnd);
                continue;
            }
            ...
            {
                sLog.outErrorDb("Spell %u listed in `spell_area` have quest (%u) requirement for "
                                "start and end in same time",
                                spell,
                                spellArea.questEnd);
                continue;
            }
        ...
        if (spellArea.auraSpell)
        {
            ...
            {
                sLog.outErrorDb(
                    "Spell %u listed in `spell_area` have wrong aura spell (%u) requirement",
                    spell,
                    abs(spellArea.auraSpell));
                continue;
            }
            ...
            {
                case SPELL_AURA_DUMMY:
                case SPELL_AURA_GHOST: break;
                default:
                    sLog.outErrorDb("Spell %u listed in `spell_area` have aura spell requirement "
                                    "(%u) without dummy/phase/ghost aura in effect 0",
                                    spell,
                                    abs(spellArea.auraSpell));
                    continue;
            }
            ...
            {
                sLog.outErrorDb(
                    "Spell %u listed in `spell_area` have aura spell (%u) requirement for itself",
                    spell,
                    abs(spellArea.auraSpell));
                continue;
            }

            // not allow autocast chains by auraSpell field (but allow use as alternative if not
            // present)
            if (spellArea.autocast && spellArea.auraSpell > 0)
            {
                bool chain                        = false;
                SpellAreaForAuraMapBounds saBound = GetSpellAreaForAuraMapBounds(spellArea.spellId);
                for (SpellAreaForAuraMap::const_iterator itr = saBound.first; itr != saBound.second;
                     ++itr)
                {
                ...
                if (chain)
                {
                    sLog.outErrorDb("Spell %u listed in `spell_area` have aura spell (%u) "
                ...
                for (SpellAreaMap::const_iterator itr2 = saBound2.first; itr2 != saBound2.second;
                     ++itr2)
                {
        ...
        {
            sLog.outErrorDb("Spell %u listed in `spell_area` have wrong race mask (%u) requirement",
                            spell,
                            spellArea.raceMask);
            continue;
        }

        if (spellArea.gender != GENDER_NONE && spellArea.gender != GENDER_FEMALE &&
            spellArea.gender != GENDER_MALE)
        {
            sLog.outErrorDb("Spell %u listed in `spell_area` have wrong gender (%u) requirement",
                            spell,
                            spellArea.gender);
            continue;
        }
        ...
        SpellArea const* sa =
            &mSpellAreaMap.insert(SpellAreaMap::value_type(spell, spellArea))->second;
...
SpellCastResult SpellMgr::GetSpellAllowedInLocationError(SpellEntry const* spellInfo,
                                                         Unit const* caster,
                                                         Player const* player)
{
    // bg spell checks
    ...
    switch (spellInfo->Id)
    {
        ...
        case 23539:
        {
            ...
            return player->GetMapId() == 30 && bg && bg->GetStatus() != STATUS_WAIT_JOIN ?
                       SPELL_CAST_OK :
                       SPELL_FAILED_REQUIRES_AREA;
        }
        case 23333: // Warsong Flag
        case 23335: // Silverwing Flag
            return player && player->GetMapId() == 489 && player->InBattleGround() ?
                       SPELL_CAST_OK :
                       SPELL_FAILED_REQUIRES_AREA;
        case 2584: // Waiting to Resurrect
        {
            return player && player->InBattleGround() ? SPELL_CAST_OK :
                                                         ...
                                                         Player const* player)
{
    ...
    return GetSpellAllowedInLocationError(
        spellInfo, nullptr /* zone/area already checked */, player);
}
...
void SpellMgr::LoadSkillRaceClassInfoMap()
{
    ...
    for (uint32 i = 0; i < sSkillRaceClassInfoStore.GetNumRows(); ++i)
    {
        ...
        mSkillRaceClassInfoMap.insert(
            SkillRaceClassInfoMap::value_type(skillRCInfo->skillId, skillRCInfo));
...
void SpellMgr::CheckUsedSpells(char const* table)
{
    ...
    do
    {
        ...
        {
            sLog.outError("Table '%s' for spell %u have wrong SpellFamily value(%u), skipped.",
                          table,
                          spell,
                          family);
            continue;
        ...
        if (spellIcon < -1)
        {
            sLog.outError("Table '%s' for spell %u have wrong SpellIcon value(%u), skipped.",
                          table,
                          spell,
                          spellIcon);
            continue;
        ...
        if (spellVisual < -1)
        {
            sLog.outError("Table '%s' for spell %u have wrong SpellVisual value(%u), skipped.",
                          table,
                          spell,
                          spellVisual);
            continue;
        ...
        if (category < -1 ||
            (category >= 0 && sSpellCategoryStore.find(category) == sSpellCategoryStore.end()))
        {
            sLog.outError("Table '%s' for spell %u have wrong SpellCategory value(%u), skipped.",
                          table,
                          spell,
                          category);
            continue;
        ...
        {
            sLog.outError("Table '%s' for spell %u have wrong SpellEffect type value(%u), skipped.",
                          table,
                          spell,
                          effectType);
            continue;
        ...
        {
            sLog.outError("Table '%s' for spell %u have wrong SpellAura type value(%u), skipped.",
                          table,
                          spell,
                          auraType);
            continue;
        ...
        {
            sLog.outError("Table '%s' for spell %u have wrong EffectIdx value(%u), skipped.",
                          table,
                          spell,
                          effectIdx);
            continue;
        ...
        if (spell)
        {
            ...
            {
                sLog.outError(
                    "Spell %u '%s' not exist but used in %s.", spell, name.c_str(), code.c_str());
                continue;
            }

            if (family >= 0 && spellEntry->SpellFamilyName != uint32(family))
            {
                sLog.outError("Spell %u '%s' family(%u) <> %u but used in %s.",
                              spell,
                              name.c_str(),
            ...
            {
                sLog.outError("Spell %u '%s' icon(%u) <> %u but used in %s.",
                              spell,
                              name.c_str(),
            ...
            {
                sLog.outError("Spell %u '%s' visual(%u) <> %u but used in %s.",
                              spell,
                              name.c_str(),
            ...
            {
                sLog.outError("Spell %u '%s' category(%u) <> %u but used in %s.",
                              spell,
                              name.c_str(),
                              spellEntry->Category,
                              category,
            ...
            if (effectIdx >= EFFECT_INDEX_0)
            {
                ...
                {
                    sLog.outError("Spell %u '%s' effect%d <> %u but used in %s.",
                                  spell,
                                  name.c_str(),
                                  effectIdx + 1,
                                  effectType,
                ...
                if (auraType >= 0 &&
                    spellEntry->EffectApplyAuraName[ effectIdx ] != uint32(auraType))
                {
                    sLog.outError("Spell %u '%s' aura%d <> %u but used in %s.",
                                  spell,
                                  name.c_str(),
                                  effectIdx + 1,
                                  auraType,
            ...
            else
            {
                ...
                {
                    sLog.outError("Spell %u '%s' not have effect %u but used in %s.",
                                  spell,
                                  name.c_str(),
                                  effectType,
                                  code.c_str());
                ...
                {
                    sLog.outError("Spell %u '%s' not have aura %u but used in %s.",
                                  spell,
                                  name.c_str(),
                                  auraType,
                                  code.c_str());
...
bool SpellArea::IsFitToRequirements(Player const* player, uint32 newZone, uint32 newArea) const
{
    ...
    if (questStart)
    {
        ...
        if (!player || (!questStartCanActive || !player->IsActiveQuest(questStart)) &&
...
void SpellMgr::LoadSpellAffects()
{
    ...
    //                                                0      1         2
    auto result = WorldDatabase.Query("SELECT entry, effectId, SpellFamilyMask FROM spell_affect");
    if (!result)
    {
    ...
    do
    {
        ...
        {
            sLog.outErrorDb("Spell %u listed in `spell_affect` have invalid effect index (%u)",
                            entry,
                            effectId);
            continue;
        }
        ...
        {
            sLog.outErrorDb("Spell %u listed in `spell_affect` have not "
                            "SPELL_AURA_ADD_FLAT_MODIFIER (%u) or SPELL_AURA_ADD_PCT_MODIFIER (%u) "
                            "or SPELL_AURA_ADD_TARGET_TRIGGER (%u) or "
        ...
        if (spellInfo->EffectItemType[ effectId ])
        {
            ...
            {
                sLog.outErrorDb("Spell %u listed in `spell_affect` have redundant (same with "
                                "EffectItemType%d) data for effect index (%u) and not needed, "
                                "skipped.",
                                entry,
                                effectId + 1,
                                effectId);
        ...
        mSpellAffectMap.insert(
            SpellAffectMap::value_type((entry << 8) + effectId, spellAffectMask));
    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); ++id)
    {
        ...
        for (int effectId = 0; effectId < MAX_EFFECT_INDEX; ++effectId)
        {
            ...
            sLog.outErrorDb("Spell %u (%s) misses spell_affect for effect %u",
                            id,
                            spellInfo->SpellName[ sWorld.GetDefaultDbcLocale() ],
                            effectId);
        }
    }
}
...
void SpellMgr::LoadFacingCasterFlags()
{
    ...
    //                                                0              1
    auto result = WorldDatabase.Query("SELECT entry, facingcasterflag FROM spell_facing");
    if (!result)
    {
        BarGoLink bar(1);
        bar.step();
...
void SpellMgr::LoadSpells()
{
    ...
    sLog.outString("%u spells loaded in %ums.",
                   mSpellEntryMap.size(),
                   WorldTimer::getMSTimeDiffToNow(oldMSTime));
}
