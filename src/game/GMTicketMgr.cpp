...
bool GmTicket::LoadFromDB(Field* fields)
{
    ...
    // ticketId, guid, name, message, createTime, mapId, posX, posY, posZ, lastModifiedTime,
    // closedBy, assignedTo, comment, response, completed, escalated, viewed, haveTicket,
    // ticketType, securityNeeded
    uint8 index       = 0;
...
void GmTicket::SaveToDB() const
{
    ...
    SqlStatement stmt = CharacterDatabase.CreateStatement(
        gmTicketSave,
        "REPLACE INTO gm_tickets (ticketId, guid, name, message, createTime, mapId, posX, posY, "
        "posZ, lastModifiedTime, closedBy, assignedTo, comment, response, completed, escalated, "
        "viewed, haveTicket, ticketType, securityNeeded) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, "
        "?, ?, ?, ?, ?, ?, ?, ?, ?)");
    stmt.addUInt32(_id);
...
void GmTicket::DeleteFromDB()
{
    ...
    SqlStatement stmt =
        CharacterDatabase.CreateStatement(delTicket, "DELETE FROM gm_tickets WHERE ticketId = ?");
    stmt.addUInt32(_id);
    stmt.Execute();
}
...
void GmTicket::WritePacket(WorldPacket& data) const
{
    ...
    GMTicketOpenedByGMStatus openedStatus =
        _viewed ? GMTICKET_OPENEDBYGM_STATUS_OPENED : GMTICKET_OPENEDBYGM_STATUS_NOT_OPENED;
...
std::string GmTicket::FormatMessageString(ChatHandler& handler, bool detailed) const
{
    ...
    ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTNAME,
                                  handler.playerLink(_playerName).c_str());
    ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTAGECREATE,
                                  (secsToTimeString(curTime - _createTime, true, false)).c_str());
    if (_lastModifiedTime != _createTime)
        ss << handler.PGetParseString(
            LANG_COMMAND_TICKETLISTAGE,
            (secsToTimeString(curTime - _lastModifiedTime, true, false)).c_str());

    std::string name;
    if (sObjectMgr.GetPlayerNameByGUID(_assignedTo, name))
        ss << handler.PGetParseString(LANG_COMMAND_TICKETLISTASSIGNEDTO,
                                      handler.playerLink(name).c_str());
...
{
    _needResponse =
        (needResponse == 17);     // Requires GM response. 17 = true, 1 = false (17 is default)
    _needMoreHelp = needMoreHelp; // Requests further GM interaction on a ticket to which a GM has
                                  // already responded. Basically means "has a new ticket"
}
...
void TicketMgr::LoadTickets()
{
    ...
    std::shared_ptr<QueryResult> result =
        CharacterDatabase.Query("SELECT " TICKET_TABLE_FIELDS " FROM gm_tickets");
    if (!result)
    {
    ...
    sLog.outString(
        ">> Loaded %u GM tickets in %u ms", count, WorldTimer::getMSTimeDiffToNow(oldMSTime));
}
...
void TicketMgr::LoadSurveys()
{
    ...
    uint32 oldMSTime = WorldTimer::getMSTime();
    if (std::shared_ptr<QueryResult> result =
            CharacterDatabase.Query("SELECT MAX(surveyId) FROM gm_surveys"))
    {
...
void TicketMgr::ShowEscalatedList(ChatHandler& handler) const
{
    ...
    for (GmTicketList::const_iterator itr = _ticketList.begin(); itr != _ticketList.end(); ++itr)
        if (!itr->second->IsClosed() &&
            ...
            handler.PSendSysMessage(LANG_COMMAND_TICKETESCALATED_TICKET,
                                    itr->second->FormatMessageString(handler).c_str(),
                                    itr->second->GetNeededSecurityLevel());
}
...
void TicketMgr::ReloadTicketCallback(const std::shared_ptr<QueryResult>& holder)
{
    if (!holder)
        return;
    ...
    if (!currentTicket)
    {
        sWorld.SendGMTicketText(
            LANG_COMMAND_TICKETNEW, newTicket->GetPlayerName().c_str(), newTicket->GetId());
        AddTicket(newTicket);
    }
    else // Existing ticket updated.
    {
        ...
        if (!assignedTo.empty() || !closedBy.empty() || !completedBy.empty())
        {
            ...
            std::string msg =
                newTicket->FormatMessageString(handler,
                                               closedBy.empty() ? NULL : closedBy.c_str(),
                                               assignedTo.empty() ? NULL : assignedTo.c_str(),
                                               NULL,
                                               NULL,
                                               completedBy.empty() ? NULL : completedBy.c_str());
...
void TicketMgr::ReloadTicket(uint32 ticketId)
{
    ...
    CharacterDatabase.AsyncPQueryUnsafe(
        {"SELECT " TICKET_TABLE_FIELDS " FROM gm_tickets WHERE ticketId = '%u'", ticketId},
        this,
        &TicketMgr::ReloadTicketCallback);
}
