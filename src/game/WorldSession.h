...
{
public:
    explicit PacketFilter(WorldSession* pSession)
        : m_pSession(pSession)
        , m_processLogout(false)
        , m_processType(PACKET_PROCESS_MAX_TYPE)
    {
    }
...
{
public:
    explicit MapSessionFilter(WorldSession* pSession)
...
#define ALL_SESSION_SCRIPTS(session, what)                                                        \
    for (SessionScriptsMap::iterator it = session->scripts.begin(); it != session->scripts.end(); \
         ++it)                                                                                    \
        it->second->what;
...
public:
    ...
    void SendAuctionCommandResult(AuctionEntry* auc,
                                  AuctionAction Action,
                                  AuctionError ErrorCode,
                                  InventoryResult invError = EQUIP_ERR_OK);
    ...
    // Item Enchantment
    void
    SendEnchantmentLog(ObjectGuid targetGuid, ObjectGuid casterGuid, uint32 itemId, uint32 spellId);
    ...
    void BuildPartyMemberStatsPacket(Player* player,
                                     WorldPacket* data,
                                     uint32 updateMask,
                                     bool sendAllAuras);
    ...
    void SetDisconnectedSession(); // Remove from World::m_session. Used when an account gets
                                   // disconnected.
    ...
    void ProcessAnticheatAction(const char* detector,
                                const char* reason,
                                uint32 action,
                                uint32 banTime = 0 /* Perm ban */);
...
public:                                        // opcodes handlers
    ...
    void Handle_EarlyProccess(
        WorldPacket& recvPacket); // just mark packets processed in WorldSocket::OnRead
    void
    Handle_ServerSide(WorldPacket& recvPacket); // sever side only, can't be accepted from client
    ...
    void HandleCharEnum(const std::shared_ptr<QueryResult>& result);
    ...
    static void HandleChangePlayerNameOpcodeCallBack(const std::shared_ptr<QueryResult>& result,
                                                     uint32 accountId,
                                                     std::string newname);
