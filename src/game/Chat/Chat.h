...
public:
    ...
    static void FillMessageData(WorldPacket* data,
...
protected:
    ...
    bool SetDataForCommandInTable(ChatCommand* table,
                                  const char* text,
                                  uint32 security,
    ...
    ChatCommandSearchResult FindCommand(ChatCommand* table,
                                        char const*& text,
                                        ChatCommand*& command,
                                        ChatCommand** parentCommand = nullptr,
                                        std::string* cmdNamePtr     = nullptr,
    ...
    /run if not f then f=ChatFrame_OnEvent ChatFrame_OnEvent=function(e)f(e)if p==0 and
    e=='CHAT_MSG_SYSTEM'then n=gsub(arg1,'%D*(%d*)%s.*','%1')end end end n=n or 0
    SendChatMessage('.debug send chanvisualnext '..(p==1 and n or -1))p=p==1 and 0 or 1
    */
    ...
    // extraction different type params from args string, all functions update (char** args) to
    // first unparsed tail symbol at return
    ...
    char* ExtractLinkArg(char** args,
                         char const* const* linkTypes = nullptr,
                         int* foundIdx                = nullptr,
                         char** keyPair               = nullptr,
                         char** somethingPair         = nullptr);
    ...
    char* ExtractOptNotLastArg(char** args); // extract name/number/quote/shift-link arg only if
                                             // more data in args for parse
    ...
    uint32 ExtractAccountId(char** args,
                            std::string* accountName = nullptr,
                            Player** targetIfNullArg = nullptr);
    ...
    bool ShowAccountListHelper(const std::shared_ptr<QueryResult>& result,
                               uint32* limit = nullptr,
                               bool title    = true,
                               bool error    = true);
    void ShowFactionListHelper(FactionEntry const* factionEntry,
                               LocaleConstant loc,
    ...
    bool ShowPlayerListHelper(const std::shared_ptr<QueryResult>& result,
                              uint32* limit = nullptr,
                              bool title    = true,
                              bool error    = true);
    ...
    bool LookupPlayerSearchCommand(const std::shared_ptr<QueryResult>& result,
                                   uint32* limit = nullptr);
    bool HandleBanListHelper(const std::shared_ptr<QueryResult>& result);
    bool HandleBanHelper(BanMode mode, char* args);
    bool HandleBanInfoHelper(uint32 accountid, char const* accountname);
    bool HandleUnBanHelper(BanMode mode, char* args);
    void
    HandleCharacterLevel(Player* player, ObjectGuid player_guid, uint32 oldlevel, uint32 newlevel);
    void HandleLearnSkillRecipesHelper(Player* player, uint32 skill_id);
    bool HandleGoHelper(Player* _player,
                        uint32 mapid,
                        float x,
                        float y,
                        float const* zPtr   = nullptr,
                        float const* ortPtr = nullptr);
    ...
    std::string
    GenerateDeletedCharacterGUIDsWhereStr(DeletedInfoList::const_iterator& itr,
...
public:
    ...
    explicit CliHandler(uint32 accountId,
                        AccountTypes accessLevel,
                        void* callbackArg,
                        Print* zprint)
        : m_accountId(accountId)
        , m_loginAccessLevel(accessLevel)
