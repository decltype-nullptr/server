...
void MapManager::UpdateGridState(grid_state_t state,
                                 Map& map,
                                 NGridType& ngrid,
                                 GridInfo& ginfo,
                                 const uint32& x,
...
{
    MANGOS_ASSERT(obj);
    // if(!obj->IsInWorld()) sLog.outError("GetMap: called for map %d with object (typeid %d, guid
    // %d, mapid %d, instanceid %d) who is not in world!", id, obj->GetTypeId(), obj->GetGUIDLow(),
    ...
    if (entry->IsDungeon())
    {
        if (entry->IsRaid())
        {
            ...
            if (!player->isGameMaster() && !sWorld.getConfig(CONFIG_BOOL_INSTANCE_IGNORE_RAID))
            {
                ...
                {
                    // probably there must be special opcode, because client has this string
                    // constant in GlobalStrings.lua
                    // TODO: this is not a good place to send the message
                    player->GetSession()->SendAreaTriggerMessage(
                        "You must be in a raid group to enter %s instance", mapName);
                    DEBUG_LOG("MAP: Player '%s' must be in a raid group to enter instance of '%s'",
                              player->GetName(),
                              mapName);
                    return false;
                }
        ...
        {
            DEBUG_LOG("MAP: Player '%s' can't enter instance %u on map %u. Has already entered too "
                      "many instances.",
                      player->GetName(),
                      instanceId,
                      mapid);
        ...
        {
            DEBUG_LOG("MAP: Player '%s' can't enter instance '%s' while an encounter is in
        progress.", player->GetName(), GetMapName()); player->SendTransferAborted(GetId(),
        TRANSFER_ABORT_ZONE_IN_COMBAT); return(false);
        }*/
    }
...
{
public:
    ContinentAsyncUpdater(uint32 updateDiff, Map* m)
        : diff(updateDiff)
        , map(m)
    {
    }
...
void MapManager::Update(uint32 diff)
{
    ...
    std::vector<MapAsyncUpdater*> instanceUpdaters(
        sWorld.getConfig(CONFIG_UINT32_MAPUPDATE_INSTANCED_UPDATE_THREADS));
    std::vector<ContinentAsyncUpdater*> continentsUpdaters;
    for (int i = 0; i < instanceUpdaters.size(); ++i)
        instanceUpdaters[ i ] =
            new MapAsyncUpdater(&updateFinished, mapsDiff); // Will be deleted at thread end
    ...
    for (MapMapType::iterator iter = i_maps.begin(); iter != i_maps.end(); ++iter)
    {
        ...
        {
            if (WorldTimer::getMSTimeDiff(iter->second->GetLastPlayerLeftTime(), now) >
                sWorld.getConfig(CONFIG_UINT32_EMPTY_MAPS_UPDATE_TIME))
                continue;
        }
    ...
    std::vector<ACE_Based::Thread*> asyncUpdateThreads(instanceUpdaters.size() +
    ...
    for (int tid = 0; tid < continentsUpdaters.size(); ++tid)
        asyncUpdateThreads[ tid + instanceUpdaters.size() ] =
            new ACE_Based::Thread(continentsUpdaters[ tid ]);
...
void MapManager::InitMaxInstanceId()
{
    ...
    auto result = CharacterDatabase.Query("SELECT MAX(id) FROM instance");
    if (result)
    {
...
///// in case of battlegrounds it will only return an existing map, those maps are created by
/// bg-system
...
DungeonMap* MapManager::CreateDungeonMap(uint32 id, uint32 InstanceId, DungeonPersistentState* save)
{
    ...
    DEBUG_LOG("MapInstanced::CreateInstanceMap: %s map instance %d for %d created",
              save ? "" : "new ",
              InstanceId,
              id);
...
{
    DEBUG_LOG("MapInstanced::CreateBattleGroundMap: instance:%d for map:%d and bgType:%d created.",
              InstanceId,
              id,
              bg->GetTypeID());
...
bool IsNorthTo(float x,
               float y,
               float const* limits,
               int count /* last case is limits[2*count - 1] */)
{
    int insideCount = 0;
    for (int i = 0; i < count - 1; ++i)
    {
        if ((limits[ 2 * i + 1 ] < y && y < limits[ 2 * i + 3 ]) ||
            (limits[ 2 * i + 1 ] > y && y > limits[ 2 * i + 3 ]))
        {
            float threshold = limits[ 2 * i ] + (limits[ 2 * i + 2 ] - limits[ 2 * i ]) *
                                                    (y - limits[ 2 * i + 1 ]) /
                                                    (limits[ 2 * i + 3 ] - limits[ 2 * i + 1 ]);
            if (x > threshold)
                ++insideCount;
        }
    }
    ...
    switch (mapId)
    {
        case 0:
        {
            const static float topNorthSouthLimit[] = {
                2032.048340f,  -6927.750000f, 1634.863403f,  -6157.505371f, 1109.519775f,
                -5181.036133f, 1315.204712f,  -4096.020508f, 1073.089233f,  -3372.571533f,
                825.833191f,   -3125.778809f, 657.343994f,   -2314.813232f, 424.736145f,
                -1888.283691f, 744.395813f,   -1647.935425f, 1424.160645f,  -654.948181f,
                1447.065308f,  -169.751358f,  1208.715454f,  189.748703f,   1596.240356f,
                998.616699f,   1577.923706f,  1293.419922f,  1458.520264f,  1727.373291f,
                1591.916138f,  3728.139404f};
            const static float ironforgeAreaSouthLimit[] = {
                -7491.33f, 3093.74f,  -7472.04f, -391.88f,  -6366.68f, -730.10f,
                -6063.96f, -1411.76f, -6087.62f, -2190.21f, -6349.54f, -2533.66f,
                -6308.63f, -3049.32f, -6107.82f, -3345.30f, -6008.49f, -3590.52f,
                -5989.37f, -4312.29f, -5806.26f, -5864.11f};
            const static float stormwindAreaNorthLimit[] = {
                -8004.25f,  3714.11f,   -8075.00f,  -179.00f,   -8638.00f, 169.00f,    -9044.00f,
                35.00f,     -9068.00f,  -125.00f,   -9094.00f,  -147.00f,  -9206.00f,  -290.00f,
                -9097.00f,  -510.00f,   -8739.00f,  -501.00f,   -8725.50f, -1618.45f,  -9810.40f,
            ...
            const static float stormwindAreaSouthLimit[] = {
                ...
                839.069580f,    -9946.341797f,  743.102844f,    -10287.361328f, 760.076477f,
                -10083.828125f, 380.389893f,    -10148.072266f, 80.056450f,     -10014.583984f,
                -161.638519f,   -9978.146484f,  -361.638031f,   -9877.489258f,  -563.304871f,
                -9980.967773f,  -1128.510498f,  -9991.717773f,  -1428.793213f,  -9887.579102f,
                -1618.514038f,  -10169.600586f, -1801.582031f,  -9966.274414f,  -2227.197754f,
                -9861.309570f,  -2989.841064f,  -9944.026367f,  -3205.886963f,  -9610.209961f,
                -3648.369385f,  -7949.329590f,  -4081.389404f,  -7910.859375f,  -5855.578125f};
            if (IsNorthTo(
                    x, y, topNorthSouthLimit, sizeof(topNorthSouthLimit) / (2 * sizeof(float))))
                return MAP0_TOP_NORTH;
            if (x > -2521)
                return MAP0_MIDDLE_NORTH;
            if (IsNorthTo(x,
                          y,
                          ironforgeAreaSouthLimit,
                          sizeof(ironforgeAreaSouthLimit) / (2 * sizeof(float))))
                return MAP0_IRONFORGE_AREA;
            if (IsNorthTo(x,
                          y,
                          stormwindAreaNorthLimit,
                          sizeof(stormwindAreaNorthLimit) / (2 * sizeof(float))))
                return MAP0_MIDDLE;
            if (IsNorthTo(x,
                          y,
                          stormwindAreaSouthLimit,
                          sizeof(stormwindAreaNorthLimit) / (2 * sizeof(float))))
        ...
        case 1:
        {
            const static float northMiddleLimit[] = {
                -2280.00f, 4054.00f,  -2401.00f, 2365.00f,  -2432.00f, 1338.00f,  -2286.00f,
                769.00f,   -2137.00f, 662.00f,   -2044.54f, 489.86f,   -1808.52f, 436.39f,
                -1754.85f, 504.55f,   -1094.55f, 651.75f,   -747.46f,  647.73f,   -685.55f,
                408.43f,   -311.38f,  114.43f,   -358.40f,  -587.42f,  -377.92f,  -748.70f,
                -512.57f,  -919.49f,  -280.65f,  -1008.87f, -81.29f,   -930.89f,  284.31f,
                -1105.39f, 568.86f,   -892.28f,  1211.09f,  -1135.55f, 879.60f,   -2110.18f,
                788.96f,   -2276.02f, 899.68f,   -2625.56f, 1281.54f,  -2689.42f, 1521.82f,
                -3047.85f, 1424.22f,  -3365.69f, 1694.11f,  -3615.20f, 2373.78f,  -4019.96f,
                2388.13f,  -5124.35f, 2193.79f,  -5484.38f, 1703.57f,  -5510.53f, 1497.59f,
                -6376.56f, 1368.00f,  -8530.00f};
            const static float durotarSouthLimit[] = {
                2755.00f,  -3766.00f, 2225.00f,  -3596.00f, 1762.00f,  -3746.00f, 1564.00f,
                -3943.00f, 1184.00f,  -3915.00f, 737.00f,   -3782.00f, -75.00f,   -3742.00f,
                -263.00f,  -3836.00f, -173.00f,  -4064.00f, -81.00f,   -4091.00f, -49.00f,
                -4089.00f, -16.00f,   -4187.00f, -5.00f,    -4192.00f, -14.00f,   -4551.00f,
                -397.00f,  -4601.00f, -522.00f,  -4583.00f, -668.00f,  -4539.00f, -790.00f,
                -4502.00f, -1176.00f, -4213.00f, -1387.00f, -4674.00f, -2243.00f, -6046.00f};
            const static float valleyoftrialsSouthLimit[] = {-324.00f,
                                                             -3869.00f,
                                                             -774.00f,
                                                             -3992.00f,
                                                             -965.00f,
            ...
            const static float middleToSouthLimit[]       = {
                -2402.01f,     4255.70f,      -2475.933105f, 3199.568359f,  // Desolace
                ...
                -4824.16f,     -2310.11f,     -5102.913574f, -2647.062744f,
                -5248.286621f, -3034.536377f, -5246.920898f, -3339.139893f,
            ...
            if (IsNorthTo(x,
                          y,
                          valleyoftrialsSouthLimit,
                          sizeof(valleyoftrialsSouthLimit) / (2 * sizeof(float))))
                return MAP1_VALLEY;
            if (IsNorthTo(
                    x, y, middleToSouthLimit, sizeof(middleToSouthLimit) / (2 * sizeof(float))))
...
void MapManager::SwitchPlayersInstances()
{
    ...
    {
        for (it = m_scheduledInstanceSwitches[ continent ].begin();
             it != m_scheduledInstanceSwitches[ continent ].end();
             ++it)
        {
