...
bool PoolObject::CanBeSpawned() const
{
    if (exclude)
        return false;
    if (spawnFlags & FLAG_SPAWN_ENABLE_IF_WORLD_POP_OVER_BLIZZLIKE &&
...
{
    if (PoolFlags & POOL_FLAG_MAXLIMIT_SCALING_LINEAR &&
        sWorld.GetActiveSessionCount() > BLIZZLIKE_REALM_POPULATION)
        return uint32(MaxLimit *
                      float(sWorld.GetActiveSessionCount() / float(BLIZZLIKE_REALM_POPULATION)));
...
template <>
void PoolObject::CheckEventLinkAndReport<Creature>(
    uint32 poolId,
    int16 event_id,
    ...
    std::map<uint32, int16> const& /*go2event*/) const
{
    ...
    if (itr == creature2event.end() || itr->second != event_id)
        sLog.outErrorDb("Creature (GUID: %u) expected to be listed in `game_event_creature` for "
                        "event %u as part pool %u",
                        guid,
                        event_id,
                        poolId);
}
...
template <>
void PoolObject::CheckEventLinkAndReport<GameObject>(
    uint32 poolId,
    int16 event_id,
    std::map<uint32, int16> const& /*creature2event*/,
    std::map<uint32, int16> const& go2event) const
{
    ...
    if (itr == go2event.end() || itr->second != event_id)
        sLog.outErrorDb("Gameobject (GUID: %u) expected to be listed in `game_event_gameobject` "
                        "for event %u as part pool %u",
                        guid,
                        event_id,
                        poolId);
}
...
// Method to add a gameobject/creature guid to the proper list depending on pool type and chance
// value
template <class T>
                                           ...
                                           std::map<uint32, int16> const& go2event) const
{
    for (uint32 i = 0; i < EqualChanced.size(); ++i)
        EqualChanced[ i ].template CheckEventLinkAndReport<T>(
            poolId, event_id, creature2event, go2event);

    for (uint32 i = 0; i < ExplicitlyChanced.size(); ++i)
        ExplicitlyChanced[ i ].template CheckEventLinkAndReport<T>(
            poolId, event_id, creature2event, go2event);
}
...
PoolObject* PoolGroup<T>::RollOne(SpawnedPoolData& spawns, uint32 triggerFrom)
{
    if (!ExplicitlyChanced.empty())
    {
        ...
        for (uint32 i = 0; i < ExplicitlyChanced.size(); ++i)
        {
            ...
            // Triggering object is marked as spawned at this time and can be also rolled (respawn
            // case) so this need explicit check for this case
            if (roll < 0 && ExplicitlyChanced[ i ].CanBeSpawned() &&
                (ExplicitlyChanced[ i ].guid == triggerFrom ||
                 !spawns.IsSpawnedObject<T>(ExplicitlyChanced[ i ].guid)))
    ...
    if (!EqualChanced.empty())
    {
        ...
        for (int i = 0; i < EqualChanced.size(); ++i)
            if (EqualChanced[ i ].CanBeSpawned() &&
                (EqualChanced[ i ].guid == triggerFrom ||
                 !spawns.IsSpawnedObject<T>(EqualChanced[ i ].guid)))
    ...
    if (GameObjectData const* data = sObjectMgr.GetGOData(guid))
    {
        ...
        if (Map* dataMap = mapState.GetMap())
            if (GameObject* pGameobject =
                    dataMap->GetGameObject(ObjectGuid(HIGHGUID_GAMEOBJECT, data->id, guid)))
...
{
    for (PoolObjectList::iterator itr = ExplicitlyChanced.begin(); itr != ExplicitlyChanced.end();
         ++itr)
    {
...
template <class T>
void PoolGroup<T>::SpawnObject(MapPersistentState& mapState,
                               uint32 limit,
                               uint32 triggerFrom,
                               bool instantly)
{
...
template <>
void PoolGroup<Creature>::Spawn1Object(MapPersistentState& mapState,
                                       PoolObject* obj,
                                       bool instantly)
{
    if (CreatureData const* data = sObjectMgr.GetCreatureData(obj->guid))
    {
        ...
        if (dataMap && dataMap->IsLoaded(data->posX, data->posY))
        {
            ...
            {
                // if new spawn replaces a just despawned creature, not instantly spawn but set
                // respawn timer
                if (!instantly)
                {
        ...
        // for not loaded grid just update respawn time (avoid work for instances until implemented
        // support)
        else if (!instantly)
...
template <>
void PoolGroup<GameObject>::Spawn1Object(MapPersistentState& mapState,
                                         PoolObject* obj,
                                         bool instantly)
{
    if (GameObjectData const* data = sObjectMgr.GetGOData(obj->guid))
    {
        ...
        if (dataMap && dataMap->IsLoaded(data->posX, data->posY))
        {
            ...
            else
            {
                ...
                {
                    // if new spawn replaces a just despawned object, not instantly spawn but set
                    // respawn timer
                    if (!instantly)
                    {
        ...
        else if (!instantly)
        {
            ...
            if (data->spawntimesecs >= 0)
                mapState.SaveGORespawnTime(
                    obj->guid, time(NULL) + data->ComputeRespawnDelay(data->spawntimesecs));
        }
    }
}
...
void PoolGroup<GameObject>::ReSpawn1Object(MapPersistentState& mapState, PoolObject* obj)
{
    if (GameObjectData const* data = sObjectMgr.GetGOData(obj->guid))
    {
        ...
        if (Map* dataMap = mapState.GetMap())
            if (GameObject* pGameobject =
                    dataMap->GetGameObject(ObjectGuid(HIGHGUID_GAMEOBJECT, data->id, obj->guid)))
...
struct PoolMapChecker
{
    ...
    bool
    CheckAndRemember(uint32 mapid, uint32 pool_id, char const* tableName, char const* elementName)
    {
        ...
        {
            sLog.outErrorDb("`%s` has %s spawned at instanceable map %u when one or several other "
                            "spawned at different map %u in pool id %i, skipped.",
                            tableName,
                            elementName,
                            mapid,
                            poolMapEntry->id,
        ...
        {
            sLog.outErrorDb("`%s` has %s spawned at map %u when one or several other spawned at "
                            "different instanceable map %u in pool id %i, skipped.",
                            tableName,
                            elementName,
                            mapid,
                            poolMapEntry->id,
...
bool CheckPoolAndChance(const char* table, uint16 pool_id, float chance)
{
    ...
    {
        sLog.outErrorDb("`%s` pool id (%i) is out of range compared to max pool id in "
    ...
    {
        sLog.outErrorDb(
            "`%s` has an invalid chance (%f) in pool id (%i), skipped.", table, chance, pool_id);
        return false;
    }
    return true;
}
...
{
    auto result = WorldDatabase.Query("SELECT MAX(entry) FROM pool_template");
    if (!result)
    {
    ...
    result = WorldDatabase.Query(
        "SELECT entry, max_limit, flags, description, instance FROM pool_template");
    if (!result)
    {
        mPoolTemplate.clear();
        sLog.outString(">> Table pool_template is empty:");
    ...
    do
    {
        ...
        pPoolTemplate.InstanceId =
            sWorld.getConfig(CONFIG_BOOL_CONTINENTS_INSTANCIATE) ? fields[ 4 ].GetUInt32() : 0;
    ...
    result = WorldDatabase.Query(
        ...
        "FROM pool_creature_template LEFT JOIN creature ON creature.id = "
    ...
    else
    {
        ...
        do
        {
            ...
            if (!data)
            {
                sLog.outErrorDb("`%s` has a non existing creature spawn (GUID: %u) defined for "
                                "pool id (%u), skipped.",
                                table,
                                guid,
                                pool_id);
            ...
            // `pool_creature` and `pool_creature_template` can't have guids duplicates (in second
            // case because entries also unique) So if guid already listed in pools then this
            // duplicate from alt.table Also note: for added guid not important what case we skip
            // from 2 tables
            if (uint16 alt_pool_id = IsPartOfAPool<Creature>(guid))
            {
                sLog.outErrorDb("`%s` has guid %u for pool %u that already added to pool %u from "
                                "`pool_creature_template` for creature entry %u, skipped.",
                                table,
                                guid,
                                pool_id,
                                alt_pool_id,
        ...
        sLog.outString();
        sLog.outString(
            ">> Loaded %u creatures in pools from `pool_creature` and `pool_creature_template`",
            count);
    }
    ...
    result = WorldDatabase.Query("SELECT guid, pool_entry, chance, 0, flags FROM pool_gameobject "
                                 "UNION "
                                 "SELECT guid, pool_entry, chance, pool_gameobject_template.id, "
                                 "pool_gameobject_template.flags "
                                 "FROM pool_gameobject_template LEFT JOIN gameobject ON "
                                 "gameobject.id = pool_gameobject_template.id");
    ...
    if (!result)
    {
        ...
        sLog.outString();
        sLog.outString(">> Loaded %u gameobject in pools from `pool_gameobject` and "
    ...
    else
    {
        ...
        do
        {
            ...
            if (!data)
            {
                sLog.outErrorDb("`%s` has a non existing gameobject spawn (GUID: %u) defined for "
                                "pool id (%u), skipped.",
                                table,
                                guid,
                                pool_id);
            ...
            {
                sLog.outErrorDb("`%s` has a not lootable gameobject spawn (GUID: %u, type: %u) "
                                "defined for pool id (%u), skipped.",
                                table,
                                guid,
                                goinfo->type,
                                pool_id);
            ...
            // `pool_gameobject` and `pool_gameobject_template` can't have guids duplicates (in
            // second case because entries also unique) So if guid already listed in pools then this
            // duplicate from alt.table Also note: for added guid not important what case we skip
            // from 2 tables
            if (uint16 alt_pool_id = IsPartOfAPool<GameObject>(guid))
            {
                sLog.outErrorDb("`%s` has guid %u for pool %u that already added to pool %u from "
                                "`pool_gameobject_template` for gameobject entry %u, skipped.",
                                table,
                                guid,
                                pool_id,
                                alt_pool_id,
    ...
    else
    {
        ...
        do
        {
            ...
            {
                sLog.outErrorDb("`pool_pool` included pool_id (%i) is out of range compared to max "
                                "pool id in `pool_template`, skipped.",
                                child_pool_id);
                continue;
            }
            ...
            {
                sLog.outErrorDb(
                    "`pool_pool` pool_id (%i) includes itself, dead-lock detected, skipped.",
                    child_pool_id);
                continue;
            }
        ...
        for (uint16 i = 0; i < max_pool_id; ++i)
        {
            ...
            for (SearchMap::iterator poolItr = mPoolSearchMap.find(i);
                 poolItr != mPoolSearchMap.end();
                 poolItr = mPoolSearchMap.find(poolItr->second))
            {
                // if child pool not have map data then it empty or have not checked child then will
                // checked and all line later
                if (MapEntry const* childMapEntry = mPoolTemplate[ poolItr->first ].mapEntry)
                {
                    if (!mapChecker.CheckAndRemember(childMapEntry->id,
                                                     poolItr->second,
                                                     "pool_pool",
                ...
                if (checkedPools.find(poolItr->second) != checkedPools.end())
                {
                    ...
                    ss << "The pool(s) ";
                    for (std::set<uint16>::const_iterator itr = checkedPools.begin();
                         itr != checkedPools.end();
                         ++itr)
                        ss << *itr << " ";
                    ss << "create(s) a circular reference, which can cause the server to "
    ...
    for (uint16 pool_entry = 0; pool_entry < mPoolTemplate.size(); ++pool_entry)
    {
        if (mPoolTemplate[ pool_entry ].IsAutoSpawn())
        {
            ...
            {
                sLog.outErrorDb("Pool Id (%u) has all creatures or gameobjects with explicit "
                                "chance sum <>100 and no equal chance defined. The pool system "
                                "cannot pick one to spawn.",
                                pool_entry);
...
template <>
void PoolManager::SpawnPoolGroup<Creature>(MapPersistentState& mapState,
                                           uint16 pool_id,
                                           uint32 db_guid,
                                           bool instantly)
{
    if (!mPoolCreatureGroups[ pool_id ].isEmpty())
        mPoolCreatureGroups[ pool_id ].SpawnObject(
            mapState, mPoolTemplate[ pool_id ].GetSpawnCount(), db_guid, instantly);
}

// Call to spawn a pool, if cache if true the method will spawn only if cached entry is different
// If it's same, the gameobject is respawned only (added back to map)
template <>
void PoolManager::SpawnPoolGroup<GameObject>(MapPersistentState& mapState,
                                             uint16 pool_id,
                                             uint32 db_guid,
                                             bool instantly)
{
...
template <>
void PoolManager::SpawnPoolGroup<Pool>(MapPersistentState& mapState,
                                       uint16 pool_id,
                                       uint32 sub_pool_id,
                                       bool instantly)
{
    if (!mPoolPoolGroups[ pool_id ].isEmpty())
        mPoolPoolGroups[ pool_id ].SpawnObject(
            mapState, mPoolTemplate[ pool_id ].GetSpawnCount(), sub_pool_id, instantly);
}
...
bool PoolManager::CheckPool(uint16 pool_id) const
{
    return pool_id <= max_pool_id && mPoolGameobjectGroups[ pool_id ].CheckPool() &&
           mPoolCreatureGroups[ pool_id ].CheckPool() && mPoolPoolGroups[ pool_id ].CheckPool();
}

// Method that check linking all elements to event
void PoolManager::CheckEventLinkAndReport(uint16 pool_id,
                                          int16 event_id,
                                          std::map<uint32, int16> const& creature2event,
                                          std::map<uint32, int16> const& go2event) const
{
...
template <>
void PoolManager::SetExcludeObject<GameObject>(uint16 pool_id,
...
template <typename T>
void PoolManager::UpdatePool(MapPersistentState& mapState,
                             uint16 pool_id,
                             uint32 db_guid_or_pool_id)
{
...
template void PoolManager::UpdatePool<Pool>(MapPersistentState& mapState,
                                            uint16 pool_id,
                                            uint32 db_guid_or_pool_id);
template void PoolManager::UpdatePool<GameObject>(MapPersistentState& mapState,
                                                  uint16 pool_id,
                                                  uint32 db_guid_or_pool_id);
template void PoolManager::UpdatePool<Creature>(MapPersistentState& mapState,
                                                uint16 pool_id,
                                                uint32 db_guid_or_pool_id);
...
// used for calling from global systems when need spawn pool in all appropriate map persistent
// states
void PoolManager::SpawnPoolInMaps(uint16 pool_id, bool instantly)
{
    ...
    sMapPersistentStateMgr.DoForAllStatesWithMapId(
        poolTemplate.mapEntry->id, poolTemplate.InstanceId, worker);
}
...
{
    // spawn pool for expected map or for not initialized shared pools state for non-instanceable
    // maps
...
struct UpdatePoolInMapsWorker
{
    explicit UpdatePoolInMapsWorker(PoolManager& mgr, uint32 pool_id, uint32 db_guid_or_pool_id)
        : i_mgr(mgr)
        , i_pool_id(pool_id)
        , i_db_guid_or_pool_id(db_guid_or_pool_id)
    {
    }
