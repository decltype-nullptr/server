...
namespace MMAP
{
    ...
    bool MMapManager::loadMapData(uint32 mapId)
    {
        ...
        {
            dtFreeNavMesh(mesh);
            sLog.outError("MMAP:loadMapData: Failed to initialize dtNavMesh for mmap %03u from "
                          "file %s with %u tiles. Result 0x%x.",
                          mapId,
                          fileName,
                          params.maxTiles,
                          r);
    ...
    bool MMapManager::loadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        {
            sLog.outError(
                "MMAP:loadMap: %03u%02i%02i.mmtile was built with generator v%i, expected v%i",
                mapId,
                x,
                y,
        ...
        {
            sLog.outError(
                "MMAP:loadMap: Bad header or data in mmap %03u%02i%02i.mmtile", mapId, x, y);
            fclose(file);
            return false;
        }
        ...
        // memory allocated for data is now managed by detour, and will be deallocated when the tile
        // is removed
        dtStatus dResult =
            mmap->navMesh->addTile(data, fileHeader.size, DT_TILE_FREE_DATA, 0, &tileRef);
        ...
        {
            sLog.outError(
                "MMAP:loadMap: Could not load %03u%02i%02i.mmtile into navmesh [result 0x%x]",
                mapId,
                x,
                y,
                dResult);
    ...
    bool MMapManager::unloadMap(uint32 mapId, int32 x, int32 y)
    {
        ...
        if (loadedMMaps.find(mapId) == loadedMMaps.end())
        {
            ...
            DEBUG_LOG("MMAP:unloadMap: Asked to unload not loaded navmesh map. %03u%02i%02i.mmtile",
                      mapId,
                      x,
                      y);
            return false;
        ...
        if (mmap->mmapLoadedTiles.find(packedGridPos) == mmap->mmapLoadedTiles.end())
        {
            // file may not exist, therefore not loaded
            DEBUG_LOG(
                "MMAP:unloadMap: Asked to unload not loaded navmesh tile. %03u%02i%02i.mmtile",
                mapId,
                x,
                y);
            return false;
        ...
        if (DT_SUCCESS != mmap->navMesh->removeTile(tileRef, NULL, NULL))
        {
            ...
            // if the grid is later reloaded, dtNavMesh::addTile will return error but no extra
            // memory is used we cannot recover from this error - assert out
            sLog.outError(
                "MMAP:unloadMap: Could not unload %03u%02i%02i.mmtile from navmesh", mapId, x, y);
            MANGOS_ASSERT(false);
        }
        else
        {
    ...
    bool MMapManager::unloadMap(uint32 mapId)
    {
        ...
        MMapData* mmap = loadedMMaps[ mapId ];
        for (MMapTileSet::iterator i = mmap->mmapLoadedTiles.begin();
             i != mmap->mmapLoadedTiles.end();
             ++i)
        {
            uint32 x = (i->first >> 16);
            uint32 y = (i->first & 0x0000FFFF);
            if (DT_SUCCESS != mmap->navMesh->removeTile(i->second, NULL, NULL))
                sLog.outError("MMAP:unloadMap: Could not unload %03u%02i%02i.mmtile from navmesh",
                              mapId,
                              x,
                              y);
            else
    ...
    bool MMapManager::unloadMapInstance(uint32 mapId, uint32 instanceId)
    {
        ...
        {
            DEBUG_LOG("MMAP:unloadMapInstance: Asked to unload not loaded dtNavMeshQuery mapId "
                      "%03u instanceId %u",
                      mapId,
                      instanceId);
            return false;
    ...
    dtNavMeshQuery const* MMapManager::GetNavMeshQuery(uint32 mapId)
    {
        ...
        if (it == mmap->navMeshQueries.end())
        {
            ...
            if (DT_SUCCESS != navMeshQuery->init(mmap->navMesh, 2048, tid))
            {
                ...
                sLog.outError("MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for mapId "
                              "%03u thread %u",
                              mapId,
                              tid);
                return NULL;
            ...
            DETAIL_LOG("MMAP:GetNavMeshQuery: created dtNavMeshQuery for mapId %03u thread %u",
                       mapId,
                       tid);
    ...
    bool MMapManager::loadGameObject(uint32 displayId)
    {
        ...
        char* fileName = new char[ pathLen ];
        snprintf(
            fileName, pathLen, (sWorld.GetDataPath() + "mmaps/go%04i.mmap").c_str(), displayId);
        ...
        if (dtStatusFailed(r))
        {
            dtFreeNavMesh(mesh);
            sLog.outError(
                "MMAP:loadGameObject: Failed to initialize dtNavMesh from file %s. Result 0x%x.",
                fileName,
                r);
            delete[] fileName;
            return false;
    ...
    dtNavMeshQuery const* MMapManager::GetModelNavMeshQuery(uint32 displayId)
    {
        ...
        if (mmap->navMeshQueries.find(tid) == mmap->navMeshQueries.end())
        {
            ...
            if (mmap->navMeshQueries.find(tid) == mmap->navMeshQueries.end())
            {
                ...
                {
                    dtFreeNavMeshQuery(query);
                    sLog.outError("MMAP:GetNavMeshQuery: Failed to initialize dtNavMeshQuery for "
                                  "displayid %03u tid %u",
                                  displayId,
                                  tid);
                    return NULL;
                ...
                DETAIL_LOG("MMAP:GetNavMeshQuery: created dtNavMeshQuery for displayid %03u tid %u",
                           displayId,
                           tid);
