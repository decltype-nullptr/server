...
enum FactionFlags
{
    FACTION_FLAG_VISIBLE = 0x01, // makes visible in client (set or can be set at interaction with
                                 // target of this faction)
    FACTION_FLAG_AT_WAR = 0x02,  // enable AtWar-button in client. player controlled (except
                                 // opposition team always war state), Flag only set on initial
                                 // creation
    ...
    FACTION_FLAG_INVISIBLE_FORCED = 0x08, // always overwrite FACTION_FLAG_VISIBLE and hide faction
                                          // in rep.list, used for hide opposite team factions
...
typedef std::pair<FactionStateList::const_iterator, FactionStateList::const_iterator>
...
public: // constructors and global modifiers
    ...
    void SaveToDB();
    void LoadFromDB(const std::shared_ptr<QueryResult>& result);
...
public: // accessors
    ...
    {
        return factionEntry->reputationListID >= 0 ? GetState(factionEntry->reputationListID) :
                                                     NULL;
    }
    ...
    ReputationRank const* GetForcedRankIfAny(FactionTemplateEntry const* factionTemplateEntry) const
    {
        ForcedReactions::const_iterator forceItr =
            m_forcedReactions.find(factionTemplateEntry->faction);
...
private: // internal helper functions
    ...
    bool
    SetOneFactionReputation(FactionEntry const* factionEntry, int32 standing, bool incremental);
