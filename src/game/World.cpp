...
Player* World::FindPlayerInZone(uint32 zone)
{
    ...
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        {
            // Used by the weather system. We return the player to broadcast the change weather
            // message to him and all players in the zone.
            return player;
        }
    }
    return nullptr;
...
{
    ///- Find the session, kick the user, but we can't delete session at this moment to prevent
...
void World::AddSession_(WorldSession* s)
{
    ...
    if (pLimit > 0 && Sessions >= pLimit && !CanSkipQueue(s))
    {
        ...
        DETAIL_LOG("PlayerQueue: Account id %u is in Queue Position (%u).",
                   s->GetAccountId(),
                   ++QueueSize);
        return;
    }
    ...
    if (pLimit > 0)
    {
        ...
        SqlStatement stmt =
            LoginDatabase.CreateStatement(id, "UPDATE realmlist SET population = ? WHERE id = ?");
...
int32 World::GetQueuedSessionPos(WorldSession* sess)
{
    ...
    for (Queue::const_iterator iter = m_QueuedSessions.begin(); iter != m_QueuedSessions.end();
         ++iter, ++position)
        if ((*iter) == sess)
            return position;
...
void World::LoadConfigSettings(bool reload)
{
    if (reload)
    {
        if (!sConfig.Reload())
        {
            sLog.outError("World settings reload fail: can't read settings from %s.",
                          sConfig.GetFilename().c_str());
            return;
        }
    }

    ///- Read the version of the configuration file and warn the user in case of emptiness or
    /// mismatch
    ...
    else
    {
        if (confVersion < _MANGOSDCONFVERSION)
        {
            sLog.outError(
                "*****************************************************************************");
            sLog.outError(
                " WARNING: Your mangosd.conf version indicates your conf file is out of date!");
            sLog.outError(
    ...
    setConfigPos(
        CONFIG_FLOAT_RATE_CREATURE_ELITE_ELITE_DAMAGE, "Rate.Creature.Elite.Elite.Damage", 1.0f);
    ...
    setConfigPos(CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_DAMAGE,
                 ...
                 1.0f);
    setConfigPos(
        CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_DAMAGE, "Rate.Creature.Elite.RARE.Damage", 1.0f);
    ...
    setConfigPos(
        CONFIG_FLOAT_RATE_CREATURE_ELITE_RAREELITE_HP, "Rate.Creature.Elite.RAREELITE.HP", 1.0f);
    setConfigPos(
        CONFIG_FLOAT_RATE_CREATURE_ELITE_WORLDBOSS_HP, "Rate.Creature.Elite.WORLDBOSS.HP", 1.0f);
    setConfigPos(CONFIG_FLOAT_RATE_CREATURE_ELITE_RARE_HP, "Rate.Creature.Elite.RARE.HP", 1.0f);
    setConfigPos(
        CONFIG_FLOAT_RATE_CREATURE_NORMAL_SPELLDAMAGE, "Rate.Creature.Normal.SpellDamage", 1.0f);
    ...
    setConfig(
        CONFIG_FLOAT_RATE_REST_OFFLINE_IN_TAVERN_OR_CITY, "Rate.Rest.Offline.InTavernOrCity", 1.0f);
    ...
    setConfigMinMax(CONFIG_FLOAT_RATE_TARGET_POS_RECALCULATION_RANGE,
    ...
    setConfigPos(
        CONFIG_FLOAT_CREATURE_FAMILY_ASSISTANCE_RADIUS, "CreatureFamilyAssistanceRadius", 10.0f);
    ...
    setConfigMinMax(
        CONFIG_UINT32_MIN_LEVEL_STAT_SAVE, "PlayerSave.Stats.MinLevel", 0, 0, MAX_LEVEL);
    setConfig(CONFIG_BOOL_STATS_SAVE_ONLY_ON_LOGOUT, "PlayerSave.Stats.SaveOnlyOnLogout", true);

    setConfigMin(CONFIG_UINT32_INTERVAL_GRIDCLEAN,
                 "GridCleanUpDelay",
    ...
    setConfig(CONFIG_UINT32_INTERVAL_CHANGEWEATHER,
              "ChangeWeatherInterval",
              10 * MINUTE * IN_MILLISECONDS);

    if (configNoReload(
            reload, CONFIG_UINT32_PORT_WORLD, "WorldServerPort", DEFAULT_WORLDSERVER_PORT))
    ...
    setConfig(
        CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_CHANNEL, "AllowTwoSide.Interaction.Channel", false);
    setConfig(
        CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GROUP, "AllowTwoSide.Interaction.Group", false);
    setConfig(
        CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_GUILD, "AllowTwoSide.Interaction.Guild", false);
    setConfig(
        CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_TRADE, "AllowTwoSide.Interaction.Trade", false);
    setConfig(
        CONFIG_BOOL_ALLOW_TWO_SIDE_INTERACTION_AUCTION, "AllowTwoSide.Interaction.Auction", false);
    ...
    setConfigMin(CONFIG_UINT32_CHARACTERS_PER_ACCOUNT,
                 "CharactersPerAccount",
                 50,
                 getConfig(CONFIG_UINT32_CHARACTERS_PER_REALM));
    ...
    if (configNoReload(reload, CONFIG_UINT32_MAX_PLAYER_LEVEL, "MaxPlayerLevel", DEFAULT_MAX_LEVEL))
        setConfigMinMax(CONFIG_UINT32_MAX_PLAYER_LEVEL,
                        "MaxPlayerLevel",
                        DEFAULT_MAX_LEVEL,
                        1,
                        DEFAULT_MAX_LEVEL);

    setConfigMinMax(CONFIG_UINT32_START_PLAYER_LEVEL,
                    "StartPlayerLevel",
                    1,
                    1,
    ...
    setConfigMinMax(CONFIG_UINT32_START_HONOR_POINTS,
                    "StartHonorPoints",
                    0,
                    0,
    ...
    setConfig(
        CONFIG_UINT32_INSTANCE_UNLOAD_DELAY, "Instance.UnloadDelay", 30 * MINUTE * IN_MILLISECONDS);
    ...
    setConfigMinMax(CONFIG_UINT32_START_GM_LEVEL,
                    "GM.StartLevel",
                    1,
    ...
    if (reload)
    {
        m_timers[ WUPDATE_UPTIME ].SetInterval(getConfig(CONFIG_UINT32_UPTIME_UPDATE) * MINUTE *
                                               IN_MILLISECONDS);
    ...
    {
        sLog.outError("MaxOverspeedPings (%i) must be in range 2..infinity (or 0 to disable "
                      "check). Set to 2.",
    ...
    setConfig(
        CONFIG_UINT32_CREATURE_FAMILY_ASSISTANCE_DELAY, "CreatureFamilyAssistanceDelay", 1500);
    ...
    // note: disable value (-1) will assigned as 0xFFFFFFF, to prevent overflow at calculations
    // limit it to max possible player level MAX_LEVEL(100)
    ...
    setConfig(
        CONFIG_UINT32_CHAT_STRICT_LINK_CHECKING_SEVERITY, "ChatStrictLinkChecking.Severity", 0);
    ...
    setConfigMinMax(
        CONFIG_FLOAT_GHOST_RUN_SPEED_WORLD, "Death.Ghost.RunSpeed.World", 1.0f, 0.1f, 10.0f);
    setConfigMinMax(
        CONFIG_FLOAT_GHOST_RUN_SPEED_BG, "Death.Ghost.RunSpeed.Battleground", 1.0f, 0.1f, 10.0f);
    ...
    setConfigMinMax(CONFIG_UINT32_BATTLEGROUND_QUEUE_ANNOUNCER_JOIN,
                    ...
                    2);
    setConfig(
        CONFIG_BOOL_BATTLEGROUND_QUEUE_ANNOUNCER_START, "Battleground.QueueAnnouncer.Start", false);
    ...
    setConfig(CONFIG_UINT32_BATTLEGROUND_PREMADE_GROUP_WAIT_FOR_MATCH,
              ...
              0);
    setConfig(CONFIG_UINT32_BATTLEGROUND_PREMADE_QUEUE_GROUP_MIN_SIZE,
    ...
    m_relocation_ai_notify_delay =
        sConfig.GetIntDefault("Visibility.AIRelocationNotifyDelay", 1000u);
    m_relocation_lower_limit_sq =
        pow(sConfig.GetFloatDefault("Visibility.RelocationLowerLimit", 10), 2);
    ...
    {
        sLog.outError("Visibility.Distance.Grey.Object can't be greater %f",
    ...
    m_MaxVisibleDistanceOnContinents =
        sConfig.GetFloatDefault("Visibility.Distance.Continents", DEFAULT_VISIBILITY_DISTANCE);
    if (m_MaxVisibleDistanceOnContinents < 45 * getConfig(CONFIG_FLOAT_RATE_CREATURE_AGGRO))
    {
        sLog.outError("Visibility.Distance.Continents can't be less max aggro radius %f",
    ...
    {
        sLog.outError("Visibility.Distance.Continents can't be greater %f",
    ...
    m_MaxVisibleDistanceInInstances =
        sConfig.GetFloatDefault("Visibility.Distance.Instances", DEFAULT_VISIBILITY_INSTANCE);
    if (m_MaxVisibleDistanceInInstances < 45 * getConfig(CONFIG_FLOAT_RATE_CREATURE_AGGRO))
    {
        sLog.outError("Visibility.Distance.Instances can't be less max aggro radius %f",
                      45 * getConfig(CONFIG_FLOAT_RATE_CREATURE_AGGRO));
    ...
    {
        sLog.outError("Visibility.Distance.Instances can't be greater %f",
                      MAX_VISIBILITY_DISTANCE - m_VisibleUnitGreyDistance);
    ...
    m_MaxVisibleDistanceInBG =
        sConfig.GetFloatDefault("Visibility.Distance.BG", DEFAULT_VISIBILITY_BG);
    if (m_MaxVisibleDistanceInBG < 45 * sWorld.getConfig(CONFIG_FLOAT_RATE_CREATURE_AGGRO))
    {
        sLog.outError("Visibility.Distance.BG can't be less max aggro radius %f",
                      45 * getConfig(CONFIG_FLOAT_RATE_CREATURE_AGGRO));
    ...
    {
        sLog.outError("Visibility.Distance.BG can't be greater %f",
                      MAX_VISIBILITY_DISTANCE - m_VisibleUnitGreyDistance);
        m_MaxVisibleDistanceInBG = MAX_VISIBILITY_DISTANCE - m_VisibleUnitGreyDistance;
    }

    m_MaxVisibleDistanceInFlight =
        sConfig.GetFloatDefault("Visibility.Distance.InFlight", DEFAULT_VISIBILITY_DISTANCE);
    if (m_MaxVisibleDistanceInFlight + m_VisibleObjectGreyDistance > MAX_VISIBILITY_DISTANCE)
    {
        sLog.outError("Visibility.Distance.InFlight can't be greater %f",
    ...
    setConfigMinMax(CONFIG_UINT32_CHARDELETE_MIN_LEVEL,
                    "CharDelete.MinLevel",
                    0,
                    0,
                    getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL));
    ...
    if (configNoReload(
            reload, CONFIG_UINT32_GUID_RESERVE_SIZE_CREATURE, "GuidReserveSize.Creature", 100))
        setConfigPos(CONFIG_UINT32_GUID_RESERVE_SIZE_CREATURE, "GuidReserveSize.Creature", 100);
    if (configNoReload(
            reload, CONFIG_UINT32_GUID_RESERVE_SIZE_GAMEOBJECT, "GuidReserveSize.GameObject", 100))
    ...
    if (reload)
    {
        if (dataPath != m_dataPath)
            sLog.outError(
                "DataDir option can't be changed at mangosd.conf reload, using current value (%s).",
                m_dataPath.c_str());
    }
    else
    {
    ...
    if (!enableHeight)
        sLog.outError("VMAP height use disabled! Creatures movements and other things will be in "
                      "broken state.");
    ...
    //    sLog.outString( "WORLD: VMap config keys are: vmap.enableLOS, vmap.enableHeight,
    //    vmap.ignoreMapIds, vmap.ignoreSpellIds");
    ...
    setConfig(
        CONFIG_FLOAT_MAX_CREATURES_STEALTH_DETECT_RANGE, "MaxCreaturesStealthDetectRange", 15.0f);
    ...
    setConfig(
        CONFIG_BOOL_ACCURATE_PVP_PURCHASE_REQUIREMENTS, "PvP.AccuratePurchaseRequirements", false);
...
void World::LoadNostalriusConfig(bool reload)
{
    ...
    // m_configNostalrius[CONFIG_PHASE_WHISP]                =
    // sConfig.GetIntDefault("Phase.Allow.WhoList",   20);
    m_configNostalrius[ CONFIG_BANLIST_RELOAD_TIMER ] =
        sConfig.GetIntDefault("BanListReloadTimer", 60);

    setConfig(CONFIG_BOOL_MMAP_ENABLED, "mmap.enabled", true);
    sLog.outString("WORLD: mmap pathfinding %sabled",
                   getConfig(CONFIG_BOOL_MMAP_ENABLED) ? "en" : "dis");

    setConfigMinMax(CONFIG_UINT32_PET_DEFAULT_LOYALTY, "Pet.DefaultLoyalty", 1, 1, 6);
    setConfigMinMax(
        CONFIG_UINT32_MAP_OBJECTSUPDATE_THREADS, "MapUpdate.ObjectsUpdate.MaxThreads", 4, 1, 20);
    setConfigMinMax(
        CONFIG_UINT32_MAP_OBJECTSUPDATE_TIMEOUT, "MapUpdate.ObjectsUpdate.Timeout", 100, 10, 2000);
    setConfigMinMax(CONFIG_UINT32_MAP_VISIBILITYUPDATE_THREADS,
                    "MapUpdate.VisibilityUpdate.MaxThreads",
                    4,
                    1,
                    20);
    setConfigMinMax(CONFIG_UINT32_MAP_VISIBILITYUPDATE_TIMEOUT,
                    "MapUpdate.VisibilityUpdate.Timeout",
                    100,
                    10,
                    2000);
    setConfigMinMax(CONFIG_UINT32_MAPUPDATE_INSTANCED_UPDATE_THREADS,
                    "MapUpdate.Instanced.UpdateThreads",
                    2,
                    0,
                    20);
    setConfigMinMax(
        CONFIG_UINT32_MTCELLS_THREADS, "MapUpdate.Continents.MTCells.Threads", 0, 0, 20);
    setConfigMinMax(CONFIG_UINT32_MTCELLS_SAFEDISTANCE,
                    "MapUpdate.Continents.MTCells.SafeDistance",
                    1066,
                    0,
                    34112);
    setConfigMinMax(
        CONFIG_UINT32_MAPUPDATE_UPDATE_PACKETS_DIFF, "MapUpdate.UpdatePacketsDiff", 100, 1, 10000);
    setConfigMinMax(
        CONFIG_UINT32_MAPUPDATE_UPDATE_PLAYERS_DIFF, "MapUpdate.UpdatePlayersDiff", 100, 1, 10000);
    setConfigMinMax(
        CONFIG_UINT32_MAPUPDATE_UPDATE_CELLS_DIFF, "MapUpdate.UpdateCellsDiff", 100, 1, 10000);
    setConfigMinMax(CONFIG_UINT32_INACTIVE_PLAYERS_SKIP_UPDATES,
                    "Continents.InactivePlayers.SkipUpdates",
                    0,
                    0,
                    100);
    setConfig(CONFIG_UINT32_MAPUPDATE_TICK_LOWER_GRID_ACTIVATION_DISTANCE,
              "MapUpdate.ReduceGridActivationDist.Tick",
              0);
    setConfig(CONFIG_UINT32_MAPUPDATE_TICK_INCREASE_GRID_ACTIVATION_DISTANCE,
              "MapUpdate.IncreaseGridActivationDist.Tick",
              0);
    setConfig(CONFIG_UINT32_MAPUPDATE_MIN_GRID_ACTIVATION_DISTANCE,
              "MapUpdate.MinGridActivationDistance",
              0);
    setConfig(
        CONFIG_UINT32_MAPUPDATE_TICK_LOWER_VISIBILITY_DISTANCE, "MapUpdate.ReduceVisDist.Tick", 0);
    setConfig(CONFIG_UINT32_MAPUPDATE_TICK_INCREASE_VISIBILITY_DISTANCE,
              "MapUpdate.IncreaseVisDist.Tick",
              0);
    setConfig(
        CONFIG_UINT32_MAPUPDATE_MIN_VISIBILITY_DISTANCE, "MapUpdate.MinVisibilityDistance", 0);
    ...
    setConfigMinMax(
        CONFIG_UINT32_MAX_POINTS_PER_MVT_PACKET, "Movement.MaxPointsPerPacket", 80, 5, 10000);
    setConfigMinMax(CONFIG_UINT32_RELOCATION_VMAP_CHECK_TIMER,
    ...
    setConfig(
        CONFIG_UINT32_PERFLOG_SLOW_MAPSYSTEM_UPDATE, "PerformanceLog.SlowMapSystemUpdate", 100);
    ...
    setConfig(CONFIG_UINT32_PERFLOG_SLOW_UNIQUE_SESSION_UPDATE,
              "PerformanceLog.SlowUniqueSessionUpdate",
              20);
    ...
    setConfig(CONFIG_UINT32_DYN_RESPAWN_AFFECT_RESPAWN_TIME_BELOW,
              "DynamicRespawn.AffectRespawnTimeBelow",
              0);
    ...
    setConfig(
        CONFIG_UINT32_YELLRANGE_QUADRATICSCALE_MAXLEVEL, "YellRange.QuadraticScale.MaxLevel", 0);
    ...
    setConfig(
        CONFIG_UINT32_ITEM_INSTANTSAVE_QUALITY, "Item.InstantSave.Quality", ITEM_QUALITY_ARTIFACT);
    ...
    setConfig(CONFIG_UINT32_PBCAST_DIFF_LOWER_VISIBILITY_DISTANCE,
    ...
    m_timers[ WUPDATE_CORPSES ].SetInterval(getConfig(CONFIG_UINT32_CORPSES_UPDATE_MINUTES) *
                                            MINUTE * IN_MILLISECONDS);

    sLog.outString("* Anticrash : options 0x%x rearm after %usec",
                   getConfig(CONFIG_UINT32_ANTICRASH_OPTIONS),
                   getConfig(CONFIG_UINT32_ANTICRASH_REARM_TIMER) / 1000);
    sLog.outString("* Pathfinding : [%s]", getConfig(CONFIG_BOOL_MMAP_ENABLED) ? "ON" : "OFF");

    // Update packet broadcaster config
    if (reload)
    {
        sWorld.m_broadcaster->UpdateConfiguration(
            getConfig(CONFIG_UINT32_PACKET_BCAST_THREADS),
...
void World::SetInitialWorldSettings()
{
    ...
    {
        sLog.outError("Correct *.map files not found in path '%smaps' or *.vmtree/*.vmtile files "
                      "in '%svmaps'. Please place *.map and vmap files in appropriate directories "
                      "or correct the DataDir value in the mangosd.conf file.",
                      m_dataPath.c_str(),
                      m_dataPath.c_str());
        Log::WaitBeforeContinueIfNeed();
        exit(1);
    ...
    ///- Loading strings. Getting no records means core load has to be canceled because no error
    ...
    LoginDatabase.PExecute("UPDATE realmlist SET icon = %u, timezone = %u WHERE id = '%u'",
                           server_type,
                           realm_zone,
                           realmID);
    ...
    if (!isMapServer)
    {
        ...
        CharacterDatabase.PExecute(
            "DELETE FROM corpse WHERE corpse_type = '0' OR time < (UNIX_TIMESTAMP()-'%u')",
            3 * DAY);
    }
    ...
    DetectDBCLang();
    sObjectMgr.SetDBCLocaleIndex(GetDefaultDbcLocale()); // Get once for all the locale index of DBC
    ...
    if (!isMapServer)
    {
        ...
        sMapPersistentStateMgr.CleanupInstances(); // must be called before
                                                   // `creature_respawn`/`gameobject_respawn` tables
    ...
    ///- Init highest guids before any guid using table loading to prevent using not initialized
    /// guids in some code.
    ...
    sLog.outString(
        "Loading Items..."); // must be after LoadRandomEnchantmentsTable and LoadPageTexts
    ...
    sObjectMgr.LoadQuests(); // must be loaded after DBCs, creature_template, item_template,
                             // gameobject tables
    ...
    sLog.outString("Loading Game Event Data..."); // must be after sPoolMgr.LoadFromDB and quests to
                                                  // properly load pool events and quests for events
    sLog.outString();
    ...
    sObjectMgr
        .LoadVendors(); // must be after load CreatureTemplate, VendorTemplate, and ItemTemplate
    ...
    sLog.outString();
    sScriptMgr.LoadQuestStartScripts(); // must be after load Creature/Gameobject(Template/Data) and
                                        // QuestTemplate
    sScriptMgr.LoadQuestEndScripts();   // must be after load Creature/Gameobject(Template/Data) and
                                        // QuestTemplate
    ...
    sEventAIMgr.LoadCreatureEventAI_Texts(
        false); // false, will checked in LoadCreatureEventAI_Scripts
    ...
    LoginDatabase.PExecute(
        "INSERT INTO uptime (realmid, starttime, startstring, uptime) VALUES('%u', " UI64FMTD
        ", '%s', 0)",
        realmID,
        uint64(m_startTime),
        isoDate);
    ...
    // Not sure if this can be moved up in the sequence (with static data loading) as it uses
    // MapManager
    if (!isMapServer)
    {
    ...
    m_broadcaster = std::make_unique<MovementBroadcaster>(
        sWorld.getConfig(CONFIG_UINT32_PACKET_BCAST_THREADS),
    ...
    sLog.outString("SERVER STARTUP TIME: %i minutes %i seconds",
                   uStartInterval / 60000,
                   (uStartInterval % 60000) / 1000);
}
...
{
public:
    WorldAsyncTasksExecutor(int me, int total)
        : me(me)
        , total(total)
    {
    }
...
void World::Update(uint32 diff)
{
    ...
    if (getConfig(CONFIG_UINT32_PERFLOG_SLOW_SESSIONS_UPDATE) &&
        updateSessionsTime > getConfig(CONFIG_UINT32_PERFLOG_SLOW_SESSIONS_UPDATE))
    ...
    if (m_timers[ WUPDATE_UPTIME ].Passed())
    {
        ...
        m_timers[ WUPDATE_UPTIME ].Reset();
        LoginDatabase.PExecute("UPDATE uptime SET uptime = %u, maxplayers = %u WHERE realmid = %u "
    ...
    for (int i = 0; i < threadsCount; ++i)
        asyncTaskThreads.push_back(
            new ACE_Based::Thread(new WorldAsyncTasksExecutor(i, threadsCount)));
    ...
    if (m_timers[ WUPDATE_GROUPS ].Passed())
    {
        ...
        {
            for (ObjectMgr::GroupMap::const_iterator i = sObjectMgr.GetGroupMapBegin();
                 i != sObjectMgr.GetGroupMapEnd();
                 ++i)
    ...
    if (getConfig(CONFIG_UINT32_PERFLOG_SLOW_MAPSYSTEM_UPDATE) &&
        updateMapSystemTime > getConfig(CONFIG_UINT32_PERFLOG_SLOW_MAPSYSTEM_UPDATE))
        sLog.out(LOG_PERFORMANCE,
                 "Update map system: %ums [%ums for async]",
                 updateMapSystemTime,
                 WorldTimer::getMSTimeDiffToNow(asyncWaitBegin));
    ...
    if (getConfig(CONFIG_UINT32_PERFLOG_SLOW_ASYNC_QUERIES) &&
        asyncQueriesTime > getConfig(CONFIG_UINT32_PERFLOG_SLOW_ASYNC_QUERIES))
    ...
    /// </ul>
    ///- Move all creatures with "delayed move" and remove and delete all objects with "delayed
    /// remove"
...
void World::SendGlobalMessage(WorldPacket* packet, WorldSession* self, uint32 team)
{
    ...
    for (itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        if (itr->second && itr->second->GetPlayer() && itr->second->GetPlayer()->IsInWorld() &&
            itr->second != self && (team == 0 || itr->second->GetPlayer()->GetTeam() == team))
...
/// DEPRICATED, only for debug purpose. Send a System Message to all players (except self if
/// mentioned)
...
void World::KickAll()
{
    ...
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
        itr->second->KickPlayer();
    for (SessionSet::const_iterator itr = m_disconnectedSessions.begin();
         itr != m_disconnectedSessions.end();
         ++itr)
        (*itr)->KickPlayer();
}
...
void World::WarnAccount(uint32 accountId, std::string from, std::string reason, const char* type)
{
    ...
    LoginDatabase.PExecute("INSERT INTO account_banned (id, bandate, unbandate, bannedby, "
                           "banreason, active, realm) VALUES ('%u', UNIX_TIMESTAMP(), "
                           "UNIX_TIMESTAMP()+1, '%s', '%s', '0', %u)",
                           accountId,
                           from.c_str(),
                           reason.c_str(),
                           realmID);
                            ...
                            std::string author)
{
    ...
    std::shared_ptr<QueryResult> resultAccounts = nullptr; // used for kicking
    ...
    switch (mode)
    {
        case BAN_IP:
            // No SQL injection as strings are escaped
            resultAccounts = LoginDatabase.PQuery("SELECT id FROM account WHERE last_ip = '%s'",
                                                  nameOrIP.c_str());
        ...
        case BAN_ACCOUNT:
            ...
            resultAccounts = LoginDatabase.PQuery("SELECT id FROM account WHERE username = '%s'",
                                                  nameOrIP.c_str());
            break;
        case BAN_CHARACTER:
        {
        ...
        default: return BAN_SYNTAX_ERROR;
    }
...
void World::ShutdownServ(uint32 time, uint32 options, uint8 exitcode)
{
    ...
    ///- If the shutdown time is 0, set m_stopEvent (except if shutdown is 'idle' with remaining
    /// sessions)
    if (time == 0)
    {
...
void World::ShutdownMsg(bool show, Player* player)
{
        ...
        (m_ShutdownTimer > 12 * HOUR && (m_ShutdownTimer % (12 * HOUR)) == 0))
    {
        ...
        DEBUG_LOG("Server is %s in %s",
                  (m_ShutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shutting down"),
                  str.c_str());
    }
}
...
void World::UpdateSessions(uint32 diff)
{
    ...
    if (m_playerLimit >= 0 && static_cast<int32>(loggedInSessions) < hardPlayerLimit)
        if (uint32 acceptNow = getConfig(CONFIG_UINT32_LOGIN_PER_TICK))
        {
            ...
            // iter point to first not updated socket, position store new position
            int position = 1;
            for (Queue::iterator iter = m_QueuedSessions.begin(); iter != m_QueuedSessions.end();
                 ++iter, ++position)
                (*iter)->SendAuthWaitQue(position);
        }
    ...
    for (SessionMap::iterator itr = m_sessions.begin(), next; itr != m_sessions.end(); itr = next)
    {
        ...
        if (!pSession->Update(updater))
        {
            if (pSession->PlayerLoading())
                sLog.outInfo(
                    "[CRASH] World::UpdateSession attempt to delete session %u loading a player.",
                    pSession->GetAccountId());
    ...
    for (SessionSet::iterator itr = m_disconnectedSessions.begin(), next;
         itr != m_disconnectedSessions.end();
         itr = next)
    {
        next = itr;
...
void World::UpdateRealmCharCount(uint32 accountId)
{
    CharacterDatabase.AsyncPQuery(
        ...
        this,
        &World::_UpdateRealmCharCount,
        accountId);
}

void World::_UpdateRealmCharCount(const std::shared_ptr<QueryResult>& resultCharCount,
                                  uint32 accountId)
{
    if (resultCharCount)
    {
        ...
        LoginDatabase.PExecute("DELETE FROM realmcharacters WHERE acctid= '%u' AND realmid = '%u'",
                               accountId,
                               realmID);
        LoginDatabase.PExecute(
            "INSERT INTO realmcharacters (numchars, acctid, realmid) VALUES (%u, %u, %u)",
            charCount,
            accountId,
            realmID);
...
void World::SetPlayerLimit(int32 limit, bool needUpdate)
{
    ...
    // lock update need
    bool db_update_need = needUpdate || (limit < 0) != (m_playerLimit < 0) ||
...
void World::UpdateMaxSessionCounters()
{
    m_maxActiveSessionCount =
        std::max(m_maxActiveSessionCount, uint32(m_sessions.size() - m_QueuedSessions.size()));
...
void World::setConfigPos(eConfigUInt32Values index, char const* fieldname, uint32 defvalue)
{
    ...
    {
        sLog.outError("%s (%i) can't be negative. Using %u instead.",
                      fieldname,
...
void World::setConfigPos(eConfigFloatValues index, char const* fieldname, float defvalue)
{
    ...
    {
        sLog.outError(
            "%s (%f) can't be negative. Using %f instead.", fieldname, getConfig(index), defvalue);
        setConfig(index, defvalue);
    }
}

void World::setConfigMin(eConfigUInt32Values index,
                         ...
                         uint32 minvalue)
{
    ...
    {
        sLog.outError("%s (%u) must be >= %u. Using %u instead.",
                      fieldname,
                      getConfig(index),
                      minvalue,
                      minvalue);
...
void World::setConfigMin(eConfigInt32Values index,
                         char const* fieldname,
                         int32 defvalue,
                         int32 minvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        sLog.outError("%s (%i) must be >= %i. Using %i instead.",
                      fieldname,
                      getConfig(index),
                      minvalue,
                      minvalue);
...
void World::setConfigMin(eConfigFloatValues index,
                         char const* fieldname,
                         float defvalue,
                         float minvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        sLog.outError("%s (%f) must be >= %f. Using %f instead.",
                      fieldname,
                      getConfig(index),
                      minvalue,
                      minvalue);
...
void World::setConfigMinMax(eConfigUInt32Values index,
                            char const* fieldname,
                            uint32 defvalue,
                            uint32 minvalue,
                            uint32 maxvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        sLog.outError("%s (%u) must be in range %u...%u. Using %u instead.",
                      fieldname,
                      getConfig(index),
                      minvalue,
                      maxvalue,
...
void World::setConfigMinMax(eConfigInt32Values index,
                            char const* fieldname,
                            int32 defvalue,
                            int32 minvalue,
                            int32 maxvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        sLog.outError("%s (%i) must be in range %i...%i. Using %i instead.",
                      fieldname,
                      getConfig(index),
                      minvalue,
                      maxvalue,
...
void World::setConfigMinMax(eConfigFloatValues index,
                            char const* fieldname,
                            float defvalue,
                            float minvalue,
                            float maxvalue)
{
    ...
    if (getConfig(index) < minvalue)
    {
        sLog.outError("%s (%f) must be in range %f...%f. Using %f instead.",
                      fieldname,
                      getConfig(index),
                      minvalue,
                      maxvalue,
...
bool World::configNoReload(bool reload,
                           ...
                           uint32 defvalue)
{
    ...
    if (val != getConfig(index))
        sLog.outError(
            "%s option can't be changed at mangosd.conf reload, using current value (%u).",
            fieldname,
            getConfig(index));
                           ...
                           int32 defvalue)
{
    ...
    if (val != getConfig(index))
        sLog.outError(
            "%s option can't be changed at mangosd.conf reload, using current value (%i).",
            fieldname,
            getConfig(index));
                           ...
                           float defvalue)
{
    ...
    if (val != getConfig(index))
        sLog.outError(
            "%s option can't be changed at mangosd.conf reload, using current value (%f).",
            fieldname,
            getConfig(index));
                           ...
                           bool defvalue)
{
    ...
    if (val != getConfig(index))
        sLog.outError(
            "%s option can't be changed at mangosd.conf reload, using current value (%s).",
            fieldname,
            getConfig(index) ? "'true'" : "'false'");
...
void World::LogMoneyTrade(ObjectGuid sender,
                          ObjectGuid receiver,
                          uint32 amount,
                          const char* type,
                          uint32 dataInt)
{
    ...
    SqlStatement logStmt =
        LogsDatabase.CreateStatement(insLogMoney,
                                     "INSERT INTO logs_trade SET sender=?, senderType=?, "
                                     "senderEntry=?, receiver=?, amount=?, type=?, data=?");
...
void World::LogCharacter(Player* character, const char* action)
{
    ...
    SqlStatement logStmt = LogsDatabase.CreateStatement(
        insLogChar,
        "INSERT INTO logs_characters SET type=?, guid=?, account=?, name=?, ip=?, clientHash=?");
...
void World::LogCharacter(WorldSession* sess,
                         uint32 lowGuid,
...
void World::LogChat(WorldSession* sess,
                    const char* type,
                    ...
                    const char* chanStr)
{
    ...
    SqlStatement logStmt =
        LogsDatabase.CreateStatement(insLogChat,
                                     "INSERT INTO logs_chat SET type=?, guid=?, target=?, "
