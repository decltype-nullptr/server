...
public:
    LootGroup()
        : hasConditionalEqualChancedItem(false)
    {
    }
    ...
    void Process(
        Loot& loot) const; // Rolls an item from the group (if any) and adds the item to the loot
...
{
    for (LootTemplateMap::const_iterator i = m_LootTemplates.begin(); i != m_LootTemplates.end();
         ++i)
        i->second->Verify(*this, i->first);
}

// Loads a *_loot_template DB table into loot store
// All checks of the loaded template are called from here, no error reports at loot generation
// required
void LootStore::LoadLootTable()
{
    ...
    std::shared_ptr<QueryResult> result =
        WorldDatabase.PQuery("SELECT entry, item, ChanceOrQuestChance, groupid, mincountOrRef, "
    ...
    if (result)
    {
        ...
        do
        {
            ...
            {
                sLog.outErrorDb("Table '%s' entry %d item %d: maxcount value (%u) to large. must "
                                "be less %u - skipped",
                                GetName(),
                                entry,
                                item,
                ...
                sLog.out(LOG_DBERRFIX,
                         "DELETE FROM %s WHERE entry=%u AND item=%u;",
                         GetName(),
                         entry,
                         item);
            ...
            {
                sLog.outErrorDb("Table '%s' entry %u mincountOrRef %i < 0 and not allowed has "
                                "condition, skipped",
                                GetName(),
                                entry,
                                mincountOrRef);
                sLog.out(LOG_DBERRFIX,
                         "DELETE FROM %s WHERE entry=%u AND condition_id=%u AND mincountOrRef=%i;",
                         GetName(),
                         entry,
                         conditionId,
                         mincountOrRef);
            ...
            if (conditionId)
            {
                ...
                if (!condition)
                {
                    sLog.outErrorDb("Table `%s` for entry %u, item %u has condition_id %u that "
                                    "does not exist in `conditions`, ignoring",
                                    GetName(),
                                    entry,
                                    item,
                                    conditionId);
                ...
                {
                    sLog.outErrorDb("Table '%s' entry %u mincountOrRef %i < 0 and has condition %u "
            ...
            LootStoreItem storeitem = LootStoreItem(
                item, chanceOrQuestChance, group, conditionId, mincountOrRef, maxcount);
            ...
            if (m_LootTemplates.empty() || tab->first != entry)
            {
                ...
                {
                    std::pair<LootTemplateMap::iterator, bool> pr = m_LootTemplates.insert(
        ...
        sLog.outString();
        sLog.outString(">> Loaded %u loot definitions (%lu templates)",
                       count,
...
{
    sLog.outErrorDb("Table '%s' entry %d (%s) not exist but used as loot id in DB.",
                    GetName(),
                    id,
                    GetEntryName());
}
...
bool LootStoreItem::Roll(bool rate) const
{
    ...
    if (mincountOrRef < 0) // reference case
        return roll_chance_f(
            chance * (rate ? sWorld.getConfig(CONFIG_FLOAT_RATE_DROP_ITEM_REFERENCED) : 1.0f));

    ItemPrototype const* pProto = ObjectMgr::GetItemPrototype(itemid);

    float qualityModifier =
        pProto && rate ? sWorld.getConfig(qualityToRate[ pProto->Quality ]) : 1.0f;
...
bool LootStoreItem::IsValid(LootStore const& store, uint32 entry) const
{
    ...
    {
        sLog.outErrorDb("Table '%s' entry %d item %d: group (%u) must be less %u - skipped",
                        store.GetName(),
                        entry,
                        itemid,
                        group,
    ...
    if (mincountOrRef == 0)
    {
        sLog.outErrorDb("Table '%s' entry %d item %d: wrong mincountOrRef (%d) - skipped",
                        store.GetName(),
                        entry,
                        itemid,
                        mincountOrRef);
    ...
    if (mincountOrRef > 0) // item (quest or non-quest) entry, maybe grouped
    {
        ...
        {
            sLog.outErrorDb(
                "Table '%s' entry %d item %d: item entry not listed in `item_template` - skipped",
                store.GetName(),
                entry,
                itemid);
            return false;
        ...
        {
            sLog.outErrorDb("Table '%s' entry %d item %d: equal-chanced grouped entry, but group "
                            "not defined - skipped",
                            store.GetName(),
                            entry,
                            itemid);
        ...
        {
            sLog.outErrorDb(
                "Table '%s' entry %d item %d: max count (%u) less that min count (%i) - skipped",
                store.GetName(),
                entry,
                itemid,
                uint32(maxcount),
    ...
    else // mincountOrRef < 0
    {
        if (needs_quest)
            sLog.outErrorDb(
                "Table '%s' entry %d item %d: quest chance will be treated as non-quest chance",
                store.GetName(),
                entry,
                itemid);
        ...
        {
            sLog.outErrorDb(
                "Table '%s' entry %d item %d: zero chance is specified for a reference, skipped",
                store.GetName(),
                entry,
                itemid);
            return false;
...
{
    // DB conditions check
    if (conditionId && !sObjectMgr.IsPlayerMeetToCondition(
    ...
    {
        // Not quest only drop (check quest starting items for already accepted non-repeatable
        // quests)
        if (pProto->StartQuest && player->GetQuestStatus(pProto->StartQuest) != QUEST_STATUS_NONE &&
...
LootSlotType LootItem::GetSlotTypeForSharedLoot(PermissionTypes permission,
                                                Player* viewer,
                                                WorldObject const* lootTarget,
                                                bool condition_ok /*= false*/) const
{
    // ignore looted, FFA (each player get own copy) and not allowed items
    if (is_looted || freeforall || (conditionId && !condition_ok) ||
        !AllowedForPlayer(viewer, lootTarget))
        return MAX_LOOT_SLOT_TYPE;
    switch (permission)
    {
        case GROUP_PERMISSION:
            return (is_blocked || is_underthreshold) ? LOOT_SLOT_TYPE_ALLOW_LOOT :
                                                       LOOT_SLOT_TYPE_ROLL_ONGOING;
        case ALL_PERMISSION:
        case OWNER_PERMISSION: return LOOT_SLOT_TYPE_ALLOW_LOOT;
...
bool Loot::FillLoot(uint32 loot_id,
                    LootStore const& store,
                    Player* loot_owner,
                    bool personal,
                    bool noEmptyError,
                    WorldObject const* looted)
{
    // Must be provided
    if (!loot_owner)
        return false;
    ...
    {
        if (!noEmptyError)
            sLog.outErrorDb("Table '%s' loot id #%u used but it doesn't have records.",
                            store.GetName(),
                            loot_id);
        return false;
    }
...
bool Loot::IsAllowedLooter(ObjectGuid guid, bool doPersonalCheck) const
{
    ...
    for (std::vector<ObjectGuid>::const_iterator it = _allowedLooters.begin();
         it != _allowedLooters.end();
         ++it)
        if ((*it) == guid)
            return true;
    return false;
...
QuestItemList* Loot::FillNonQuestNonFFAConditionalLoot(Player* player)
{
    ...
    {
        LootItem& item = items[ i ];
        if (!item.is_looted && !item.freeforall && item.conditionId &&
    ...
    else
    {
        ...
        else if (item->conditionId && !is_looted)
        {
            QuestItemMap::const_iterator itr =
                m_playerNonQuestNonFFAConditionalItems.find(playerGuid);
...
ByteBuffer& operator<<(ByteBuffer& b, LootItem const& li)
{
    ...
    // b << uint8(0);                                        // slot type - will send after this
    // function call
    return b;
}
...
ByteBuffer& operator<<(ByteBuffer& b, LootView const& lv)
{
    ...
    switch (lv.permission)
    {
        case GROUP_PERMISSION:
        {
            ...
            for (uint8 i = 0; i < l.items.size(); ++i)
            {
                if (!l.items[ i ].is_looted && !l.items[ i ].freeforall &&
                    l.items[ i ].AllowedForPlayer(lv.viewer, l.GetLootTarget()))
                {
                    uint8 slot_type;

                    if (l.items[ i ].is_blocked)
                        slot_type = LOOT_SLOT_TYPE_ROLL_ONGOING;
                    else if (l.roundRobinPlayer == 0 || !l.items[ i ].is_underthreshold ||
        ...
        case OWNER_PERMISSION:
        {
            ...
            switch (lv.permission)
            {
                case MASTER_PERMISSION: slot_type = LOOT_SLOT_TYPE_MASTER; break;
                case OWNER_PERMISSION:
    ...
    QuestItemMap::const_iterator nn_itr =
        lootPlayerNonQuestNonFFAConditionalItems.find(lv.viewer->GetGUIDLow());
    if (nn_itr != lootPlayerNonQuestNonFFAConditionalItems.end())
    {
             ...
             ++ci)
        {
            ...
            slot_type = item.GetSlotTypeForSharedLoot(
                lv.permission, lv.viewer, l.GetLootTarget(), !ci->is_looted);
...
bool Loot::hasItemFor(Player* player) const
{
    ...
    QuestItemMap::const_iterator nn_itr =
        lootPlayerNonQuestNonFFAConditionalItems.find(player->GetGUIDLow());
    if (nn_itr != lootPlayerNonQuestNonFFAConditionalItems.end())
    {
        QuestItemList* conditional_list = nn_itr->second;
        for (QuestItemList::const_iterator ci = conditional_list->begin();
...
LootStoreItem const* LootTemplate::LootGroup::Roll(Loot const& loot) const
{
    if (!ExplicitlyChanced.empty()) // First explicitly chanced entries are checked
    {
        ...
        for (uint32 i = 0; i < ExplicitlyChanced.size(); ++i) // check each explicitly chanced entry
                                                              // in the template and modify its
                                                              // chance based on quality.
        {
...
void LootTemplate::LootGroup::Verify(LootStore const& lootstore, uint32 id, uint32 group_id) const
{
    ...
    {
        sLog.outErrorDb("Table '%s' entry %u group %d has total chance > 100%% (%f)",
                        lootstore.GetName(),
                        id,
                        group_id,
                        chance);
        sLog.out(LOG_DBERRFIX,
                 "UPDATE %s SET ChanceOrQuestChance=ChanceOrQuestChance*%f WHERE groupid=%u;",
                 lootstore.GetName(),
                 100.0f / chance,
                 group_id);
    }
    ...
    if (chance >= 100.0f && !EqualChanced.empty())
        sLog.outErrorDb("Table '%s' entry %u group %d has items with chance=0%% but group total "
                        "chance >= 100%% (%f)",
                        lootstore.GetName(),
                        id,
                        group_id,
...
void LoadLootTemplates_Creature()
{
    ...
    // this hack is used, so that we won't need to create an extra table player_loot_template for
    // just one case
    ids_set.erase(0);
...
void LoadLootTemplates_Fishing()
{
    ...
    // by default (look config options) fishing at fail provide junk loot, entry 0 use for store
    // this loot
    ids_set.erase(0);
