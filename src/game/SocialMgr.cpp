...
{
    uint32 counter = 0;
    for (PlayerSocialMap::const_iterator itr = m_playerSocialMap.begin();
         itr != m_playerSocialMap.end();
         ++itr)
    {
...
void PlayerSocial::RemoveFromSocialList(ObjectGuid friend_guid, bool ignore)
{
    ...
    {
        CharacterDatabase.PExecute(
            "DELETE FROM character_social WHERE guid = '%u' AND friend = '%u'",
            m_playerLowGuid,
            friend_guid.GetCounter());
...
void PlayerSocial::SendFriendList()
{
    ...
    for (PlayerSocialMap::iterator itr = m_playerSocialMap.begin(); itr != m_playerSocialMap.end();
         ++itr)
    {
...
void SocialMgr::GetFriendInfo(MasterPlayer* player, uint32 friend_lowguid, FriendInfo& friendInfo)
{
    ...
    MasterPlayer* pFriend =
        ObjectAccessor::FindMasterPlayer(ObjectGuid(HIGHGUID_PLAYER, friend_lowguid));
    ...
    PlayerSocialMap::const_iterator itr =
        player->GetSocial()->m_playerSocialMap.find(friend_lowguid);
    if (itr != player->GetSocial()->m_playerSocialMap.end())

        // PLAYER see his team only and PLAYER can't see MODERATOR, GAME MASTER, ADMINISTRATOR
        // characters MODERATOR, GAME MASTER, ADMINISTRATOR can see all
...
void SocialMgr::SendFriendStatus(MasterPlayer* player,
...
PlayerSocial* SocialMgr::LoadFromDB(const std::shared_ptr<QueryResult>& result, ObjectGuid guid)
{
