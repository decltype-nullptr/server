...
void TransportMgr::LoadTransportTemplates()
{
    ...
    do
    {
        ...
        if (goInfo == NULL)
        {
            sLog.outErrorDb("Transport %u has no associated GameObjectTemplate from "
        ...
        {
            sLog.outErrorDb("Transport %u (name: %s) has an invalid path specified in "
        ...
        // paths are generated per template, saves us from generating it again in case of instanced
        // transports
    ...
    sLog.outString(">> Loaded %u transport templates in %u ms",
                   count,
...
public:
    ...
    void
    operator()(uint8& mode, bool& cyclic, Movement::PointsArray& points, int& lo, int& hi) const
    {
...
void TransportMgr::GeneratePath(GameObjectInfo const* goInfo, TransportTemplate* transport)
{
    ...
    if (transport->mapsUsed.size() > 1)
    {
        ...
        transport->inInstance =
            sMapStorage.LookupEntry<MapEntry>(*transport->mapsUsed.begin())->Instanceable();
    ...
    for (size_t i = 1; i < keyFrames.size(); ++i)
    {
        if (keyFrames[ i - 1 ].Teleport || i + 1 == keyFrames.size())
        {
            ...
            spline->init_spline(
                &splinePath[ start ], i - start + extra, Movement::SplineBase::ModeCatmullrom);
            spline->initLengths();
    ...
    for (size_t i = 0; i < keyFrames.size(); ++i)
    {
        ...
        if (total_dist < 2 * accel_dist) // won't reach full speed
        {
                ...
                keyFrames[ i ].DistUntilStop) // is still accelerating
            {
                ...
                float segment_time =
                    2.0f *
                    sqrt((keyFrames[ i ].DistUntilStop + keyFrames[ i ].DistSinceStop) / accel);
                // substract acceleration time
                keyFrames[ i ].TimeTo =
                    segment_time - sqrt(2 * keyFrames[ i ].DistSinceStop / accel);
            }
            else // slowing down
                 ...
                 accel_dist) // still accelerating (but will reach full speed)
        {
            ...
            float segment_time =
                (keyFrames[ i ].DistUntilStop + keyFrames[ i ].DistSinceStop) / speed +
                (speed / accel);
            // substract acceleration time
...
Transport* TransportMgr::CreateTransport(uint32 entry, uint32 guid /*= 0*/, Map* map /*= NULL*/)
{
    ...
    if (MapEntry const* mapEntry = sMapStorage.LookupEntry<MapEntry>(mapId))
    {
        ...
        {
            sLog.outError("Transport %u (name: %s) attempted creation in instance map (id: %u) but "
                          "it is not an instanced transport!",
                          entry,
                          trans->GetName(),
                          mapId);
            delete trans;
...
void TransportMgr::SpawnContinentTransports()
{
    ...
    auto result = WorldDatabase.Query("SELECT guid, entry FROM transports");
    ...
    sLog.outString(">> Spawned %u continent transports in %u ms",
                   count,
                   WorldTimer::getMSTimeDiffToNow(oldMSTime));
}
