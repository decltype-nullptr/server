...
{
    // just deallocate movement generator, but do not Finalize since it may access to already
    // deallocated owner's memory
    while (!empty())
    {
...
void MotionMaster::DirectExpire(bool reset)
{
    ...
    typedef std::list<MovementGenerator*> MvtGenList;
    MvtGenList mvtGensToFinalize;
    while (!empty() && (top()->GetMovementGeneratorType() == CHASE_MOTION_TYPE ||
...
void MotionMaster::MoveRandom()
{
    ...
    {
        DEBUG_FILTER_LOG(
            LOG_FILTER_AI_AND_MOVEGENSS, "%s move random.", m_owner->GetGuidStr().c_str());
...
void MotionMaster::MoveTargetedHome()
{
    ...
    {
        DEBUG_FILTER_LOG(
            LOG_FILTER_AI_AND_MOVEGENSS, "%s targeted home", m_owner->GetGuidStr().c_str());
    ...
    else if (m_owner->GetTypeId() == TYPEID_UNIT && ((Creature*)m_owner)->GetCharmerOrOwnerGuid())
    {
        if (Unit* target = ((Creature*)m_owner)->GetCharmerOrOwner())
        {
            ...
            Mutate(
                new FollowMovementGenerator<Creature>(*target, PET_FOLLOW_DIST, PET_FOLLOW_ANGLE));
        }
        else
            DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS,
                             "%s attempt but fail to follow owner",
                             m_owner->GetGuidStr().c_str());
    }
    else
...
{
    DEBUG_FILTER_LOG(
        LOG_FILTER_AI_AND_MOVEGENSS, "%s move confused", m_owner->GetGuidStr().c_str());
...
{
    DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS,
                     "%s targeted point (Id: %u X: %f Y: %f Z: %f)",
                     m_owner->GetGuidStr().c_str(),
                     id,
                     x,
                     y,
...
{
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
        sLog.outError("%s attempt to call distract after assistance",
                      m_owner->GetGuidStr().c_str());
    else
    {
        DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS,
                         "%s is distracted after assistance call (Time: %u)",
                         m_owner->GetGuidStr().c_str(),
                         time);
...
void MotionMaster::MoveWaypoint(bool repeat)
{
    if (m_owner->GetTypeId() == TYPEID_UNIT)
    {
        ...
        {
            sLog.outError("Creature %s (Entry %u) attempt to MoveWaypoint() but creature is "
                          "already using waypoint",
                          m_owner->GetGuidStr().c_str(),
                          m_owner->GetEntry());
            return;
        }
        ...
        DEBUG_FILTER_LOG(LOG_FILTER_AI_AND_MOVEGENSS,
                         "Creature %s (Entry %u) start MoveWaypoint()",
                         m_owner->GetGuidStr().c_str(),
                         m_owner->GetEntry());
...
void MotionMaster::MoveTaxiFlight(uint32 path, uint32 pathnode)
{
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
    {
        if (path < sTaxiPathNodesByPath.size())
        {
            ...
            FlightPathMovementGenerator* mgen =
                new FlightPathMovementGenerator(sTaxiPathNodesByPath[ path ], pathnode);
            Mutate(mgen);
        }
        else
        {
    ...
    {
        sLog.outError(
            "%s attempt taxi to (Path %u node %u)", m_owner->GetGuidStr().c_str(), path, pathnode);
    }
}
...
void MotionMaster::Mutate(MovementGenerator* m)
{
    if (!empty())
    {
        switch (top()->GetMovementGeneratorType())
        {
            ...
            case DISTRACT_MOTION_TYPE: MovementExpired(false);
            default: break;
        }
...
{
    for (Impl::container_type::const_reverse_iterator rItr = Impl::c.rbegin();
         rItr != Impl::c.rend();
         ++rItr)
        if ((*rItr)->GetMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
            return (static_cast<WaypointMovementGenerator<Creature>*>(*rItr))
...
{
    if (m_owner->GetTypeId() == TYPEID_PLAYER)
        Mutate(new ChargeMovementGenerator<Player>(
            *(m_owner->ToPlayer()), *target, delay, triggerAutoAttack));
    else
        Mutate(new ChargeMovementGenerator<Creature>(
            *(m_owner->ToCreature()), *target, delay, triggerAutoAttack));
}
