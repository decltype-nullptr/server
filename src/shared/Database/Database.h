...
public:
    ...
    virtual std::shared_ptr<QueryResult> Query(const char* sql)           = 0;
    virtual std::shared_ptr<QueryNamedResult> QueryNamed(const char* sql) = 0;
...
public:
    ...
    inline std::shared_ptr<QueryResult> Query(const char* sql)
    {
    ...
    std::shared_ptr<QueryResult> PQuery(const char* format, ...) ATTR_PRINTF(2, 3);
    ...
    bool AsyncQuery(const char* sql,
                    Class* object,
                    void (Class::*method)(const std::shared_ptr<QueryResult>&, Args...),
                    Args... params);
    ...
    template <typename Class, typename... Args>
    bool AsyncQueryUnsafe(const char* sql,
                          Class* object,
                          void (Class::*method)(const std::shared_ptr<QueryResult>&, Args...),
                          Args... params);
    ...
    {
        template <typename... Args>
        QueryFormatter(const char* format, Args... params)
        {
            snprintf(m_buffer, MAX_QUERY_LEN, format, params...);
        }

        QueryFormatter(const char* format) { memcpy(m_buffer, format, strlen(format)); }

        char m_buffer[ MAX_QUERY_LEN ];
    };

    // PQuery / member
    template <typename Class, typename... Args>
    bool AsyncPQuery(QueryFormatter&& formatter,
                     Class* object,
                     void (Class::*method)(const std::shared_ptr<QueryResult>&, Args...),
                     Args... params);

    template <typename Class, typename... Args>
    bool AsyncPQueryUnsafe(QueryFormatter&& formatter,
                           Class* object,
                           void (Class::*method)(const std::shared_ptr<QueryResult>&, Args...),
                           Args... params);
    ...
    template <class Class>
    bool DelayQueryHolderUnsafe(Class* object,
                                void (Class::*method)(const std::shared_ptr<QueryResult>&,
                                                      SqlQueryHolder*),
                                SqlQueryHolder* holder);
    ...
    template <typename... Args>
    bool PExecute(const char* format, Args... params);

    // Writes SQL commands to a LOG file (see mangosd.conf "LogSQL")
    template <typename... Args>
    bool PExecuteLog(const char* format, Args... params);
    ...
    // must be called before first query in thread (one time for thread using one from existing
    // Database objects)
    virtual void ThreadStart();
    // must be called before finish thread run (one time for thread using one from existing Database
    // objects)
    virtual void ThreadEnd();

    // set database-wide result queue. also we should use object-bases and not thread-based result
    // queues
